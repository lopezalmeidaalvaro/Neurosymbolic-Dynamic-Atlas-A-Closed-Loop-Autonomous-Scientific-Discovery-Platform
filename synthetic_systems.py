...
def generate_lorenz(n_timesteps=5000, dt=0.01, sigma=10, rho=28, beta=8/3, initial_state=None):
    """
    ...
    for i in range(n_timesteps - 1):
        ...
        k2_x, k2_y, k2_z = lorenz_rhs(cx + 0.5*dt*k1_x, cy + 0.5*dt*k1_y, cz + 0.5*dt*k1_z)
        k3_x, k3_y, k3_z = lorenz_rhs(cx + 0.5*dt*k2_x, cy + 0.5*dt*k2_y, cz + 0.5*dt*k2_z)
...
def generate_rossler(n_timesteps=5000, dt=0.01, a=0.2, b=0.2, c=5.7, initial_state=None):
    """
    ...
    for i in range(n_timesteps - 1):
        ...
        k2_x, k2_y, k2_z = rossler_rhs(cx + 0.5*dt*k1_x, cy + 0.5*dt*k1_y, cz + 0.5*dt*k1_z)
        k3_x, k3_y, k3_z = rossler_rhs(cx + 0.5*dt*k2_x, cy + 0.5*dt*k2_y, cz + 0.5*dt*k2_z)
...
def generate_duffing(n_timesteps=5000, dt=0.01, alpha=1.0, beta=-1.0, gamma=0.3, delta=0.2, omega=1.2, initial_state=None):
    """
    ...
    def duffing_rhs(curr_t, curr_x, curr_v):
        return (
            curr_v,
            -delta * curr_v - alpha * curr_x - beta * (curr_x ** 3) + gamma * np.cos(omega * curr_t)
        )
    ...
    for i in range(n_timesteps - 1):
        ...
        k2_x, k2_v = duffing_rhs(ct + 0.5*dt, cx + 0.5*dt*k1_x, cv + 0.5*dt*k1_v)
        k3_x, k3_v = duffing_rhs(ct + 0.5*dt, cx + 0.5*dt*k2_x, cv + 0.5*dt*k2_v)
        k4_x, k4_v = duffing_rhs(ct + dt, cx + dt*k3_x, cv + dt*k3_v)
        
        x[i+1] = np.clip(cx + (dt/6.0) * (k1_x + 2*k2_x + 2*k3_x + k4_x), -50.0, 50.0)
        v[i+1] = np.clip(cv + (dt/6.0) * (k1_v + 2*k2_v + 2*k3_v + k4_v), -50.0, 50.0)
