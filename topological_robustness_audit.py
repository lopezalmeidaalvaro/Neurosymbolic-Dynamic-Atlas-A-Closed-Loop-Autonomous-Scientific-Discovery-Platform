...
def compute_topological_stability(signal, snr_levels=[30, 20, 10, 0]):
    ...
    for snr in snr_levels:
        ...
        w_dist_h0 = tda.compare_persistence_diagrams(dgms_clean[0], dgms_noisy[0], dim=0)
        w_dist_h1 = tda.compare_persistence_diagrams(dgms_clean[1], dgms_noisy[1], dim=1)
        w_dist_h2 = tda.compare_persistence_diagrams(dgms_clean[2], dgms_noisy[2], dim=2)
        ...
        print(f"    - SNR: {snr:2d} dB | H0: {w_dist_h0:.4f} | H1: {w_dist_h1:.4f} | H2: {w_dist_h2:.4f}")
...
def compute_geometric_stability(signal, snr_levels=[30, 20, 10, 0]):
    ...
    lb_eigs_clean, _ = geom.compute_laplacian_eigenmap(pc_sample_clean, n_components=5, k=6)
    ...
    for snr in snr_levels:
        ...
        try:
            lb_eigs_noisy, _ = geom.compute_laplacian_eigenmap(pc_sample_noisy, n_components=5, k=6)
            rel_error_laplace = np.mean(np.abs(lb_eigs_clean - lb_eigs_noisy) / (np.abs(lb_eigs_clean) + 1e-10))
        except Exception:
        ...
        print(f"    - SNR: {snr:2d} dB | Spearman Ricci: {spearman_ricci:.4f} | RelError LB: {rel_error_laplace:.4f}")
...
def compute_koopman_stability(signal, snr_levels=[30, 20, 10, 0]):
    ...
    for snr in snr_levels:
        noisy_sig = add_gaussian_noise(signal, snr)
        eigs_noisy, _, _, _ = koop.compute_koopman_modes(noisy_sig, emb_dim=3, lag=1, n_modes=5)
        ...
        results.append({
            "SNR": snr,
            "Wasserstein_Koopman": w_dist_koopman
        })
...
def run_full_topological_robustness_study(signal_generator_func, n_signals=10, dataset_name="Lorenz"):
    """
    ...
    json_path = os.path.join(ROOT_DIR, "artifacts", f"topological_robustness_{dataset_name.lower()}_results.json")
    ...
    fig_path = os.path.join(ROOT_DIR, "figures", f"{dataset_name.lower()}_topological_robustness.pdf")
    
    plt.style.use('seaborn-v0_8-whitegrid' if 'seaborn-v0_8-whitegrid' in plt.style.available else 'default')
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    
    # Plot TDA
    axes[0].plot(mean_tda["SNR"], mean_tda["Wasserstein_H0"], marker='o', linewidth=2.5, color='#06b6d4', label='H0 Components')
    axes[0].plot(mean_tda["SNR"], mean_tda["Wasserstein_H1"], marker='s', linewidth=2.5, color='#f43f5e', label='H1 Loops')
    axes[0].plot(mean_tda["SNR"], mean_tda["Wasserstein_H2"], marker='^', linewidth=2.5, color='#10b981', label='H2 Voids')
    ...
    axes[0].set_title("Topological Stability (TDA Persistence)", fontweight='bold', pad=12)
    axes[0].invert_xaxis()
    axes[0].legend()
    
    # Plot Geometric
    ax_geom_twin = axes[1].twinx()
    p1, = axes[1].plot(mean_geom["SNR"], mean_geom["Spearman_Ricci"], marker='o', linewidth=2.5, color='#8b5cf6', label='Spearman Ricci')
    p2, = ax_geom_twin.plot(mean_geom["SNR"], mean_geom["RelError_Laplace"], marker='s', linewidth=2.5, color='#f59e0b', label='LB Eigenvalue RelError')
    ...
    ax_geom_twin.set_ylabel("Laplace-Beltrami Mean Relative Error", color='#f59e0b', fontweight='bold')
    ...
    # Plot Koopman
    axes[2].plot(mean_koop["SNR"], mean_koop["Wasserstein_Koopman"], marker='d', linewidth=2.5, color='#ec4899', label='Koopman Spectrum')
    ...
    axes[2].set_title("Koopman Operator Eigenvalue Stability", fontweight='bold', pad=12)
    axes[2].invert_xaxis()
    axes[2].legend()
    
    plt.suptitle(f"Topological & Geometric Robustness Integrity Audit — {dataset_name}", fontsize=14, fontweight='bold', y=1.02)
    plt.tight_layout()
...
if __name__ == "__main__":
    print("Executing Topological Robustness Study on Lorenz Attractor and ECG200 dataset...")
    ...
    try:
        ...
        run_full_topological_robustness_study(ecg_gen, n_signals=3, dataset_name="ECG200")
    except Exception as e:
