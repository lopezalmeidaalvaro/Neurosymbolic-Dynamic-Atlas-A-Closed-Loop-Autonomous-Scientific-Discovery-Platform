...
def run_tests():
    ...
    try:
        ...
        assert "x" in eq_str, f"Discovered formula does not contain variable 'x': {eq_str}"
    ...
    try:
        ...
        assert successful_recoveries >= 3, f"Fewer than 3 successful recoveries: {successful_recoveries}"
        print("  Test 4 PASSED: Benchmark benchmark executed successfully with high recovery rate.")
    ...
    try:
        ...
        penalty = symbolic_discovery.add_physics_penalty(eq_wrong, expected_terms=["x * z"])
        print(f"  Wrong equation: '{eq_wrong}' | Expected: ['x * z'] | Penalty score: {penalty}")
        assert penalty > 0.0, f"Expected penalty to be positive, got {penalty}"
        
        eq_right = "-0.2*v - 1.0*x - -1.0*x**3 + 0.3*cos(1.2*t)"
        penalty_right = symbolic_discovery.add_physics_penalty(eq_right, expected_terms=["v"])
        print(f"  Right equation: '{eq_right}' | Expected: ['v'] | Penalty score: {penalty_right}")
        assert penalty_right == 0.0, f"Expected penalty to be zero, got {penalty_right}"
        
        print("  Test 5 PASSED: Physics penalty correctly penalizes symmetry/term violations.")
