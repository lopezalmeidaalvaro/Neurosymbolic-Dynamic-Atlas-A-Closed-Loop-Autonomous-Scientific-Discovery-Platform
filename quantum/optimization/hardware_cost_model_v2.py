...
DEFAULT_TWO_QUBIT_ERROR = 5e-3  # CNOT error typically around 0.5%
DEFAULT_READOUT_ERROR = 1.2e-2  # Readout typically around 1.2%
DEFAULT_T1_SEC = 150e-6        # T1 around 150 microseconds
DEFAULT_T2_SEC = 90e-6         # T2 around 90 microseconds
...
def _native_circuit(circuit: QuantumCircuit, backend: Any) -> QuantumCircuit:
    try:
        # Intentar transpilar con scheduling para obtener duraciones reales en segundos
        return transpile(
            circuit,
            backend=backend,
...
def estimate_physical_cost(
    ...
    """
    Estimate physical execution cost using corrected costing principles.
    - Gate errors are evaluated only on two-qubit gates.
    - Readout errors are evaluated only on measured qubits.
    - Coherence decay is evaluated only on active qubits based on the critical path duration.
    """
    ...
    for instruction in native.data:
        ...
        if op_name.lower() not in ("barrier", "delay"):
        ...
        # Solo acumula errores de compuerta en compuertas de dos qubits (CNOT, ECR, etc.)
        if len(qargs) == 2:
            two_qubit_count += 1
            log_gate_fidelity += math.log(max(1e-15, 1.0 - error))
        elif len(qargs) == 1:
            # Acumular un error menor para compuertas de un qubit (10x menor)
            log_gate_fidelity += math.log(max(1e-15, 1.0 - error * 0.1))
    ...
    # Si no se detectaron compuertas de medida, asumimos que todos los qubits activos son medidos
    if not measured_qubits:
        measured_qubits = used_qubits.copy()

    # Si no hay qubits activos, asumimos los qubits virtuales básicos del circuito
    if not used_qubits:
        used_qubits = set(range(native.num_qubits))
        measured_qubits = used_qubits.copy()

    # Readout fidelity solo sobre qubits medidos
    ...
    # Coherence fidelity solo sobre los qubits utilizados
    log_coherence_fidelity = 0.0
    for q in used_qubits:
        quality = qubit_quality.get(q) or get_qubit_quality(backend, q)
        qubit_quality[q] = quality
        t1 = max(quality["t1"], 1e-15)
        t2 = max(quality["t2"], 1e-15)
        # Decaimiento por coherencia sobre la duración crítica para cada qubit activo
        decay = -(critical_duration_sec / t1) - (critical_duration_sec / t2)
        log_coherence_fidelity += decay * 0.5
...
def predict_hellinger_fidelity(circuit: CircuitLike, backend: Any) -> float:
    """Predice la fidelidad de Hellinger absoluta para el circuito en el backend."""
    cost = estimate_physical_cost(circuit, backend)
    return cost["estimated_fidelity"]


def compare_compilers(circuit_a: CircuitLike, circuit_b: CircuitLike, backend: Any) -> Dict[str, Any]:
    """Compara dos compiladores A y B sobre el mismo backend y predice cuál será mejor."""
    cost_a = estimate_physical_cost(circuit_a, backend)
    cost_b = estimate_physical_cost(circuit_b, backend)
    
    fid_a = cost_a["estimated_fidelity"]
    fid_b = cost_b["estimated_fidelity"]
    delta = fid_b - fid_a
    ...
    return {
        ...
        "fidelity_b": fid_b,
        "delta": delta,
        "recommended": "B" if delta > 0 else "A"
    }
