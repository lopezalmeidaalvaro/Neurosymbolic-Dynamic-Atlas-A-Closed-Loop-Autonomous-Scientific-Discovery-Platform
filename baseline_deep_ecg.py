...
def main():
    ...
    print(f"    - Train set (DS1) : {X_train.shape[0]} windows of length {X_train.shape[1]}")
    print(f"    - Test set (DS2)  : {X_test.shape[0]} windows of length {X_test.shape[1]}")
    print(f"    - Class Balance (Train): {np.sum(y_train == 1)} Normal (N) / {np.sum(y_train == 0)} PVC (V)")
    print(f"    - Class Balance (Test) : {np.sum(y_test == 1)} Normal (N) / {np.sum(y_test == 0)} PVC (V)")
    ...
    for epoch in range(epochs):
        ...
        print(f"  Epoch {epoch+1:02d}/50 | Train Loss: {epoch_train_loss:.4f} | Val Loss: {epoch_val_loss:.4f} | Val AUC: {epoch_val_auc:.4f}")
