...
def compute_koopman_modes(signal, emb_dim=3, lag=1, n_modes=5):
    ...
    if n_points < 10:
        print("  [KOOPMAN WARNING] Point cloud too small for Koopman DMD. Returning empty arrays.")
        return np.zeros(n_modes, dtype=complex), np.zeros((n_dims, n_modes), dtype=complex), np.zeros(n_modes), np.zeros(n_modes)
    ...
    try:
        ...
        print(f"  [KOOPMAN ERROR] DMD SVD solver failed ({e}). Returning zero fallbacks.")
        return np.zeros(n_modes, dtype=complex), np.zeros((n_dims, n_modes), dtype=complex), np.zeros(n_modes), np.zeros(n_modes)
...
def extract_koopman_features(signal, emb_dim=3, lag=1, n_modes=5):
    ...
    if len(point_cloud) < 10:
        print("  [KOOPMAN WARNING] Signal too short to extract Koopman features. Returning NaNs.")
        return feat_vector
        
    try:
        eigs, modes, freqs, growths = compute_koopman_modes(signal, emb_dim=emb_dim, lag=lag, n_modes=n_modes)
...
def compute_koopman_invariants(signal, emb_dim=3, lag=1):
    ...
    try:
        eigs, modes, _, _ = compute_koopman_modes(signal, emb_dim=emb_dim, lag=lag, n_modes=n_dims)
