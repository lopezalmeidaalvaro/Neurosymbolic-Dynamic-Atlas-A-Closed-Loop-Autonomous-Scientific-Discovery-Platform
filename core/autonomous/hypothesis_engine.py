...
def evaluate_hypotheses(analysis_results: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for r in runs:
        ...
        if emb_acc < dtw_acc:
            h3_passed = False
            h3_reasons.append(f"Embedding_V2 accuracy ({emb_acc}) < DTW ({dtw_acc}) at noise={r['noise_level']}")
            
    if h3_passed:
        h3_status = "VALIDATED"
        h3_evidence = "Embedding_V2 maintained accuracy >= DTW accuracy across all noise levels."
    else:
        h3_status = "PARTIALLY_VALIDATED"
        h3_evidence = f"Accuracy dominance failed in some configurations: {'; '.join(h3_reasons)}"
