        ...
        for ext in exts:
            ...
            if not os.path.exists(local_path) or os.path.getsize(local_path) == 0:
                ...
                try:
                    ...
                    print(f"  ⚠️ Error downloading {filename} via urllib: {e}. Trying wfdb fallback.")
                    # Fallback to wfdb dl_files
                    try:
                        wfdb.dl_files("mitdb", DATA_DIR, [filename])
                        download_count += 1
                    except Exception as e2:
                        raise RuntimeError(f"Failed to ingest record file {filename}: {e2}")
...
def main():
    ...
    print("=" * 80)
    print("🔬 COMPUTATIONAL PHYSICS AUDITOR & CLINICAL VALIDATION — MIT-BIH BIFURCATED AUDIT")
    print("=" * 80)
    ...
    causal_status = "CAUSAL_MECHANISM_STABLE" if d_shap < 0.1 else "CAUSAL_MECHANISM_REORGANIZED"
