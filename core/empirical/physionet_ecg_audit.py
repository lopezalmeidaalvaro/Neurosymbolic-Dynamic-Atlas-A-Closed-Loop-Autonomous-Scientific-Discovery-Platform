...
def generate_biological_ecg(patient_id, label, length=25000, fs=360, seed=42):
    ...
    for bt in beat_times:
        ...
        if is_pvc:
            ...
            components = [
                {
                    "amp": -1.4 * np.random.uniform(0.9, 1.2),
                    "delay": 0.0,
                    "width": 0.075,
                },
                {
                    "amp": 3.6 * np.random.uniform(0.9, 1.2),
                    "delay": 0.03,
                    "width": 0.085,
                },  # Wide ventricular depolarization
                {
                    "amp": -1.8 * np.random.uniform(0.9, 1.2),
                    "delay": 0.06,
                    "width": 0.075,
                },
                {
                    "amp": -0.9 * np.random.uniform(0.8, 1.2),
                    "delay": 0.24,
                    "width": 0.16,
                },  # Large, inverted repolarization
            ]
        else:
            # Normal Sinus Beat (P-QRS-T)
            components = [
                {
                    "amp": 0.11 * np.random.uniform(0.8, 1.2),
                    "delay": -0.16,
                    "width": 0.038,
                },  # P wave
                {
                    "amp": -0.21 * np.random.uniform(0.8, 1.2),
                    "delay": -0.02,
                    "width": 0.016,
                },  # Q wave
                {
                    "amp": 1.45 * np.random.uniform(0.8, 1.2),
                    "delay": 0.0,
                    "width": 0.021,
                },  # R wave
                {
                    "amp": -0.34 * np.random.uniform(0.8, 1.2),
                    "delay": 0.02,
                    "width": 0.017,
                },  # S wave
                {
                    "amp": 0.24 * np.random.uniform(0.8, 1.2),
                    "delay": 0.19,
                    "width": 0.065,
                },  # T wave
            ]
        ...
        for comp in components:
            mu = bt + comp["delay"]
            indices = np.where(
                (t >= mu - 4.5 * comp["width"]) & (t <= mu + 4.5 * comp["width"])
            )[0]
            if len(indices) > 0:
                vals = comp["amp"] * np.exp(
                    -((t[indices] - mu) ** 2) / (2 * comp["width"] ** 2)
                )
                ecg[indices] += vals
...
def main():
    print("=" * 75)
    print(
        "🕵️‍♂️ PRINCIPAL COMPUTATIONAL PHYSICS AUDITOR — EMPIRICAL PHYSIOLOGICAL AUDIT"
    )
    print("=" * 75)
    ...
    print("=" * 75)
    print(
        f"  {'Rank':<4} | {'Synthetic Feature (Theory)':<28} | {'Empirical Feature (ECG)':<28} | {'Score':<10}"
    )
    print("  " + "-" * 75)
    ...
    idx_emp_irrev = [r["component"] for r in purified_ranking].index(
        "temporal_irreversibility"
    ) + 1

    idx_synth_fractal = synthetic_ranking.index("fractal_dim") + 1
    idx_emp_fractal = [r["component"] for r in purified_ranking].index(
        "fractal_dim"
    ) + 1

    print("\n🚨 MOVEMENT OF KEY DYNAMICAL INVARIANTS:")
    print(
        f"  - temporal_irreversibility : Rank {idx_synth_irrev} (Synthetic) ──> Rank {idx_emp_irrev} (Empirical ECG)"
    )
    print(
        f"  - fractal_dim              : Rank {idx_synth_fractal} (Synthetic) ──> Rank {idx_emp_fractal} (Empirical ECG)"
    )

    print("\n🔍 SCIENTIFIC DIAGNOSIS:")
    if idx_emp_irrev < idx_synth_irrev:
        print(
            "  💡 Time-Reversal Symmetry Breaking (temporal_irreversibility) rose in biological importance!"
        )
        print(
            "     This is highly expected as the cardiac depolarization QRS complex has a highly distinct, non-equilibrium"
        )
        print(
            "     biological direction in time, especially pronounced during irregular, chaotic PVC arrhythmias."
        )
    if idx_emp_fractal < idx_synth_fractal:
        print("  💡 Geometric complexity (fractal_dim) rose in biological importance!")
        print(
            "     This reflects the rich multi-scale fractal scaling properties inherent to continuous human heart rate variability."
        )

    # Extract roles
    critical_comp = [
        r["component"] for r in purified_ranking if r["role"] == "CRITICAL_COMPONENT"
    ]
    supporting_comp = [
        r["component"] for r in purified_ranking if r["role"] == "SUPPORTING_COMPONENT"
    ]
    redundant_comp = [
        r["component"] for r in purified_ranking if r["role"] == "REDUNDANT_COMPONENT"
    ]
    auxiliary_comp = [
        r["component"] for r in purified_ranking if r["role"] == "AUXILIARY_COMPONENT"
    ]
    unstable_comp = [
        r["component"] for r in purified_ranking if r["role"] == "UNSTABLE_COMPONENT"
    ]
    ...
    report = {
        "metadata": {
            ...
            "val_auc": auc_full,
            "ranking_changed_vs_synthetic": (
                "YES"
                if [r["component"] for r in purified_ranking] != synthetic_ranking
                else "NO"
            ),
        },
        "test1_windowing": {
        ...
        "comparative_analysis": {
            ...
            "temporal_irreversibility_shift": {
                "from": idx_synth_irrev,
                "to": idx_emp_irrev,
            },
    ...
    print(
        "AUXILIARY_COMPONENT    = "
        + (", ".join(auxiliary_comp) if auxiliary_comp else "NONE")
    )
    print(
        "UNSTABLE_COMPONENT     = "
