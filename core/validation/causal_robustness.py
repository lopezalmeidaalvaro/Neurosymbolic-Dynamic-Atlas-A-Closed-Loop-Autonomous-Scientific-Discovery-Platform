...
def main():
    ...
    for sys_name in physical_systems:
        ...
        system_projections[sys_name] = {
            "PCA": {}, "UMAP": {}, "TSNE": {}, "shifts": snapshot_shifts
        }
        ...
        for cfg, start, end in snapshot_slices:
            ...
            system_projections[sys_name]["PCA"][cfg] = compute_topological_metrics(pts_pca)
            system_projections[sys_name]["UMAP"][cfg] = compute_topological_metrics(pts_umap)
            system_projections[sys_name]["TSNE"][cfg] = compute_topological_metrics(pts_tsne)

    # 3. PROJECTION CONSISTENCY SCORE
    # Compare metrics across all snapshots (configs) using Spearman rho
    primary_metrics = ["covariance_determinant", "nearest_neighbor_distance_mean", "cluster_count"]
    ...
    for sys_name in physical_systems:
        ...
        for metric in primary_metrics:
            vals_pca = [system_projections[sys_name]["PCA"][cfg][metric] for cfg in configs]
            vals_umap = [system_projections[sys_name]["UMAP"][cfg][metric] for cfg in configs]
            vals_tsne = [system_projections[sys_name]["TSNE"][cfg][metric] for cfg in configs]
            
            rho_umap, _ = spearmanr(vals_pca, vals_umap)
            rho_tsne, _ = spearmanr(vals_pca, vals_tsne)
            
            if not np.isfinite(rho_umap): rho_umap = 0.0
            if not np.isfinite(rho_tsne): rho_tsne = 0.0
    ...
    null_models = ["white_noise", "pink_noise", "brown_noise", "random_walk", "phase_surrogate"]
    ...
    for nm in null_models:
        for seed in SEEDS:
            ...
            for noise in ROSSLER_NOISE:
                ...
                noise_std = effective_noise * std_val if std_val > 0 else effective_noise
    ...
    for sys_name in physical_systems:
        ...
        for metric in ["covariance_determinant", "nearest_neighbor_distance_mean", "nearest_neighbor_distance_std", "cluster_count"]:
            metric_means = []
            for noise in noises:
                vals = [system_projections[sys_name]["PCA"][(noise, seed)][metric] for seed in SEEDS]
            ...
            # Cohen's d
            vals_baseline = [system_projections[sys_name]["PCA"][(0.0, seed)][metric] for seed in SEEDS]
            vals_max_noise = [system_projections[sys_name]["PCA"][(max(noises), seed)][metric] for seed in SEEDS]
    ...
    for nm in null_models:
        ...
        for metric in ["covariance_determinant", "nearest_neighbor_distance_mean", "nearest_neighbor_distance_std", "cluster_count"]:
            ...
            # Cohen's d
            vals_baseline = [null_projections[nm][(0.0, seed)][metric] for seed in SEEDS]
            vals_max_noise = [null_projections[nm][(max(ROSSLER_NOISE), seed)][metric] for seed in SEEDS]
            d_val = compute_cohens_d(vals_baseline, vals_max_noise)
    ...
    for sys_name in physical_systems:
        ...
        mean_umap_rho = np.mean([cons["comparisons"][m]["PCA_UMAP_rho"] for m in ["covariance_determinant", "nearest_neighbor_distance_mean"]])
        mean_tsne_rho = np.mean([cons["comparisons"][m]["PCA_TSNE_rho"] for m in ["covariance_determinant", "nearest_neighbor_distance_mean"]])
    ...
    print("-" * 75)
    for nm in null_models:
        for metric in ["covariance_determinant", "nearest_neighbor_distance_mean", "nearest_neighbor_distance_std"]:
            res = null_results[nm][metric]
            print(f"{nm:<15} | {metric:<30} | {res['spearman_rho']:10.4f} | {res['adjusted_p']:12.4e}")
    ...
    for sys_name in physical_systems:
        for metric in primary_topological:
            ...
            if abs(res["spearman_rho"]) <= 0.8:
                ...
                approval_reasons.append(f"Physical system '{sys_name}' metric '{metric}' has |rho| = {abs(res['spearman_rho']):.4f} <= 0.8")
            if res["adjusted_p"] >= 0.05:
                approval_failed = True
                approval_reasons.append(f"Physical system '{sys_name}' metric '{metric}' has adjusted_p = {res['adjusted_p']:.4e} >= 0.05")
            if res["cohen_d"] <= 0.8:
                approval_failed = True
                approval_reasons.append(f"Physical system '{sys_name}' metric '{metric}' has Cohen's d = {res['cohen_d']:.4f} <= 0.8")
    ...
    for sys_name in physical_systems:
        ...
        if score <= 0.85:
            approval_failed = True
            approval_reasons.append(f"Physical system '{sys_name}' projection consistency score = {score:.4f} <= 0.85")
    ...
    for nm in null_models:
        ...
        for metric in primary_topological:
            res = null_results[nm][metric]
            if res["spearman_rho"] >= -0.8 or res["adjusted_p"] >= 0.05 or res["cohen_d"] <= 0.8:
                passed_all_primary = False
                break
        if passed_all_primary:
            approval_failed = True
            approval_reasons.append(f"Null model '{nm}' shows equivalent collapse (passed all physical thresholds with strong negative correlation)")
    ...
    for sys_name in physical_systems:
        ...
        if mean_sh >= 0.1:
            approval_failed = True
            approval_reasons.append(f"Physical system '{sys_name}' mean embedding shift = {mean_sh:.6f} >= 0.1")
    ...
    report_data = {
        ...
        "physical_validation": {
            sys_name: {
                ...
                "embedding_stability": {
                    "mean_shift": float(np.mean([s[1] for s in system_projections[sys_name]["shifts"]])),
                    "std_shift": float(np.mean([s[2] for s in system_projections[sys_name]["shifts"]]))
                }
            } for sys_name in physical_systems
        },
        "null_hypothesis_validation": {
            nm: {
                "metrics": null_results[nm]
            } for nm in null_models
        }
    }
