...
def main():
    ...
    for t in trajectories:
        ...
        t["group_id"] = f"{t['system']}_{t['parameter_bin']}_{ic_cluster}_{t['trajectory_id']}"
    ...
    if strict_auc_base > 0.80:
        bias_detected = True
        print(f"  ⚠️ [DATASET_BIAS_DETECTED]: Baseline features achieve high ROC-AUC = {strict_auc_base:.6f}!")
    else:
        print(f"  ✅ No Dataset Bias: Baseline features ROC-AUC = {strict_auc_base:.6f}")
    ...
    if mean_perm_auc > 0.55:
        ...
        print(f"  ❌ [PIPELINE_LEAKAGE_DETECTED]: Mean Permuted AUC = {mean_perm_auc:.6f} (> 0.55)!")
    else:
