...
def main():
    ...
    for i, name in enumerate(V3_KEYS):
        ...
        print(f"  Component: {name:<26} | Necessity={necessity: .6f} | Sufficiency={auc_io:.6f} ({'Sufficient' if is_sufficient else 'Insufficient'})")
    ...
    for i, name in enumerate(V3_KEYS):
        ...
        print(f"  Component: {name:<26} | Mean Importance={mu_i:.6f} | Std={sigma_i:.6f} | CV={cv_i:.6f} ({'UNSTABLE' if cv_i > 0.5 else 'STABLE'})")
    ...
    for rank in roles_ranking:
        print(f"  - {rank['component']:<26} | Score={rank['score']: .6f} | Role={rank['role']}")
        
    # Extract structural summaries for terminal print
    critical_comp = [r["component"] for r in roles_ranking if r["role"] == "CRITICAL_COMPONENT"]
    supporting_comp = [r["component"] for r in roles_ranking if r["role"] == "SUPPORTING_COMPONENT"]
    redundant_comp = [r["component"] for r in roles_ranking if r["role"] == "REDUNDANT_COMPONENT"]
    negligible_comp = [r["component"] for r in roles_ranking if r["role"] == "NEGLIGIBLE_COMPONENT"]
    unstable_comp = [r["component"] for r in roles_ranking if r["role"] == "UNSTABLE_COMPONENT"]
    ...
    print("NEGLIGIBLE_COMPONENT   = " + (", ".join(negligible_comp) if negligible_comp else "NONE"))
