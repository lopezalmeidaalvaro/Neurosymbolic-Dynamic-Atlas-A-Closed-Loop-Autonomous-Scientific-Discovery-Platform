...
def simulate_physical_perturbed_closure(
    ...
    if system == "lorenz":
        ...
        def rhs(t, state):
            ...
            return [sigma * (y_st - x_st), x_st * (rho - z_st) - y_st, x_st * y_st - beta * z_st]
        ...
        def rhs(t, state):
            x_st, y_st = state
            return [y_st, x_st - x_st**3 - delta_param * y_st + gamma * np.cos(omega * t)]
...
def compute_local_density_correlation(A: np.ndarray, B: np.ndarray, k: int = 15) -> float:
    nbrs_A = NearestNeighbors(n_neighbors=k + 1).fit(A)
    nbrs_B = NearestNeighbors(n_neighbors=k + 1).fit(B)
...
def main():
    ...
    for lvl in noise_levels:
        ...
        print(f"Noise {lvl*100:>2.0f}% | NN_Overlap={nn_over:.4f} | DistCorr={dist_corr:.4f} | CovSim={cov_sim:.4f} | CKA={cka_sim:.4f} | ProcSim={proc_sim:.4f}")
    ...
    for key, values in metrics_history.items():
        ...
        for i in range(1, len(values)):
            ...
            if drop > 0.30:
                passed_test1 = False
                print(f"  [CRITICAL JUMP] {key} dropped by {drop:.4f} (>0.30) from noise {noise_levels[i-1]} to {noise_levels[i]}!")
        jumps[key] = key_jumps
    ...
    print(f"LOCAL Structure Metrics (Mean across noise levels):")
    ...
    print(f"GLOBAL Structure Metrics (Mean across noise levels):")
    ...
    if local_score < 0.80 and global_score >= 0.80:
        ...
        diagnosis = "EXPECTED_PHYSICAL_LOCAL_REORGANIZATION" if global_score >= 0.80 else "EMBEDDING_FRAGILITY"
    ...
    for name, kwargs in perturbations.items():
        ...
        print(f"  {name:<20} | ProcSim={proc_sim:.4f} | DistCorr={dist_corr:.4f} | NNOver={nn_over:.4f} | Disparity={disparity:.4f}")
        
    ranked_perts = sorted(pert_results.items(), key=lambda item: item[1]["disparity_distortion"], reverse=True)
    ...
    for idx, (name, res) in enumerate(ranked_perts):
        print(f"  {idx+1}. {name:<20} : Disparity/Distortion = {res['disparity_distortion']:.4f}")
    ...
    print(f"Classifier Stability Results over 20 Seeds:")
    print(f"  ROC-AUC: mean = {mean_auc:.4f}, std = {std_auc:.4f}, CV = {cv_auc*100:.2f}%")
    ...
    if cert_status in ["PASS", "CONDITIONAL_PASS"]:
        ...
        print(f"Instability is a physical expected local reorganization, not mathematical fragility.")
        print("=" * 80)
        return 0
    else:
