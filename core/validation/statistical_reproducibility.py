...
def main():
    ...
    if not os.path.exists(SNAPSHOTS_FILE):
        print(
            f"❌ Snapshots file not found at {SNAPSHOTS_FILE}. Run latent_snapshot_exporter.py first."
        )
        sys.exit(1)
    ...
    if min_pc < required_min_points:
        ...
        fail_reasons.append(
            f"min(point_count) ({min_pc}) is less than required_min_points ({required_min_points})"
        )
    ...
    for sys_entry in systems_data:
        ...
        print(
            f"Auditing reproducibility for sweep system: {sys_name} ({len(unique_noises)} noise levels, max {max_seeds_per_noise} seeds)"
        )
        ...
        for metric in all_metrics:
            ...
            reproducibility_results[sys_name][metric] = {
                ...
                "noise_means": {
                    float(n_val): float(m_val)
                    for n_val, m_val in zip(noise_levels, metric_means)
                },
                "noise_cvs": {
                    float(n_val): float(cv_val)
                    for n_val, cv_val in zip(noise_levels, cv_values)
                },
            }
        ...
        if failed_rho_count == len(primary_metrics):
            system_failed = True
            system_reasons.append(
                f"|rho| < 0.5 for all primary metrics (rho values: {[reproducibility_results[sys_name][m]['spearman_rho'] for m in primary_metrics]})"
            )
        if failed_pvalue_count == len(primary_metrics):
            system_failed = True
            system_reasons.append(
                f"pvalue >= 0.05 for all primary metrics (pvalues: {[reproducibility_results[sys_name][m]['spearman_pvalue'] for m in primary_metrics]})"
            )
        if failed_cv_count == len(primary_metrics):
            system_failed = True
            system_reasons.append(
                f"CV > 0.5 for all primary metrics (CV values: {[reproducibility_results[sys_name][m]['mean_cv'] for m in primary_metrics]})"
            )
    ...
    print(
        f"{'System':<15} | {'Metric':<30} | {'Spearman rho':<12} | {'p-value':<10} | {'Mean CV':<8} | {'Mean CI95 width':<15}"
    )
    print("-" * 105)
    for sys_name in systems_checked:
        for metric in all_metrics:
            ...
            print(
                f"{sys_name:<15} | {metric:<30} | {rho:12.4f} | {pval:10.4e} | {cv:8.4f} | {ci95_w:15.6f}"
            )
    print("-" * 105)
