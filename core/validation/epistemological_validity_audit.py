...
REPORT_DIR = os.path.join(
    REPORT_DIR := os.path.join(
        ROOT_DIR, "dashboard", "public", "artifacts", "discoveries"
    )
)
...
def main():
    ...
    for i, name in enumerate(V3_KEYS):
        ...
        stability_shap_results[name] = {"mean": mu_i, "std": sigma_i, "cv": cv_i}
    ...
    for i, name in enumerate(V3_KEYS):
        ...
        is_regularizer = (
            C_i < 0.15 and loco_auc_necessity[name] > 0.001 and auc_full > 0.95
        )
    ...
    print("=" * 75)
    print(
        f"  {'Rank':<4} | {'Pearson/Gini Feature':<28} | {'Purified NMI/SHAP Feature':<28} | {'Score':<10}"
    )
    print("  " + "-" * 75)
    ...
    ranking_changed = (
        "NO"
        if [r["component"] for r in purified_ranking] == gini_pearson_ranking
        else "YES"
    )
    ...
    regularizer_comp = [
        r["component"] for r in purified_ranking if r["role"] == "GEOMETRIC_REGULARIZER"
    ]
    unstable_comp = [
    ...
    print(
        ...
        + (", ".join(regularizer_comp) if regularizer_comp else "NONE")
    )
    print(
        "AUXILIARY_COMPONENT    = "
