...
def main():
    ...
    cumulative_variance = {}
    for comp in [2, 3, 5, 8]:
        explained_variance_ratio[str(comp)] = float(evr[comp - 1]) if comp <= len(evr) else 0.0
    ...
    print(f"PCA(2) < 80%? {'YES (FAILURE)' if pca_compression_failure else 'NO (PASS)'}")
        ...
        for md in min_dist_sweep:
            for met in metrics_sweep:
                ...
                try:
                    ...
                    print(f"[{run_idx}/{total_runs}] Error sweeping UMAP(nn={nn}, md={md}, metric={met}): {e}")
                    
    # Sort sweep results: we prefer those that pass all metrics first, then sort by highest composite score
    sweep_results.sort(key=lambda item: (item["passed_all"], item["composite_score"]), reverse=True)
    ...
    try:
        ...
        baseline_comp = (baseline_sil / 0.30) + (baseline_trust / 0.95) + (baseline_dcor / 0.80)
        baseline_passed = (baseline_sil >= 0.30) and (baseline_trust >= 0.95) and (baseline_dcor >= 0.80)
    except Exception as e:
        print(f"Error computing exact baseline: {e}")
        baseline_sil, baseline_trust, baseline_dcor, baseline_comp, baseline_passed = 0.047930, 0.913689, 0.651027, 2.0, False
    ...
    print(f"Baseline: Silhouette={baseline_stats['silhouette']:.6f}, Trustworthiness={baseline_stats['trustworthiness']:.6f}, Distance Correlation={baseline_stats['distance_correlation']:.6f}")
    if best_config:
        print(f"Best Configuration: n_neighbors={best_config['n_neighbors']}, min_dist={best_config['min_dist']}, metric={best_config['metric']}")
        print(f"  Silhouette={best_config['silhouette']:.6f} (Passed: {best_config['passed_silhouette']})")
        print(f"  Trustworthiness={best_config['trustworthiness']:.6f} (Passed: {best_config['passed_trustworthiness']})")
        print(f"  Distance Correlation={best_config['distance_correlation']:.6f} (Passed: {best_config['passed_distance_correlation']})")
    ...
    cont_disc_evidence = (
        max(continuous_results["silhouette"], discrete_results["silhouette"]) - baseline_stats["silhouette"]
    )
    ...
    subopt_hyperparams_evidence = best_config["composite_score"] - baseline_stats["composite_score"] if best_config else 0.0
    ...
    print("\n" + "=" * 80)
    print(f"FINAL AUDIT RESULT: Most likely failure source = {most_likely_failure_source}")
    print("=" * 80)
