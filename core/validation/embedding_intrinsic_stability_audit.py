...
def _simulate_henon_local(
    ...
    for i in range(n_points + transient):
        ...
        if i >= transient:
            values.append(x)
    return Signal(_inject_noise_local(np.asarray(values, dtype=float), noise, seed), 1.0)
...
def build_dataset_perturbed(seeds: list[int], perturb: bool = True, noise_override: float = None) -> dict[str, np.ndarray]:
    dataset = {}
...
def main():
    ...
    seed_dist_corr = float((pearsonr(dist_s42, dist_s1337)[0] + pearsonr(dist_s42, dist_s9001)[0]) / 2.0)
    seed_nn_overlap = float((compute_nn_overlap(X_s42, X_s1337) + compute_nn_overlap(X_s42, X_s9001)) / 2.0)
    seed_cov_sim = float((compute_covariance_similarity(X_s42, X_s1337) + compute_covariance_similarity(X_s42, X_s9001)) / 2.0)
    ...
    for noise in noise_levels:
        dataset_noise = build_dataset_perturbed([42, 1337, 9001], perturb=False, noise_override=noise)
        ...
        print(f"Noise {noise*100:>2.0f}% | DistCorr={d_corr:.4f} | NN_Overlap={nn_over:.4f} | CovSim={cov_sim:.4f} | CosPres={cos_pres:.4f} | IntrinsicDim={noise_dim:.4f}")
        
    mean_noise_dist_corr = float(np.mean([item["distance_correlation"] for item in noise_results]))
    mean_noise_nn_overlap = float(np.mean([item["neighbor_overlap"] for item in noise_results]))
    mean_noise_cov_sim = float(np.mean([item["covariance_similarity"] for item in noise_results]))
    
    # Overall Test 1 status
    passed_test1 = (
        seed_dist_corr >= THRES_DIST_CORR and
        seed_nn_overlap >= THRES_NEIGHBOR_OVERLAP and
        seed_cov_sim >= THRES_COV_SIM and
        mean_noise_dist_corr >= THRES_DIST_CORR and
        mean_noise_nn_overlap >= THRES_NEIGHBOR_OVERLAP and
        mean_noise_cov_sim >= THRES_COV_SIM
    )
    ...
    print(f"Topology Persistence (Seed 42 vs 1337):")
    ...
    dataset_n5 = build_dataset_perturbed([42, 1337, 9001], perturb=False, noise_override=0.05)
    ...
    for i, seed in enumerate(resample_seeds):
        try:
            ...
            print(f"  Dataset {i+1:>2} | Sil={sil:.4f} | Trust={trust:.4f} | DCor={dcor:.4f}")
    ...
    passed_test3 = (
        std_r_sil > 1e-6 and
        std_r_trust > 1e-6 and
        std_r_dcor > 1e-6 and
        ci_sil_lower >= 0.30 and
        ci_trust_lower >= 0.95 and
        ci_dcor_lower >= 0.80
    )
    ...
    def compute_projection_stability(P_base: np.ndarray, P_pert: np.ndarray) -> dict[str, float]:
        d_base = pdist(P_base)
        d_pert = pdist(P_pert)
    ...
    print(f"  Raw V3 Embedding: Composite = {raw_stability['composite_stability']:.4f} "
    ...
    print(f"  PCA (2D):         Composite = {pca_stability['composite_stability']:.4f} "
    ...
    print(f"  UMAP (2D):        Composite = {umap_stability['composite_stability']:.4f} "
    ...
    print(f"  t-SNE (2D):       Composite = {tsne_stability['composite_stability']:.4f} "
    ...
    resp_score = raw_stability['composite_stability'] - umap_stability['composite_stability']
    ...
    if raw_stability['composite_stability'] >= 0.85 and umap_stability['composite_stability'] < 0.85:
        instability_source = "UMAP"
        root_cause = "UMAP_NONLINEAR_PROJECTION_SENSITIVITY"
    elif raw_stability['composite_stability'] < 0.85 and umap_stability['composite_stability'] < 0.85:
    ...
    print(f"Diagnosed Instability Source: {instability_source} (Root Cause = {root_cause})")
    ...
    if all_passed:
        ...
        print("PHASE 4.8D FAILED: One or more intrinsic stability criteria were not satisfied.")
        print("Failed tests:")
        if not passed_test1:
        ...
        if not passed_test3:
            print("  - Test 3 (True resampling non-degenerate CIs fell below thresholds)")
        print("=" * 80)
        return 1
