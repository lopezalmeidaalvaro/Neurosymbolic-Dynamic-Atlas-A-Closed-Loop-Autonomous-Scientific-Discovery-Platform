...
def estimate_lyapunov(signal, dt=0.01, emb_dim=3, delay=10):
    ...
    states = np.array(
        [signal[i : i + emb_dim * delay : delay] for i in range(num_vectors)]
    )
...
def simulate_lorenz_custom(R, length=25000, seed=42):
    ...
    sol = solve_ivp(
        lambda t, y: lorenz_custom_rhs(t, y, R),
        t_span,
        state0,
        t_eval=t_eval,
        method="RK45",
...
def simulate_rossler_custom(c, length=25000, seed=42):
    ...
    state0 = [1.0 + np.random.normal(0, 0.1), 1.0, 1.0]
    sol = solve_ivp(
        lambda t, y: rossler_custom_rhs(t, y, c),
        t_span,
        state0,
        t_eval=t_eval,
        method="RK45",
...
def simulate_duffing_custom(f, length=25000, seed=42):
    ...
    sol = solve_ivp(
        lambda t, y: duffing_custom_rhs(t, y, f),
        t_span,
        state0,
        t_eval=t_eval,
        method="RK45",
...
def main():
    ...
    print(
        "No system modifications. No parameter adjustment. Raw mathematical limits only."
    )
    ...
    for sys_name in ["lorenz", "rossler", "henon", "logistic"]:
        ...
        for seed in SEEDS:
            ...
            pr_embs = extract_v3_embeddings(
                pr, dt, window_size=1000, stride=500
            ).tolist()

            # 3) ARMA Mimic (using AR(20))
            ar = generate_arma_mimic(chaos, seed, p=20)
            ar_embs = extract_v3_embeddings(
                ar, dt, window_size=1000, stride=500
            ).tolist()
    ...
    for t_name, regimes in transitions.items():
        ...
        for seed in SEEDS:
            # Generate signals
            lorenz_sig, lorenz_dt = simulate_lorenz_custom(
                R=28.0, length=length, seed=seed
            )
            lorenz_sig = (lorenz_sig - np.mean(lorenz_sig)) / np.std(lorenz_sig)

            rossler_sig, rossler_dt = simulate_rossler_custom(
                c=5.7, length=length, seed=seed
            )
            ...
            distances = []
            features_variance = (
                np.var(embs, axis=0).tolist() if len(embs) > 0 else [0.0] * 8
            )
        ...
        test3_results[t_name] = {
            ...
            "embedding_change": np.mean(
                [t["embedding_change"] for t in t_list], axis=0
            ).tolist(),
    ...
    for alpha in np.linspace(0.0, 0.9, 10):
        ...
        perf_curve.append(
            {"alpha": float(alpha), "roc_auc": auc_eval, "pr_auc": pr_auc_eval}
        )
    ...
    test4_results = {
        ...
        "f1_score": f1,
        "ece": ece,
        "calibration_curve": {"confidences": bin_confs, "accuracies": bin_accs},
        "performance_vs_difficulty": perf_curve,
        "breakpoint_alpha": breakpoint_alpha,
    }

    print(
        f"Test 4 Complete. ROC-AUC: {roc_auc:.4f}, ECE: {ece:.4f}, Breakpoint alpha: {breakpoint_alpha}"
    )
    ...
    adversaries_groups = {
        ...
        "nonstationary_transition": np.array(
            extract_v3_embeddings(
                simulate_lorenz_custom(R=28.0, seed=42)[0][:12500], 0.01
            )
        ),
    }
    ...
    for adv_name, adv_embs in adversaries_groups.items():
        ...
        dist_p = np.linalg.norm(
            P_scaled[:, None, :] - P_scaled[None, :, :], axis=-1
        ).flatten()
        dist_a = np.linalg.norm(
            A_scaled[:, None, :] - A_scaled[None, :, :], axis=-1
        ).flatten()
    ...
    print(
        f"BREAKPOINT_THRESHOLD   = {test4_results['breakpoint_alpha']:.2f} (alpha boundary)"
    )
    ...
    for name, res5 in test5_results.items():
        print(
            f"  - {name:<30} : {res5['failure_type']} (CKA={res5['cka']:.4f}, Overlap={res5['neighbor_overlap']:.4f})"
        )

    print("\nROBUSTNESS_CURVE (Performance-vs-Difficulty):")
    for pt in test4_results["performance_vs_difficulty"]:
        print(
            f"  - Alpha = {pt['alpha']:.1f} | ROC-AUC = {pt['roc_auc']:.4f} | PR-AUC = {pt['pr_auc']:.4f}"
        )
