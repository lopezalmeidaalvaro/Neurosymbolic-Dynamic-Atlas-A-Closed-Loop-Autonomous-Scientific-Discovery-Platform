...
def main():
    ...
    print(
        f"¿El sistema de estados es completamente observable?: {is_fully_observable}\n"
    )
    ...
    nominal_sim = net.simulate(
        duration=duration, dt=dt, Q_solar_func=std_solar_flux, method="LSODA"
    )
    ...
    for p_name, p_info in parameters.items():
        ...
        for direction in [+0.10, -0.10]:
            ...
            res_perturbed = net_perturbed.simulate(
                duration=duration, dt=dt, Q_solar_func=std_solar_flux, method="LSODA"
            )
        ...
        print(
            f"    -> Parámetro: {p_name:12s} | Sensibilidad normalizada: {overall_sens:.4f}%"
        )
    ...
    bar_colors = [
        (
            "#ff2a5f"
            if observability_classes[name] == "Practically Observable (Alta/Media)"
            else "#64748b"
        )
        for name in p_names
    ]
    ...
    # Styling
    ax.set_title(
        "Análisis de Sensibilidad Normalizada de Sensores por Parámetro",
        color="white",
        fontsize=14,
        pad=15,
    )
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"- **Sensores disponibles:** 3 ($T_\\text{{CPU}}$, $T_\\text{{bat}}$, $T_\\text{{rad}}$)\n"
        )
        f.write(
            f"- **Rango de la Matriz de Observabilidad ($\\mathcal{{O}}$):** **{rank_O}**\n\n"
        )
        ...
        if is_fully_observable:
            ...
            f.write(
                "> El rango de $\\mathcal{O}$ es menor que $6$. El sistema de estados no es completamente observable. Algunas combinaciones de temperaturas no pueden ser deducidas con los sensores actuales.\n\n"
            )
        ...
        for p_name, p_info in parameters.items():
            ...
            f.write(
                f"| `{p_name}` | {p_info['field']} de nodo(s) {p_info['index']} | {sens:.4f}% | **{classification}** |\n"
            )
