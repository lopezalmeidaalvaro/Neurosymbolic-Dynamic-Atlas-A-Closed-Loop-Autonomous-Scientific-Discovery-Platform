    ...
    def __init__(self, port=None, baudrate=115200):
        ...
        if port is not None:
            try:
                ...
                print(
                    "[*] HIL: Conmutando a 'MODO HARDWARE EMULADO' de alta fidelidad."
                )
        else:
            print(
                "[*] HIL: Sin puerto serial especificado. Iniciando en 'MODO HARDWARE EMULADO' (TVAC Emulated)."
            )
...
def run_hil_tvac_calibration():
    ...
    print(
        "[*] Iniciando ciclo continuo de calibración en tiempo real (1 hora emulada)..."
    )
    ...
    for step in range(n_steps + 1):
        ...
        if sensor_readings is not None:
            ...
            T_cpu_measured, T_rad_measured, T_bat_measured = (
                interface.get_emulated_measurement(plant_state, power, dt=interval)
            )
        ...
        dt_C_cpu = max(
            ...
            min(
                500.0,
                dt_C_cpu - 50.0 * residual * dT_dt * (T_est_k / max(10.0, dt_C_cpu)),
            ),
        )
        dt_eps_rad = max(
            0.1, min(0.98, dt_eps_rad - 0.002 * residual * (T_est_k * 0.1))
        )
        ...
        if step % 120 == 0:
            print(
                f"  [Tiempo {t_curr:4d}s]: Medida={T_cpu_measured:5.2f}°C, Predicha={T_cpu_estimated:5.2f}°C, Cal_C={dt_C_cpu:6.2f}, Cal_eps={dt_eps_rad:5.3f}"
            )
    ...
    rmse_ss = np.sqrt(
        np.mean((ss_df["T_cpu_measured"] - ss_df["T_cpu_estimated"]) ** 2)
    )

    # Transient state: first 45 minutes (t < 2700s)
    tr_df = df[df["time"] < 2700.0]
    rmse_tr = np.sqrt(
        np.mean((tr_df["T_cpu_measured"] - tr_df["T_cpu_estimated"]) ** 2)
    )
    ...
    print(
        f"  - Error RMSE Transitorio: {rmse_tr:.4f}°C (Objetivo < 5°C) -> ¿Pasado?: {rmse_tr < 5.0}"
    )
    print(
        f"  - Error RMSE Estacionario: {rmse_ss:.4f}°C (Objetivo < 3°C) -> ¿Pasado?: {rmse_ss < 3.0}"
    )
    ...
    plt.plot(
        df["time"] / 60.0,
        df["T_cpu_measured"],
        label="Sensor ds18B20 Medido (TVAC)",
        color="#ff2a5f",
        alpha=0.8,
        linewidth=2.0,
    )
    plt.plot(
        df["time"] / 60.0,
        df["T_cpu_estimated"],
        label="Predicción Gemelo Digital (Calibrado)",
        color="#00f0ff",
        linestyle="--",
        linewidth=2.0,
    )
    plt.plot(
        df["time"] / 60.0,
        df["T_cpu_real"],
        label="Temperatura Real de Planta",
        color="#26ffad",
        alpha=0.4,
        linewidth=1.5,
    )
    ...
    ax.set_title(
        "Validación HIL Real-Time en Cámara de Vacío Térmico (TVAC)",
        color="white",
        fontsize=14,
        pad=15,
    )
    ...
    plt.savefig(
        chart_path, facecolor=plt.gcf().get_facecolor(), edgecolor="none", dpi=150
    )
    plt.close()
