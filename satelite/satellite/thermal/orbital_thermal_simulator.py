...
def run_simulation(power, area, absorptivity, emissivity, num_orbits=3):
    ...
    """
    print(
        f"[*] Starting thermal simulation: P={power}W, A={area}m2, alpha={absorptivity}, eps={emissivity}"
    )
    ...
    for step in range(total_steps):
        ...
        if step >= (total_steps - steps_per_orbit):
            orbit_time_min = (
                (step - (total_steps - steps_per_orbit)) * STEP_SIZE
            ) / 60.0
            telemetry.append(
                {
...
def save_csv(telemetry, file_path):
    ...
    with open(file_path, "w", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["Time_Min", "Temp_C", "Q_Solar_W", "Q_Rad_Out_W", "Is_Eclipse"],
        )
        writer.writeheader()
...
def plot_telemetry(telemetry, plot_path, p, a, alpha, eps):
    try:
        ...
        # Reference limits
        ax1.axhline(
            85.0, color="red", linestyle="--", alpha=0.6, label="Burnout Limit (85°C)"
        )
        ax1.axhline(
            -40.0, color="blue", linestyle="--", alpha=0.6, label="Freeze Limit (-40°C)"
        )
        ...
        plt.title(
            f"LEO Spacecraft Orbit Thermal Cycle\n(P={p}W, A={a}m2, alpha={alpha}, eps={eps})",
            color="white",
            pad=15,
        )
        ...
        print(
            "[!] matplotlib not installed. Skipping PNG plot generation (CSV generated successfully)."
        )


def main():
    parser = argparse.ArgumentParser(
        description="Orbital Thermal Simulator for LEO Satellites"
    )
    parser.add_argument(
        "--power", type=float, default=220.0, help="Internal Electrical Power (W)"
    )
    parser.add_argument(
        "--area", type=float, default=2.2, help="Radiator Surface Area (m2)"
    )
    parser.add_argument(
        "--absorptivity", type=float, default=0.28, help="Solar Absorptivity (alpha)"
    )
    parser.add_argument(
        "--emissivity", type=float, default=0.78, help="Infrared Emissivity (epsilon)"
    )
    parser.add_argument(
        "--orbits", type=int, default=3, help="Number of simulated orbits"
    )
    ...
    if not (0.01 <= args.area <= 100.0):
        parser.error(
            "Physical bound exceeded: Radiator area must be within [0.01, 100.0] m2."
        )
    if not (0.01 <= args.absorptivity <= 1.0):
        parser.error(
            "Physical bound exceeded: Solar absorptivity must be within [0.01, 1.0]."
        )
    if not (0.01 <= args.emissivity <= 1.0):
        parser.error(
            "Physical bound exceeded: Infrared emissivity must be within [0.01, 1.0]."
        )
    ...
    output_csv = args.output or os.path.join(
        script_dir, "..", "models", "telemetry.csv"
    )
    output_plot = args.plot or os.path.join(
        script_dir, "..", "models", "thermal_simulation.png"
    )

    # Run simulation
    telemetry = run_simulation(
        args.power, args.area, args.absorptivity, args.emissivity, args.orbits
    )
    ...
    if max_t > 85.0 or min_t < -40.0:
        print(
            " [CRITICAL ALERT] Spacecraft exceeds safety limits! Check radiator configurations."
        )
