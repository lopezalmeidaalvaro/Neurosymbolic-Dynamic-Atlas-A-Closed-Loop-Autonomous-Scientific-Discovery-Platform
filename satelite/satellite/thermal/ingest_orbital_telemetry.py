...
class TLEParser:
    ...
    def __init__(self, tle_lines: List[str]):
        ...
        self.semi_major_axis = (
            398600.4418 / ((self.mean_motion * 2.0 * math.pi / 86400.0) ** 2)
        ) ** (
            1.0 / 3.0
        )  # km
...
def load_flight_telemetry(satellite: str, hours: int = 50) -> pd.DataFrame:
    ...
    for t in timestamps:
        ...
        t_structure += (
            (
                flux * 0.1
                + 1.8 * (t_cpu - t_structure)
                + 0.5 * (t_battery - t_structure)
                - q_rad
            )
            / 1000.0
            * 60.0
        ...
        rows.append(
            {
                ...
                "sensor_temp_structure": round(
                    t_structure + np.random.normal(0.0, 0.10), 2
                ),
            }
        )
...
def validate_digital_twin():
    ...
    print(
        " -> Solving 50-hour transient LEO orbit integrations (this may take a few seconds)..."
    )
    ...
    if os.path.exists(rf_model_path):
        ...
        X_flight = np.array([[18.0, 0.12, 0.85], [32.0, 0.18, 0.85]])
        y_flight = np.array(
            [
        ...
        if os.path.exists(dataset_path):
            ...
            print(
                f"[+] Calibrated surrogate model fine-tuned and saved to: {calib_path}"
            )
...
"""
    ...
    report_path = os.path.join(
        PARENT_DIR, "thermal", "operational_validation_report.md"
    )
