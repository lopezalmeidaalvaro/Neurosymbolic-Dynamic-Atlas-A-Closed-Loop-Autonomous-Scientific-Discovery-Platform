...
def train_neural_ode():
    ...
    print(
        f"Neural ODE Evaluation: RMSE = {rmse:.4f}°C/K, Inference latency for batch = {t_int:.2f} ms"
    )
    ...
    print(
        f"Neural ODE model successfully versioned & saved to {os.path.join(models_dir, versioned_filename)}"
    )
    print(
        f"Neural ODE model successfully saved to {os.path.join(models_dir, 'neural_ode_thermal.pth')}"
    )
