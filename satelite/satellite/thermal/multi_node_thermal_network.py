...
class ThermalNetwork:
    ...
    def __init__(self, config_dict=None):
        ...
        self.C = np.array([250.0, 800.0, 300.0, 1500.0, 200.0, 300.0])  # J/K
        self.Q = np.array([15.0, 3.0, 5.0, 0.0, 0.0, 0.0])  # W (Internal generation)
        ...
        self.k[1, 3] = self.k[3, 1] = 0.4  # Battery-Structure
        self.k[2, 3] = self.k[3, 2] = 1.5  # Payload-Structure
        self.k[4, 3] = self.k[3, 4] = 6.0  # Radiator-Structure
        self.k[5, 3] = self.k[3, 5] = 0.15  # Panels-Structure

        # Override with custom config if provided
        if config_dict:
            if "C" in config_dict:
                self.C = np.array(config_dict["C"])
            if "Q" in config_dict:
                self.Q = np.array(config_dict["Q"])
            if "eps" in config_dict:
                self.eps = np.array(config_dict["eps"])
            if "A" in config_dict:
                self.A = np.array(config_dict["A"])
            if "k" in config_dict:
                self.k = np.array(config_dict["k"])
    ...
    def simulate(
        ...
        def ode_system(t, y):
            ...
            return self.dTdt(
                y, t, q_solar_val, use_cavity_radiation=use_cavity_radiation
            )
    ...
    def plot_network(self, result, output_path=None):
        ...
        for i in range(6):
            ...
            ax.plot(
                times,
                temps[i],
                label=f"{name} (Max: {result['max_temps'][name]:.1f}°C)",
                color=colors[i],
                linestyle=styles[i],
                linewidth=2.0,
            )
        ...
        ax.set_title(
            "LEO Cubesat Multi-Node Thermal Network Simulation",
            color="white",
            fontsize=14,
            pad=15,
        )
...
def run_baselines():
    ...
    config_high = {"Q": [30.0, 1.0, 10.0, 0.0, 0.0, 0.0]}
    ...
    for name, res in [
        ...
        for node in ["CPU", "Battery", "Structure"]:
            print(
                f" -> {node} Max Temp: {res['max_temps'][node]:.2f}°C, Time to Critical: {res['time_to_critical'][node]}"
            )
