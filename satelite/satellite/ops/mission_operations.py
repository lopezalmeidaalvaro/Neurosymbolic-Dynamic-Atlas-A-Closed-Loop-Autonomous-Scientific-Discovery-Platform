...
class MissionOperationsSimulator:
    ...
    def execute_commands(self, t, station, T_tx_c):
        ...
        if rand < 0.15:
            ...
            cmd_desc = (
                "Modo Seguro (SAFE_MODE) comandado desde tierra por emergencia térmica"
            )
        else:
            cmd_type = "NO_CMD"
    ...
    def run_simulation(self):
        ...
        print(
            f"[+] Simulación completada con éxito. Resultados guardados en: {csv_path}"
        )
        ...
        num_obs_exec = len(
            [o for o in self.observation_logs if o["Status"] == "EXECUTED"]
        )
        num_obs_post = len(
            [o for o in self.observation_logs if o["Status"] == "POSTPONED"]
        )
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"**Generado:** {time.strftime('%Y-%m-%d %H:%M:%S')} | **Duración de Simulación:** 7 días (Vuelo real)\n\n"
            )
            f.write(
            ...
            f.write(
                f"| **Contactos de Estación (AOS)** | {num_aos} | Pases de telemetría exitosos |\n"
            )
            f.write(
                f"| **Telecomandos Ejecutados** | {num_cmds} | Comandos procesados (2-5s latencia) |\n"
            )
            f.write(
                f"| **Observaciones Ejecutadas** | {num_obs_exec} | Imágenes científicas almacenadas |\n"
            )
            f.write(
                f"| **Observaciones Pospuestas** | {num_obs_post} | Postergaciones por restricción térmica / eclipse |\n"
            )
            f.write(
                f"| **Alertas de Sobrecalentamiento** | {num_anomalies} | Throttling de transmisión activado |\n\n"
            )
            ...
            for c in self.command_logs[:15]:
                f.write(
                    f"| {c['Timestamp_s']:.0f} | {c['Station']} | `{c['Command']}` | {c['Latency_s']:.2f} s | {c['Description']} | {c['T_tx_at_execution_C']:.2f}°C |\n"
                )
            ...
            for o in self.observation_logs[:15]:
                f.write(
                    f"| {o['Timestamp_s']:.0f} | **{o['Status']}** | {o['T_payload_C']:.2f}°C | {o['Description']} |\n"
                )
