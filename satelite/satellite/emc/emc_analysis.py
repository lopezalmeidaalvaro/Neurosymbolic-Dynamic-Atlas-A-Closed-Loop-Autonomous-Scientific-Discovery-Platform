    ...
    def calculate_distances(self):
        ...
        for i in range(self.n_nodes):
            for j in range(len(self.sources)):
                dist = np.linalg.norm(
                    self.sensor_positions[i] - self.source_positions[j]
                )
    ...
    def compute_noise_waveform(self, t, active_sources=None):
        ...
        for i in range(self.n_nodes):
            ...
            if active_sources.get("DC_DC_Converter", False):
                dist = distances[i, 0]
                coupled_amp = (
                    self.source_amplitudes["DC_DC_Converter"]
                    * self.coupling_factors["DC_DC_Converter"]
                    / dist
                )
            ...
            if active_sources.get("PWM_Heater", False):
                dist = distances[i, 1]
                coupled_amp = (
                    self.source_amplitudes["PWM_Heater"]
                    * self.coupling_factors["PWM_Heater"]
                    / dist
                )
            ...
            if active_sources.get("Reaction_Wheels", False):
                dist = distances[i, 2]
                coupled_amp = (
                    self.source_amplitudes["Reaction_Wheels"]
                    * self.coupling_factors["Reaction_Wheels"]
                    / dist
                )
                noise_v += coupled_amp * (
                    np.sin(2.0 * np.pi * f_wheels * t)
                    + 0.5 * np.sin(2.0 * np.pi * 2 * f_wheels * t)
                )
            ...
            if active_sources.get("RF_Transmitter", False):
                dist = distances[i, 3]
                coupled_amp = (
                    self.source_amplitudes["RF_Transmitter"]
                    * self.coupling_factors["RF_Transmitter"]
                    / dist
                )
...
def run_emc_study():
    ...
    noise_nominal = model.compute_noise_waveform(
        t, active_sources={s: True for s in model.sources}
    )
    ...
    noise_silent = model.compute_noise_waveform(
        t, active_sources={"DC_DC_Converter": True}
    )
    ...
    for i in range(model.n_nodes):
        ...
        shielding_needed = (
            "Sí (Sesgo crítico RF/PWM)" if temp_err_nom > 1.5 else "No (Seguro)"
        )

        print(
            f"Sensor: {name:10s} | Nom Error: {temp_err_nom:5.2f}°C (SNR: {snr_nom:4.1f}dB) | Sil Error: {temp_err_sil:5.2f}°C (SNR: {snr_sil:4.1f}dB) | Blindaje: {shielding_needed}"
        )
    ...
    ax1.plot(
        t_ms,
        noise_nominal[0] * 1000.0,
        label="Nominal (Todos los subsistemas)",
        color="#ff2a5f",
        linewidth=1.5,
    )
    ax1.plot(
        t_ms,
        noise_silent[0] * 1000.0,
        label="Silencioso (Solo DC-DC)",
        color="#00f0ff",
        linewidth=1.5,
        alpha=0.7,
    )
    ax1.set_title(
        "Forma de Onda de Ruido Acoplado en el Sensor de CPU (Top)",
        color="white",
        fontsize=12,
        pad=10,
    )
    ...
    ax2.set_title(
        "Ruido Acoplado en Batería (Cercanía a Calefactor PWM e Involucración Motor ADCS)",
        color="white",
        fontsize=12,
        pad=10,
    )
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        for _, r in df_emc.iterrows():
            f.write(
                f"| **{r['Sensor']}** | {r['Nominal_Temp_Error_C']:.2f}°C | {r['Nominal_SNR_dB']:.1f} dB | {r['Silent_Temp_Error_C']:.2f}°C | {r['Silent_SNR_dB']:.1f} dB | **{r['Shielding_Required']}** |\n"
            )
        ...
        f.write(
            "> 1. **Sensor de CPU (Top)**: Es el nodo más expuesto a la interferencia por acoplamiento de RF debido a la cercanía con el Transmisor de Banda S/X (2.2 GHz, 2W). La rectificación de RF en el amplificador operacional de instrumentación induce una tensión continua parásita equivalente a un sesgo térmico de **"
            + f"{df_emc.loc[df_emc['Sensor'] == 'CPU', 'Nominal_Temp_Error_C'].values[0]:.2f}°C**. Requiere blindaje Faraday con pintura conductiva de níquel en la tapa superior.\n"
        )
        f.write(
            "> 2. **Sensor de Batería (Middle)**: Sufre picos capacitivos rápidos de **"
            + f"{df_emc.loc[df_emc['Sensor'] == 'Battery', 'Nominal_Temp_Error_C'].values[0]:.2f}°C** generados por los flancos de subida del Calefactor PWM a 1 kHz. Se recomienda rutear la señal del termistor en par trenzado apantallado (STP).\n"
        )
        f.write(
