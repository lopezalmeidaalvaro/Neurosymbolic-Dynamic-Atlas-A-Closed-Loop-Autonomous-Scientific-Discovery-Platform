...
class SoftwareAssuranceAnalyzer:
    ...
    def analyze(self):
        ...
        for fun_name, start_pos in functions:
            ...
            req_mapped = self.ecss_reqs.get(
                fun_name, "ECSS-E-ST-40C-REQ-TBD (General FSW Module)"
            )
        ...
        with open(violations_path, "w", encoding="utf-8") as f:
            f.write(
                "=== REGISTRO DE INFRACCIONES DE SOFTWARE DE VUELO (ECSS & MISRA C) ===\n\n"
            )
            if len(self.violations) == 0:
                f.write(
                    "[SAFE] Cero infracciones encontradas. El código de vuelo es 100% conforme.\n"
                )
            else:
        ...
        with open(report_path, "w", encoding="utf-8") as f:
            ...
            f.write(
                f"**Generado:** {time.strftime('%Y-%m-%d %H:%M:%S')} | **Archivo Evaluado:** `surrogate_mlp_inference.c`\n\n"
            )
            f.write(
            ...
            for _, r in df_trace.iterrows():
                doc_status = (
                    "Sí"
                    if r["Precondition_Documented"] == "YES"
                    and r["Postcondition_Documented"] == "YES"
                    else "No"
                )
                state_str = "CONFORME" if r["Status"] == "COMPLIANT" else "NO_CONFORME"
                f.write(
                    f"| **{r['Function_Name']}** | {r['Lines_Of_Code']} | {r['Cyclomatic_Complexity']} | {r['ECSS_Requirement']} | {doc_status} | **{state_str}** |\n"
                )
            ...
            if len(self.violations) == 0:
                ...
                f.write(
                    "> - Cero infracciones críticas encontradas. El software de vuelo cumple plenamente con las restricciones de **Zero dynamic memory allocation** post-boot y determinismo de bucles.\n\n"
                )
            else:
                f.write("> [!WARNING]\n")
...
if __name__ == "__main__":
    # Path to C file
    c_file = (
        Path(__file__).resolve().parents[1] / "flight" / "surrogate_mlp_inference.c"
    )

    # Check if the C file exists. If not, generate a mock or run the ONNX exporter to create it
    if not c_file.exists():
        print(
            "[CRITICAL] surrogate_mlp_inference.c not found. Generating C code via exporter..."
        )
        import subprocess

        subprocess.run(
            ["python", "satellite/flight/export_to_onnx.py"],
