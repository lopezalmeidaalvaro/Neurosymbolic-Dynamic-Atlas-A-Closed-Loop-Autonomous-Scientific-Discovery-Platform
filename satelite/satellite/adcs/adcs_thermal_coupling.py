...
def run_adcs_study():
    ...
    for idx, mode in enumerate(modes):
        ...
        for k in range(num_steps):
            ...
            # Compute fluxes
            Q_sol_faces, Q_IR_faces = coupling.compute_heat_fluxes(
                t_curr, q, is_eclipse
            )
        ...
        plt.plot(
            times / 3600.0,
            temp_history_cpu,
            label=f"CPU ({mode} | Max: {max_cpu:.1f}°C)",
            color=colors[idx],
            linewidth=1.8,
        )

    ax.set_title(
        "Comparación de Temperaturas de CPU según Orientación ADCS (10 Órbitas)",
        color="white",
        fontsize=13,
        pad=15,
    )
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        for _, r in df_adcs.iterrows():
            eval_str = (
                "Óptimo y frío (Alta disipación)"
                if r["Mode"] == "Nadir-pointing"
                else (
                    "Caliente por solar tracker"
                    if r["Mode"] == "Sun-pointing"
                    else "Térmicamente equilibrado (Spinning)"
                )
            )
            f.write(
                f"| **{r['Mode']}** | {r['T_max_CPU_C']:.2f}°C | {r['T_avg_CPU_C']:.2f}°C | {r['T_max_Battery_C']:.2f}°C | {r['T_avg_Battery_C']:.2f}°C | {r['Average_Solar_Flux_W']:.2f} W | {eval_str} |\n"
            )
        ...
        f.write(
            "> 1. **Modo Nadir-pointing**: Al mantener el radiador ($+Z$, cara 4) orientado permanentemente al espacio profundo (espacio frío), se maximiza el coeficiente de radiación externa de calor. Esto resulta en las temperaturas de CPU más bajas y estables (**"
            + f"{df_adcs.loc[df_adcs['Mode'] == 'Nadir-pointing', 'T_max_CPU_C'].values[0]:.2f}°C**).\n"
        )
        f.write(
            "> 2. **Modo Sun-pointing**: Al girar continuamente el chasis para apuntar la cara frontal hacia el Sol, se capta la máxima irradiancia directa de $1361\\text{ W/m}^2$. Esto eleva significativamente las temperaturas globales, exigiendo una disipación robusta en la CPU (**"
            + f"{df_adcs.loc[df_adcs['Mode'] == 'Sun-pointing', 'T_max_CPU_C'].values[0]:.2f}°C**).\n"
        )
        f.write(
