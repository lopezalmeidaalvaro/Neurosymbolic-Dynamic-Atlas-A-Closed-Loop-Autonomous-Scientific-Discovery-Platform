...
def main():
    parser = argparse.ArgumentParser(
        description="Satellite Thermal Digital Twin Pipeline Orchestrator"
    )
    parser.add_argument(
        "--from-stage",
        type=str,
        ...
        help="Stage to start execution from",
    )
    parser.add_argument(
        "--to-stage",
        type=str,
        default="T28",
        choices=list(STAGES.keys()),
        help="Stage to end execution at",
    )
    ...
    for stage in active_stages:
        ...
        try:
            ...
            result = subprocess.run(
                [sys.executable, str(script_absolute_path)], cwd=cwd, check=True
            )
            elapsed = time.time() - t_start
            times[stage] = {"status": "SUCCESS", "time_sec": elapsed}
            print(
                f"\n[+] STAGE {stage} COMPLETED SUCCESSFULLY IN {elapsed:.2f} SECONDS."
            )
    ...
    for stage, data in times.items():
        print(
            f" -> Stage {stage:4s}: {data['status']:8s} | Elapsed: {data['time_sec']:6.2f}s"
        )
    print("=" * 70 + "\n")
