...
def main():
    ...
    try:
        ...
        print(
            df_results[
                ["Case_ID", "Case_Name", "RMSE_C", "R2_Score", "Speedup"]
        ...
        print(
            f"[+] Summary Mean RMSE: {mean_rmse:.3f}°C (Expected: 0.374°C) | Tag: Derived from T18 validation"
        )
        print(
            f"[+] Summary Mean Speedup: {mean_speedup:.0f}x (Expected: 3600x) | Tag: Numerical simulation (transient FEM)"
        )
        ...
        if os.path.exists(scatter_plot):
            ...
            print(
                f"  - {scatter_plot}: {plot_hash} | Labeled: Derived from T18 validation"
            )
