...
try:
    ...
    print("  [GEOM WARNING] 'GraphRicciCurvature' not found. Using resilient pure-Python LP solver fallback.")
...
def build_neighborhood_graph(point_cloud, k=10, method='knn'):
    ...
    if method == 'knn':
        ...
        A = kneighbors_graph(point_cloud, n_neighbors=k, mode='distance', include_self=False)
...
def compute_ollivier_ricci_curvature(G):
    ...
    if RICCI_AVAILABLE:
        try:
            ...
            print(f"  [GEOM ERROR] GraphRicciCurvature failed ({e}). Bypassing to fallback...")
...
def compute_node_curvature(G_with_curvature):
    ...
    for node in G_with_curvature.nodes():
        ...
        if incident_edges:
            curvs = [G_with_curvature[u][v].get('ricciCurvature', 0.0) for u, v in incident_edges]
...
def compute_curvature_statistics(node_curvatures):
    ...
    return {
        ...
        "kurtosis": float(kurtosis(node_curvatures)) if len(node_curvatures) > 2 else 0.0,
...
def compute_spectral_statistics(eigenvalues):
    ...
    if len(eigenvalues) < 2:
        return {
            "mean": 0.0, "variance": 0.0, "gap_ratio": 0.0, "spectral_dimension": 0.0
        }
...
def extract_geometric_features(signal, emb_dim=3, lag=1, k=10):
    ...
    if len(point_cloud) < 10:
        print("  [GEOM WARNING] Signal too short to extract geometrical features. Returning NaNs.")
        return feat_vector
    ...
    try:
        ...
        # 5. Diffusion Maps
        diff_coords, diff_lambdas = compute_diffusion_map(point_cloud, n_components=5, t=1, k=k)
        ...
        feat_vector[17] = float(np.sum(diff_lambdas ** 2)) if len(diff_lambdas) > 0 else 0.0
