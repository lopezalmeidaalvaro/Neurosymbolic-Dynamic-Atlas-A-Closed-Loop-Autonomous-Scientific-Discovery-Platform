...
def run_tests():
    ...
    try:
        lorenz_data = generate_lorenz(n_timesteps=1000, dt=0.01, initial_state=[10.0, 10.0, 20.0])
    ...
    try:
        ...
        dgms = res["dgms"]
        print(f"  Computed persistence diagrams. H0 shape: {dgms[0].shape} | H1 shape: {dgms[1].shape}")
    ...
    try:
        ...
        assert abs(lb_eigs[0]) < 0.02, f"First LB eigenvalue should be near 0, got {lb_eigs[0]}"
    ...
    try:
        eigs, modes, freqs, growths = koop.compute_koopman_modes(signal, emb_dim=3, lag=1, n_modes=5)
        mags = np.abs(eigs)
        ...
        print(f"  Count of conservative modes (|lambda| approx 1): {conservative_count}")
        
        assert conservative_count >= 1, "Expected at least one conservative mode near 1.0."
    ...
    try:
        ...
        assert nan_fraction < 0.30, f"Expected NaN fraction < 30%, got {nan_fraction * 100:.2f}%"
    ...
    try:
        ...
        assert w_h1_0 > w_h1_30, f"Expected Wasserstein distance at 0dB ({w_h1_0:.4f}) to be greater than at 30dB ({w_h1_30:.4f})."
