...
DAMAGE_PER_CYCLE = 0.005 # Each cycle with delta_T > 50°C adds 0.5% structural fatigue damage

class SatelliteNode:
    """
    Instance representing a single satellite in the constellation.
    """
    def __init__(self, sat_id: int, plane: int, phase: float, altitude_km: float = 400.0):
        self.sat_id = sat_id
        self.plane = plane
        self.phase = phase
        self.altitude = altitude_km
        self.period = 86400.0 / ( (398600.4418 / ( (altitude_km + 6378.137) ** 3 )) ** 0.5 / (2.0 * math.pi) )
...
def run_satellite_transient_job(args) -> Tuple[int, List[float], float, float]:
    ...
    for step_idx in range(steps):
        ...
        T = sat.network.simulate(duration=3600, dt=120, Q_solar_func=lambda t: q_solar)["temperatures"]
        ...
        if cpu_t > max_t: max_t = cpu_t
        if cpu_t < min_t: min_t = cpu_t
...
def main():
    ...
    print(f"[+] Concurrent simulation completed in {time.time() - start_sim:.2f} seconds.")
    ...
    for i in range(n_satellites):
        ...
        print(f"Spacecraft Sat-{sat_id}: Peak CPU: {max_t:.2f}°C, Remaining Life: {rul:.2%}, Status: {status}")
        rows_report.append({
            "sat_id": sat_id,
            "max_t": max_t,
            "rul": rul,
    ...
    if anomalous_sats:
        ...
        for sat, dev, cause in anomalous_sats:
            print(f" -> Sat-{sat} exhibited >{dev:.1f}σ deviation from constellation mean! Cause: {cause}")
    ...
    for i in range(n_satellites):
        ...
        if i == 7:
            ...
            color = '#ef4444'
            alpha = 1.0
        ax.plot(hours_arr, all_temps[i], label=f"Sat-{i} (RUL: {rows_report[i]['rul']:.0%})" if i in [0, 7] else "", 
                color=color, linestyle=style, alpha=alpha, linewidth=1.5)
        
    ax.plot(hours_arr, mean_temps, label="Constellation Average", color='#eab308', linewidth=2.0)
    ax.set_title("7-Day Spacecraft Constellation Coupled Thermal Trajectories", color='white', fontsize=12, pad=15)
...
"""
    ...
    additional_section = additional_section.replace("{SAT7_TEMP}", f"{rows_report[7]['max_t']}")
    additional_section = additional_section.replace("{CSV_PATH}", csv_path.replace("\\", "/"))
    additional_section = additional_section.replace("{PLOT_PATH}", plot_path.replace("\\", "/"))
