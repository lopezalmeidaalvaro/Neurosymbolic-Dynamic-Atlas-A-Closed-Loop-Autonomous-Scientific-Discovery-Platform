...
def generate_dataset(n_configs=500, output_path="satellite/thermal/thermal_dataset.csv"):
    """
    ...
    for idx in range(n_configs):
        ...
        row = {
            ...
            "time_to_critical": sim_res["time_to_critical"] if sim_res["time_to_critical"] is not None else -1.0,
...
if __name__ == "__main__":
    ...
    print(f"Power: {first_row['power']:.2f} W, Area: {first_row['area']:.4f} m^2, Emissivity: {first_row['emissivity']:.2f}")
    print(f"Max Temp: {first_row['max_temp']:.2f}°C, Time to Critical: {first_row['time_to_critical']}")
