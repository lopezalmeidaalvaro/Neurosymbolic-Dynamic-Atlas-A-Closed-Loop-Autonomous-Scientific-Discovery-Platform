...
def fit_analytical_steady_state(X, y):
    ...
    return popt, f"((power / (emissivity * {SIGMA:.3e} * area * {popt[0]:.4f})) + {T_AMB**4:.1f})**0.25 - 273.15 + {popt[1]:.4f}"
...
def fit_max_temp(X, y):
    ...
    return popt, f"{popt[0]:.4f} * ((power / (emissivity * {SIGMA:.3e} * area))**0.25 - 273.15) + {popt[1]:.4f} * power + {popt[2]:.4f}"
...
def fit_time_to_critical(X, y):
    ...
    return popt, f"{popt[0]:.4f} * {C_THERMAL * (358.15 - 293.15):.1f} / (power - {popt[1] * SIGMA:.3e} * emissivity * area * 1.116e10)"
...
def fit_cooling_rate(X, y):
    ...
    popt, _ = curve_fit(func, (A, e, P), y, p0=[1.0, 0.0], maxfev=10000)
    return popt, f"{popt[0]:.4f} * (emissivity * {SIGMA:.3e} * area * 8.1e9) / {C_THERMAL} + {popt[1]:.4f}"
...
def discover_equations():
    ...
    cooling_rate = (df['emissivity'] * SIGMA * df['area'] * (300.0**4 - T_AMB**4)) / C_THERMAL
