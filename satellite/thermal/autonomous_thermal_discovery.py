...
class AutonomousThermalScientist:
    ...
    def fit_symbolic_equation(self, X, Y):
        ...
        try:
            popt, _ = curve_fit(model_func, (areas, densities, heights, paths), Y, p0=[50.0, 1.0, 50.0, 1.0], maxfev=2000)
            c0, c1, c2, c3 = popt
            
            # Construct SymPy representation
            A, D, H, P = sp.symbols('Area fin_density fin_height conduction_path_length')
            ...
            try:
                popt, _ = curve_fit(linear_func, (areas, densities, heights, paths), Y, p0=[60.0, -50.0, 30.0, -0.1])
                c0, c1, c2, c3 = popt
                A, D, H, P = sp.symbols('Area fin_density fin_height conduction_path_length')
    ...
    def run_discovery_loop(self, max_iterations=10):
        ...
        for iteration in range(max_iterations):
            ...
            for _ in range(100):
                ...
                distances = [np.linalg.norm(candidate - item[0]) for item in self.evaluated_points]
                ...
                sorted_items = sorted(self.evaluated_points, key=lambda item: np.linalg.norm(candidate - item[0]))
                ...
                temp_factor = max(0.0, 1.0 - abs(est_temp - 77.5) / 10.0) # centered at 77.5°C
            ...
            print(f"  [Experiment]: Testing active learning design at Area={next_design[7]:.3f}m2, Density={next_design[0]:.1f} fins/m")
            res = self.run_simulation_sandbox(next_design)
            self.evaluated_points.append((next_design, res))
            print(f"  [Sandbox Output]: Max CPU Temp: {res['max_temp']:.2f}°C, Mass: {res['mass']:.3f}kg")
            ...
            print(f"  [Epistemic Gain]: Iteration Gain: {gain:.4f}, Cumulative: {self.epistemic_gain:.4f}")
    ...
    def generate_discovery_report(self):
        ...
        critical_temps = [item[1]["max_temp"] for item in self.evaluated_points if item[1]["max_temp"] > 70.0]
        ...
        bounds_ranges = np.array([self.optimizer.bounds[k][1] - self.optimizer.bounds[k][0] for k in self.optimizer.param_keys])
        coverage = np.prod(ranges / bounds_ranges) * 100.0 # Percentage
        
        report = f"""# Autonomous Thermal Discovery Report
