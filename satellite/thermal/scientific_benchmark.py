...
def run_scientific_benchmark():
    ...
    print(f"Digital Twin vs. FEM -> RMSE: {rmse_twin:.4f}°C, MAE: {mae_twin:.4f}°C, R²: {r2_twin:.4%}")
    print(f"Single-Node vs. FEM  -> RMSE: {rmse_single:.4f}°C, MAE: {mae_single:.4f}°C, R²: {r2_single:.4%}")
...
\midrule
Single-Node Lumped & """ + f"{rmse_s:.3f}" + r""" & """ + f"{r2_s*100.0:.2f}" + r"""\% & 0.010 \\
\textbf{Coupled Digital Twin} & \textbf{""" + f"{rmse_t:.3f}" + r"""} & \textbf{""" + f"{r2_t*100.0:.2f}" + r"""\%} & \textbf{0.050} \\
...
The digital twin achieves a computational speedup of **""" + f"{speedup:.1f}" + r"""$\times$** compared to the high-fidelity solver, enabling real-time telemetry processing and prediction.
