...
class UncertaintyEngine:
    ...
    def predict_with_uncertainty(self, model, inputs, method="ensemble"):
        ...
        if method == "ensemble":
            ...
            n_simulations = 200 # Set to 200 for fast sandbox execution (can scale to 1000)
            max_temps = []
    ...
    def run_reliability_analysis(self, area=0.15, eps=0.85, power=15.0):
        ...
        """
        print(f"[*] Running Uncertainty Quantification for Area={area}m2, Emissivity={eps}, Power={power}W...")
        
        # 1. Physical bootstrap simulation
        uq_res = self.predict_with_uncertainty(None, [area, eps, power], method="bootstrap_physics")
        
        # 2. Compute safety probability
        rel_score = self.reliability_score(uq_res["mean"], uq_res["std"], self.critical_threshold)
        print(f" -> Mean CPU Max Temp: {uq_res['mean']:.2f}°C, Std: {uq_res['std']:.2f}°C")
        print(f" -> 95% Confidence Interval: [{uq_res['ci95'][0]:.2f}, {uq_res['ci95'][1]:.2f}]°C")
    ...
    def plot_distribution(self, uq_res, output_path):
        ...
        # Histogram
        n, bins, patches = ax.hist(data, bins=25, density=True, alpha=0.3, color='#00f0ff', edgecolor='#00f0ff')
        ...
        # Critical Limit Line
        ax.axvline(self.critical_threshold, color='red', linestyle='--', linewidth=2.0, label='Critical Limit (85°C)')
        # 95% bounds
        ax.axvline(uq_res["ci95"][0], color='#ffb821', linestyle=':', linewidth=1.5, label='95% Lower CI')
        ax.axvline(uq_res["ci95"][1], color='#ffb821', linestyle=':', linewidth=1.5, label='95% Upper CI')
        
        ax.set_title("LEO Cubesat Peak CPU Temperature Uncertainty Distribution", color='white', fontsize=12, pad=10)
