...
class TVACChamber:
    ...
    def read_pressure(self):
        ...
        if self.emulated:
            ...
            self.pressure_mbar = max(1e-7, self.pressure_mbar + np.random.normal(0, 1e-7))
    ...
    def simulate_tvac_run(self, network, duration_sec=5400, dt=10.0):
        ...
        for step_idx, t in enumerate(times):
            ...
            for i in range(6):
                Q_in = emu_net.Q[i]
                Q_cond = sum(emu_net.k[i, j] * (T_sensor[j] - T_sensor[i]) for j in range(6) if emu_net.k[i, j] > 0)
                Q_rad = emu_net.eps[i] * 5.67e-8 * emu_net.A[i] * (T_sensor[i]**4 - T_shroud**4)
            ...
            for i in range(6):
                Q_in = dt_net.Q[i]
                Q_cond = sum(dt_net.k[i, j] * (T_dt[j] - T_dt[i]) for j in range(6) if dt_net.k[i, j] > 0)
                Q_rad = dt_net.eps[i] * 5.67e-8 * dt_net.A[i] * (T_dt[i]**4 - T_shroud**4)
...
"""
    report_md += report_footer.replace("{AVG_PRESSURE}", f"{df_tvac['Pressure_mbar'].mean():.2e}")
...
def plot_tvac_results(df_tvac, output_path):
    ...
    ax1.plot(times_min, df_tvac["T_Meas_CPU"], label="Measured CPU Temp (Sensor PT100)", color="#38bdf8", linewidth=1.5, alpha=0.8)
    ax1.plot(times_min, df_tvac["T_Pred_CPU"], label="Predicted CPU Temp (Digital Twin)", color="#ff2a5f", linewidth=2.0, linestyle="--")
    ax1.plot(times_min, df_tvac["T_Real_CPU"], label="True Physical CPU Temp (Ideal)", color="#26ffad", linewidth=1.0, linestyle=":")
    ax1.set_title("CPU Node Thermal Vacuum Correlation (Active Heater Profile)", color='white', fontsize=12, pad=10)
    ...
    ax2.plot(times_min, df_tvac["T_Meas_Radiator"], label="Measured Radiator Temp (IR Sensor)", color="#ffb821", linewidth=1.5, alpha=0.7)
    ax2.plot(times_min, df_tvac["T_Pred_Radiator"], label="Predicted Radiator Temp (Digital Twin)", color="#00f0ff", linewidth=2.0, linestyle="--")
    ax2.set_title("Radiator Node Shroud Dissipation Correlation (Contactless IR)", color='white', fontsize=12, pad=10)
