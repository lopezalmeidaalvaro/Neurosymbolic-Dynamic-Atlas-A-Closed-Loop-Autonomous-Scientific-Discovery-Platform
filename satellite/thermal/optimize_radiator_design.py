...
def optimize_design():
    ...
    for idx in range(100):
        ...
        cost = area * (1.0 + (1.0 - emissivity))  # Cost proportional to area & coating complexity
    ...
    for step in range(200):
        ...
        if feasible_pts:
            ...
            area = float(np.clip(best_pt["area"] + np.random.normal(0, 0.05), 0.01, 0.50))
            emissivity = float(np.clip(best_pt["emissivity"] + np.random.normal(0, 0.05), 0.10, 0.95))
        else:
    ...
    if df_feasible.empty:
        print("Warning: No feasible designs under 85°C found. Relaxing constraint to find best candidates.")
    ...
    plt.scatter(
        df_all["max_temp"], df_all["area"], 
        c='gray', alpha=0.15, label='All Explored Designs'
    )
    
    # Highlight Pareto front
    sc = plt.scatter(
        df_pareto["max_temp"], df_pareto["area"], 
        c=df_pareto["cost"], cmap='viridis', s=60, edgecolors='black', 
        label='Pareto Front (Non-dominated)'
    )
    
    plt.colorbar(sc, label='Coating & Production Cost')
    plt.axvline(85.0, color='red', linestyle='--', label='Max Allowable Temperature (85°C)')
    plt.xlabel('Peak Temperature (°C)')
    plt.ylabel('Radiator Area / Mass (m²)')
    plt.title(f'Satellite Radiator Multi-Objective Pareto Optimization (Power: {fixed_power}W)')
    ...
    optimal_design_summary = {
        ...
        "efficiency_status": "Optimal" if best_design["max_temp"] < 65.0 else "Warning (Near thermal limit)"
    }
