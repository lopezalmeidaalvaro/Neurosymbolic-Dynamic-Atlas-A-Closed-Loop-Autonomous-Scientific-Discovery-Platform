...
MATERIALS_DB = {
    "Kapton HN": {
        ...
        "heritage": ["Hubble Space Telescope", "ISS", "James Webb Space Telescope", "Mars Pathfinder"]
    },
    "Teflon FEP": {
    ...
    "AZ-93 white paint": {
        ...
        "heritage": ["Space Shuttle", "Mars Science Laboratory (Curiosity)", "Artemis Orion"]
    },
    "Z306 black paint": {
    ...
    "Beta cloth": {
        ...
        "min_use_temp_C": -200.0,
        "heritage": ["Apollo spacesuits", "Space Shuttle payload bay", "ISS outer covers"]
    },
    "MLI 10-layer stack": {
        ...
        "max_use_temp_C": 300.0,
        "min_use_temp_C": -200.0,
        "heritage": ["Almost all deep space and LEO missions", "James Webb", "Mars Rovers"]
    },
    "Graphite epoxy composite": {
        ...
        "heritage": ["Chandra X-ray Observatory structure", "Mars Express", "Cubesat structures"]
    }
}
...
def get_material_by_cost(max_cost):
    ...
    """
    return [name for name, props in MATERIALS_DB.items() if props["relative_cost"] <= max_cost]

def apply_material(network, node_index, material_name, elapsed_time_years=0.0, atox_fluence=0.0):
    """
...
def compare_materials(material_list, scenario_name="LEO Nominal"):
    ...
    for name in material_list:
        ...
        records.append({
            ...
            "Uso Recomendado": "Radiadores" if p["alpha"] < 0.2 else ("Aislamiento MLI" if p["eps_BOL"] < 0.05 else "Estructura/Interior")
        })
