...
class CADThermalMesh:
    ...
    def create_dummy_cad_file(self, filepath, shape="cube"):
        ...
        for f in faces:
            ...
            v1, v2, v3 = np.array(f[0])*0.1, np.array(f[1])*0.1, np.array(f[2])*0.1
    ...
    def import_cad(self, filepath):
        ...
        print(f"[+] CAD: Imported mesh. Vertices: {self.vertices.shape[0]}, Faces: {self.faces.shape[0]}")
    ...
    def generate_thermal_mesh(self, mesh_data, shape_type="cube"):
        ...
        print(f"[+] CAD Voxelizer: Generated thermal mesh (Shape: {shape_type}). Occupied Voxels: {len(occupied_voxels)} / 1000 grid cells.")
    ...
    def extract_thermal_network(self, voxels):
        ...
        v_capacity = v_volume * RHO_AL * CP_AL # 0.000001 m3 * 2700 * 896 = 2.419 J/K per voxel
        ...
        network_data = {
            "nodes": nodes_info,
            "conductance_matrix": k_matrix.tolist()
        }
        
        # Save JSON
        with open("cad_thermal_network.json", "w") as f:
            json.dump(network_data, f, indent=4)
        print(f"[+] CAD network: Extracted thermal graph and saved to: cad_thermal_network.json")
        
        return network_data

    def simulate_3d_thermal(self, network, duration=3600):
        """
        Integrates thermodynamics over the voxelized 3D graph using solve_ivp.
        """
        print("[*] CAD network: Launching 3D transient thermal simulation...")
        nodes = network["nodes"]
        ...
        t_eval = np.arange(0, duration + 60.0, 60.0)
        ...
        print("[+] CAD network: Simulation finished successfully. Telemetry saved to: cad_simulation_results.csv")
    ...
    def visualize_3d_heatmap(self, network, temperatures, output_path):
        ...
        sc = ax.scatter(coords[:, 0], coords[:, 2], coords[:, 1], c=temperatures, 
                        cmap='coolwarm', s=45, alpha=0.9, edgecolors='black', linewidth=0.2)
        ...
        ax.set_title("3D CAD Voxelized Temperature Distribution Heatmap", color='white', fontsize=12, pad=15)
        ...
        plt.close()
        print(f"[+] CAD network: Saved 3D temperature heatmap visualization to: {output_path}")
...
def main():
    ...
    cad_path = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "cad", "cubesat_cube.stl")
