...
class HardwareInTheLoopSimulator(BaseHILAndSensorInterface):
    ...
    def __init__(self, miscalibrated=True):
        ...
        if miscalibrated:
            self.dt_C_cpu = 320.0     # Deliberately high (320 J/K instead of 200)
            self.dt_eps_rad = 0.55    # Deliberately low (0.55 instead of 0.85)
        else:
            self.dt_C_cpu = 200.0
    ...
    def connect_hardware(self, config=None):
        ...
        if sys.platform.startswith("linux") and os.path.exists("/sys/class/thermal/thermal_zone0/temp"):
    ...
    def run_hil_loop(self, duration=1800, interval=5):
        ...
        for step in range(n_steps + 1):
            ...
            action, power_next = self.control_action(T_predicted_horizon, threshold=80.0)
            ...
            if step % 60 == 0:
                print(f"  [Time {t_curr:4.0f}s]: Measured={T_measured:5.2f}°C, Pred_60s={T_predicted_horizon:5.2f}°C, Action={action:8s}, C_est={self.dt_C_cpu:6.2f}, eps_est={self.dt_eps_rad:5.3f}")
    ...
    def generate_report(self, df):
        ...
        report = f"""# Hardware-in-the-Loop (HIL) Real-Time Validation Report
