...
def run_validation():
    ...
    for i in range(10):
        ...
        Q_rad = (
            ...
            * area
            * (temps_K**4 - model.ambient_temp_K**4)
        )
    ...
    report_lines.append(
        f"**Verdict:** {p1_verdict} ({p1_passed}/10 Passed, {p1_excellent}/10 Excellent)"
    )
    report_lines.append(
    ...
    for idx, (p, a, e, err) in enumerate(p1_results):
        ...
        report_lines.append(
            f"| {idx+1} | {p:.2f} | {a:.4f} | {e:.2f} | {err*100:.4f}% | {status} |"
        )
    report_lines.append("\n")
    ...
    for i in range(10):
        ...
        model = ThermalServerModel(
            power=power, area=area, emissivity=emissivity, heat_capacity=500.0
        )
        ...
        p2_results.append(
            (
                ...
                area,
                emissivity,
                T_final_sim_K - 273.15,
                T_eq_analitica_K - 273.15,
                err_rel,
            )
        )
    ...
    report_lines.append(
        f"**Verdict:** {p2_verdict} ({p2_passed}/10 Converged under 0.5%)"
    )
    report_lines.append(
    ...
    for idx, (p, a, e, sim_t, ana_t, err) in enumerate(p2_results):
        ...
        report_lines.append(
            f"| {idx+1} | {p:.2f} | {a:.4f} | {e:.2f} | {sim_t:.2f} | {ana_t:.2f} | {err*100:.4f}% | {status} |"
        )
    report_lines.append("\n")
    ...
    base_model = ThermalServerModel(
        power=20.0, area=0.10, emissivity=0.80, initial_temp=100.0
    )
    base_res = base_model.simulate()
    base_max = base_res["max_temp"]

    # ↑power
    p_up_model = ThermalServerModel(
        power=30.0, area=0.10, emissivity=0.80, initial_temp=100.0
    )
    ...
    if p_up_max > base_max:
        ...
        p3_passed = False
        p3_logs.append(
            f"FAIL: ↑power did not increase max_temp (base_max={base_max:.2f}, p_up_max={p_up_max:.2f})"
        )

    # ↑area
    a_up_model = ThermalServerModel(
        power=20.0, area=0.20, emissivity=0.80, initial_temp=100.0
    )
    ...
    if a_up_max < base_max:
        ...
        p3_passed = False
        p3_logs.append(
            f"FAIL: ↑area did not decrease max_temp (base_max={base_max:.2f}, a_up_max={a_up_max:.2f})"
        )

    # ↑emissivity
    e_up_model = ThermalServerModel(
        power=20.0, area=0.10, emissivity=0.90, initial_temp=100.0
    )
    ...
    if e_up_max < base_max:
        ...
        p3_passed = False
        p3_logs.append(
            f"FAIL: ↑emissivity did not decrease max_temp (base_max={base_max:.2f}, e_up_max={e_up_max:.2f})"
        )
