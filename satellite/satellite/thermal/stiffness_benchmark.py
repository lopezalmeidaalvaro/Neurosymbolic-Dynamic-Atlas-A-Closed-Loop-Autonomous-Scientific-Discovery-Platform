...
def run_benchmark():
    ...
    config_dict = {
        "C": [
            ...
            200.0,
            300.0,
        ],  # Stiff: CPU C is extremely small (10 J/K instead of 200 J/K)
    ...
    for solver in solvers:
        ...
        try:
            ...
            print(
                f" -> {solver:5s} | Status: SUCCESS | Elapsed: {elapsed:6.4f}s | CPU bounds: [{min_cpu:5.2f}C, {max_cpu:5.2f}C]"
            )
            ...
            print(
                f" -> {solver:5s} | Status: FAILED  | Elapsed: {elapsed:6.4f}s | Error: {e}"
            )
    ...
    for solver in solvers:
        ...
        if data["status"] == "SUCCESS":
            report_lines.append(
                f"| **{solver}** | SUCCESS | {data['time_sec']:.4f}s | {data['min_cpu_temp']:.2f}°C | {data['max_cpu_temp']:.2f}°C |"
            )
        else:
            report_lines.append(
                f"| **{solver}** | FAILED | {data['time_sec']:.4f}s | N/A | N/A |"
            )
