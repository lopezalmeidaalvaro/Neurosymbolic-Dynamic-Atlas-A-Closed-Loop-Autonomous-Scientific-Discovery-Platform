...
def simulate_mission_lifetime(network, mission_duration_days=365, orbit_params=None):
    ...
    for day in eval_steps:
        ...
        results.append(row)
        print(
            f" -> Day {day:3d}: CPU Max Temp = {row['T_Max_CPU']:.2f}°C, Radiator Emissivity = {row['Radiator_Emissivity']:.4f}"
        )
...
def plot_degradation_trends(df_results, output_path):
    ...
    ax1.plot(
        days,
        df_results["Radiator_Emissivity"],
        label="Radiator Emissivity (Node 4)",
        color="#38bdf8",
        linewidth=2.0,
    )
    ax1.plot(
        days,
        df_results["Panels_Emissivity"],
        label="Solar Panels Emissivity (Node 5)",
        color="#ffb821",
        linewidth=2.0,
    )
    ax1.set_title(
        "Material Thermal Properties Degradation Over Mission Life",
        color="white",
        fontsize=12,
        pad=10,
    )
    ...
    ax2.plot(
        days,
        df_results["T_Max_Structure"],
        label="Structure Max Temperature",
        color="#a55eff",
        linewidth=1.5,
    )
    ax2.axhline(
        ...
        color="#ff2a5f",
        linestyle=":",
        label="CPU Critical Limit (85°C)",
        alpha=0.6,
    )

    ax2.set_title(
        "Long-term Thermal Peak Drifts and Limit Approximations",
        color="white",
        fontsize=12,
        pad=10,
    )
    ...
    ax2.legend(
        facecolor="#0f172a", edgecolor="#1e293b", labelcolor="white", loc="lower right"
    )
...
def generate_aging_report(df_results, pred):
    ...
    report_template = r"""# Spacecraft Material Degradation & Thermal Drift Report
...
"""
    ...
    report_md = (
        ...
        .replace(
            "{INIT_COND}", f"{df_results['CPU_Structure_Conductance'].iloc[0]:.4f}"
        )
        .replace(
            "{EOL_COND}", f"{df_results['CPU_Structure_Conductance'].iloc[-1]:.4f}"
        )
...
"""
    report_md += (
        report_footer.replace("{REG_R2}", f"{pred['regression_r2']:.6f}")
