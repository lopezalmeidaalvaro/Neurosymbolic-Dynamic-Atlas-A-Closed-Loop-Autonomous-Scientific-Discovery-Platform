...
def total_power_profile(
    t, T_bat, heater_state, pass_schedule, imaging_schedule, slew_schedule
):
    """
        ...
        for k in range(num_steps - 1):
            ...
            def step_ode(t_val, T_val):
                return self.dTdt(
                    ...
                    t_val,
                    Q_solar_func(t_val),
                    use_cavity_radiation=use_cavity_radiation,
                )
...
def run_transient_study():
    ...
    for node in net.node_names:
        print(
            f"Nodo: {node:10s} | T_max: {res['max_temps'][node]:6.2f}°C | Tiempo Sobrecalentamiento: {res['overheat_times'][node]:5.1f} s"
        )
    print(
        f"Tiempo Total Sobrecalentamiento Satélite: {res['total_overheat_time']:.1f} s\n"
    )
    ...
    ax1.plot(
        t_min,
        res["p_heater"],
        label="Calefactor Batería",
        color="#ffb821",
        linewidth=1.5,
    )
    ax1.plot(
        t_min,
        np.array(res["p_payload"]) - 5.0,
        label="Payload Bursts (+8W)",
        color="#26ffad",
        linewidth=1.5,
    )
    ax1.plot(
        t_min,
        res["p_total"],
        label="Consumo Eléctrico Total (W)",
        color="#a55eff",
        linewidth=2.0,
    )

    ax1.set_title(
        "Telemetría de Potencia Transitoria (3 Órbitas)",
        color="white",
        fontsize=12,
        pad=10,
    )
    ...
    for i in range(6):
        ...
        ax2.plot(
            t_min,
            res["temperatures"][i],
            label=f"{name} (Max: {res['max_temps'][name]:.1f}°C)",
            color=colors[i],
            linewidth=1.8,
        )
        # Critical Limit
        ax2.axhline(
            net.critical_limits[name], color=colors[i], linestyle=":", alpha=0.3
        )

    ax2.set_title(
        "Respuesta Térmica de Nodos con Radiación Interna",
        color="white",
        fontsize=12,
        pad=10,
    )
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        f.write(
            f"**Generado:** {time.strftime('%Y-%m-%d %H:%M:%S')} | **Semilla Principal:** 42\n\n"
        )
        f.write(
        ...
        f.write("| :--- | :---: | :---: | :---: | :---: |\n")
        f.write(
            f"| **T_max CPU** | {df_dataset['T_max_CPU'].mean():.2f}°C | {df_dataset['T_max_CPU'].std():.2f}°C | {df_dataset['T_max_CPU'].max():.2f}°C | 85.00°C |\n"
        )
        f.write(
            f"| **T_max Batería** | {df_dataset['T_max_Battery'].mean():.2f}°C | {df_dataset['T_max_Battery'].std():.2f}°C | {df_dataset['T_max_Battery'].max():.2f}°C | 50.00°C |\n"
        )
        f.write(
            f"| **T_max Payload** | {df_dataset['T_max_Payload'].mean():.2f}°C | {df_dataset['T_max_Payload'].std():.2f}°C | {df_dataset['T_max_Payload'].max():.2f}°C | 60.00°C |\n"
        )
        f.write(
            f"| **Consumo Energía (Wh)** | {df_dataset['Total_Energy_Wh'].mean():.2f} Wh | {df_dataset['Total_Energy_Wh'].std():.2f} Wh | {df_dataset['Total_Energy_Wh'].max():.2f} Wh | - |\n"
        )
        f.write(
            f"| **Sobrecalentamiento CPU (s)** | {df_dataset['Overheat_CPU_sec'].mean():.1f} s | {df_dataset['Overheat_CPU_sec'].std():.1f} s | {df_dataset['Overheat_CPU_sec'].max():.1f} s | - |\n"
        )
        f.write(
            f"| **Sobrecalentamiento Batería (s)** | {df_dataset['Overheat_Battery_sec'].mean():.1f} s | {df_dataset['Overheat_Battery_sec'].std():.1f} s | {df_dataset['Overheat_Battery_sec'].max():.1f} s | - |\n"
        )
