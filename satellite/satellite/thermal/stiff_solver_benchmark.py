    ...
    scenarios["2. Alta carga"] = {
        "config": {
            ...
            "A": [
                ...
                0.05,
                0.20,
            ],  # Radiator area A[4] reduced to 0.05 m2
        },
...
class DynamicThermalNetwork(ThermalNetwork):
    ...
    def __init__(
        self, config_dict=None, active_control=False, degradation=False, duration=5400.0
    ):
...
def run_solver_scenario(scenario_name, sc_data, solver_name, rtol=1e-6, atol=1e-6):
    ...
    with warnings.catch_warnings(record=True) as caught_warnings:
        ...
        try:
            ...
            for w in caught_warnings:
                if (
                    "integration" in str(w.message).lower()
                    or "step size" in str(w.message).lower()
                    or "excess work" in str(w.message).lower()
                ):
    ...
    # Energy error
    energy_error = compute_energy_conservation(
        sol, net, solar_func, dynamic_net=(active_control or degradation)
    )
...
def main():
    ...
    for sc_name, sc_data in scenarios.items():
        ...
        ref_interp = interp1d(
            ref_res["t"], ref_res["y"], kind="cubic", fill_value="extrapolate"
        )
        y_ref_grid = ref_interp(t_grid)  # shape (6, 1000)

        trajectories[sc_name] = {"t_grid": t_grid, "y_ref": y_ref_grid, "solvers": {}}
        ...
        for solver in solvers:
            ...
            if res["status"] == "SUCCESS":
                ...
                sol_interp = interp1d(
                    res["t"], res["y"], kind="linear", fill_value="extrapolate"
                )
                ...
                print(
                    f" SUCCESS | {res['runtime_s']:.3f}s | {res['steps']} pasos | Error Rel: {rel_err:.2e} | Conservación Energía: {energy_conserved}"
                )
    ...
    for idx, (sc_name, sc_data) in enumerate(trajectories.items()):
        ...
        ax.plot(
            sc_data["t_grid"] / 60.0,
            sc_data["y_ref"][0] - 273.15,
            label="Ref (Radau tol=1e-10)",
            color="white",
            linestyle="--",
            alpha=0.8,
            linewidth=2.0,
    ...
    for sc in df["Scenario"].unique():
        ...
        valid_df = sc_df[
            (sc_df["Status"] == "SUCCESS") & (sc_df["EnergyConserved"] == True)
        ]
        if valid_df.empty:
            ...
            recommendations[sc] = (
                f"**{fastest['Solver']}** (Más rápido: {fastest['Runtime_s']:.3f}s, {fastest['Steps']} pasos)"
            )
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        for _, r in df.iterrows():
            ...
            cons_str = (
                f"Sí ({r['EnergyErrorPct']:.2f}%)"
                if r["EnergyConserved"]
                else (
                    f"No ({r['EnergyErrorPct']:.2f}%)"
                    if not pd.isna(r["EnergyErrorPct"])
                    else "N/A"
                )
            )
            f.write(
                f"| {r['Scenario']} | **{r['Solver']}** | {r['Status']} | {r['Steps']} | {r['Runtime_s']:.4f}s | {err_str} | {warn_str} | {cons_str} |\n"
            )
