...
class ThermalServerModel:
    ...
    def __init__(
        self, power, area, emissivity, heat_capacity=500.0, initial_temp=293.15
    ):
        """
        Constructor.
    ...
    def simulate(self, duration=3600.0, dt=10.0):
        ...
        for i in range(1, steps):
            ...
            if T >= self.critical_temp_K and time_to_critical is None:
                if T_prev < self.critical_temp_K:
                    t_prev = times[i - 1]
                    fraction = (
                        (self.critical_temp_K - T_prev) / (T - T_prev)
                        if (T - T_prev) != 0
                        else 0.0
                    )
        ...
        noise = np.random.normal(
            0.0, 0.01 * np.abs(T_media_C), size=(grid_size, grid_size)
        )
        ...
        for i in range(grid_size):
            for j in range(grid_size):
                ...
                T_celda = (
                    T_media_C
                    + delta_T * np.exp(-(r**2) / (sigma_gauss**2))
                    + noise[i, j]
                )
    ...
    def steady_state_temp(self):
        ...
        """
        T_eq_K = (
            self.power / (self.emissivity * self.stefan_boltzmann * self.area)
