...
def ingest_and_calibrate():
    ...
    nasa_cubesat_data = [
        ...
        {
            "mission": "NASA CubeSat-3",
            "power": 48.0,
            "area": 0.10,
            "emissivity": 0.75,
            "max_temp": 86.1,
            "time_to_critical": 1250.0,
        },
    ]

    esa_opssat_data = [
        {
            "mission": "ESA OPS-SAT-A",
            "power": 8.0,
            "area": 0.02,
            "emissivity": 0.90,
            "max_temp": 32.1,
    ...
    kaggle_space_data = [
        ...
        {
            "mission": "Kaggle Craft-X3",
            "power": 30.0,
            "area": 0.04,
            "emissivity": 0.60,
            "max_temp": 92.5,
            "time_to_critical": 840.0,
        },
    ]
    ...
    if not os.path.exists(rf_model_path):
        raise FileNotFoundError(
            f"Surrogate model not found at {rf_model_path}. Train surrogates first."
        )
    ...
    print(
        f" -> Simulation-to-Reality temperature gap measured: Mean error = {avg_temp_gap:.2f}°C, Max error = {max_temp_gap:.2f}°C"
    )
    ...
    print(
        f" -> Calibrated surrogate model error reduced to: Mean error = {avg_temp_gap_calib:.2f}°C"
    )
    ...
    for idx, row in df_real.iterrows():
        t_crit_str = (
            f"{row['time_to_critical']:.0f}s"
            if row["time_to_critical"] >= 0
            else "Safe"
        )
        report_lines.append(
            f"| {row['mission']} | {row['power']:.1f} | {row['area']:.3f} | {row['emissivity']:.2f} | {row['max_temp']:.1f} | {t_crit_str} |"
        )
