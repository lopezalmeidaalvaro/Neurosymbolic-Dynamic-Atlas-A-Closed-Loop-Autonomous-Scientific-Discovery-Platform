...
def calculate_margins_and_verify():
    ...
    df_aging = simulate_mission_lifetime(
        net, mission_duration_days=365, orbit_params=orbit_params
    )
    eol_emissivity = df_aging["Radiator_Emissivity"].iloc[-1]
    emissivity_degradation_pct = (
        (net.base_eps[4] - eol_emissivity) / net.base_eps[4]
    ) * 100.0
    ...
    verification_records = [
        ...
        },
        {
            ...
            "Status": (
                "PASSED" if (margin_r2_hot >= 0 and margin_r2_cold >= 0) else "FAILED"
            ),
        },
        {
            "ID": "R3",
...
def generate_compliance_pdf(df_matrix, margins):
    ...
    try:
        ...
        # 1. Header and Title
        story.append(
            Paragraph("Spacecraft Flight Qualification Certificate", title_style)
        )
        story.append(
            Paragraph(
                f"<b>Verification Date:</b> {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} &nbsp;&nbsp;|&nbsp;&nbsp; <b>Model Ver:</b> T27-ECSS-1.0",
                body_style,
            )
        )
        story.append(
            Paragraph(
                "<b>Reference Standards:</b> ESA ECSS-E-ST-31C, ECSS-Q-ST-70C, ECSS-E-ST-10-02C",
                body_style,
            )
        )
        ...
        story.append(
            Paragraph(
                "This document certifies that the multi-node thermal digital twin of the LEO Cubesat has been evaluated against the European Space Agency standard qualification envelopes. The verification matrix below compiles model simulation predictions, joint aging degradation shifts, and design safety margins under uncertainty models.",
                body_style,
            )
        )
        story.append(Spacer(1, 10))
        ...
        story.append(
            Paragraph("<b>Flight Qualification Authority Signatures:</b>", h2_style)
        )
        story.append(Spacer(1, 10))
        ...
        sig_data = [
            ...
            [
                "Space Thermal Systems Lead (ESA-ESTEC)",
                "Digital Twin Certification Engine",
            ],
        ]
        ...
        print(
            f"[!] ReportLab PDF compiler warning: {e}. Writing safe ECSS PDF certificate..."
        )
