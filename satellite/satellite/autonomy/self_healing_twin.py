...
class SelfHealingTwin:
    ...
    def detect_drift(self, t):
        ...
        last_10_means = [
            np.mean(
                np.array(self.residuals_history[-self.sliding_window_len + k :])[:, 0]
            )
            for k in range(21)
        ]
        if all(abs(m) > 3.0 for m in last_10_means[-10:]):
            return (
                "DRIFT_DETECTED",
                f"Mean residual bias is {mean_res:.2f}°C persistently",
            )
    ...
    def autocalibrate(self, t):
        ...
        measured_window = np.array(
            self.measured_history[-self.sliding_window_len :]
        )  # Shape: (30, 6)
        ...
        res = scipy.optimize.minimize(
            loss_func, x0, method="Nelder-Mead", options={"maxiter": 100}
        )
        ...
        if res.success:
            ...
            if (0.05 <= eps_opt <= 0.95) and (100.0 <= C_opt <= 5000.0):
                ...
                print(
                    f"    [SUCCESS] Calibración aplicada: Emisividad={eps_opt:.4f}, Capacidad CPU={C_opt:.1f} J/K"
                )
                print(
                    f"              MSE reducida de {error_before:.2f} a {error_after:.2f}"
                )
                return True
            else:
                self.logs.append(
                ...
                print(
                    "    [FAILED] Parámetros optimizados no pasaron el control de plausibilidad física."
                )
        else:
...
def run_self_healing_sim():
    ...
    plt.plot(
        t_min,
        df_results["True_Temp_CPU"],
        label="CPU Real (Físico)",
        color="#ff2a5f",
        linewidth=2.0,
    )
    plt.plot(
        t_min,
        df_results["Twin_Temp_CPU"],
        label="CPU Gemelo Digital (Predicho)",
        color="#00f0ff",
        linewidth=2.0,
        linestyle="--",
    )
    ...
    if healed:
        healing_time = (
            df_results[df_results["Healed"] == 1]["Timestamp_s"].values[0] / 60.0
        )
        plt.axvline(
            healing_time,
            color="#26ffad",
            linestyle=":",
            label=f"Autocalibración Aplicada (t={healing_time:.1f}min)",
        )

    ax.set_title(
        "Validación de Autocalibración y Self-Healing del Gemelo Digital",
        color="white",
        fontsize=13,
        pad=15,
    )
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        ...
        if healed:
            ...
            f.write(
                f"| **Emisividad Radiador ($\\epsilon$)** | 0.85 | 0.45 | {log_entry['Adjusted_Emissivity']:.4f} | **{(abs(log_entry['Adjusted_Emissivity'] - 0.45)/0.45)*100.0:.2f}%** |\n"
            )
            f.write(
                f"| **Capacidad Térmica CPU ($C$)** | 200.0 | 200.0 | {log_entry['Adjusted_Capacity']:.1f} | **{(abs(log_entry['Adjusted_Capacity'] - 200.0)/200.0)*100.0:.2f}%** |\n\n"
            )
            ...
            f.write(
                "> - **Autocalibración**: La optimización Nelder-Mead redujo el error cuadrático medio (MSE) de residuos de **"
                + f"{log_entry['MSE_Before']:.2f}** a **{log_entry['MSE_After']:.2f}**, realineando la predicción del Gemelo Digital con las mediciones reales del hardware.\n\n"
            )
        else:
            f.write(
                "| **Emisividad Radiador ($\\epsilon$)** | 0.85 | 0.45 | No Sanado | - |\n\n"
            )
