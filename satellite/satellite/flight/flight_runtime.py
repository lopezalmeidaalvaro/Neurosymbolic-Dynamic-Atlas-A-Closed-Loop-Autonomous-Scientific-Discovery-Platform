...
def main():
    ...
    if not onnx_path.exists():
        raise FileNotFoundError(
            f"ONNX model not found at {onnx_path}. Run export_to_onnx.py first."
        )
    ...
    print(
        "Ciclo | Temp Medida | Potencia OBC | Pred Max Temp | Pred T_Critical | Acción"
    )
    print(
        "---------------------------------------------------------------------------------"
    )
    ...
    for c in range(1, cycles + 1):
        ...
        raw_input = np.array(
            [[current_power, radiator_area, radiator_eps]], dtype=np.float32
        )
        ...
        print(
            f"{c:5d} | {cpu_temp_measured:10.2f}°C | {current_power:10.1f}W | {pred_max_temp:11.2f}°C | {pred_time_to_crit:13.1f}s | {action:8s}"
        )
