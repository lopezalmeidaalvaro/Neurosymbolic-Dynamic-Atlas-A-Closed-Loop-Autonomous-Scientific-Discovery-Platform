...
class RobustEKF:
    ...
    def __init__(self, C, eps, A, k, dt=5.0):
        ...
        self.R = np.eye(self.n_states) * 0.25 # Measurement noise covariance (sigma = 0.5C -> var = 0.25)
        self.R0 = self.R.copy()
    ...
    def predict(self, u_Q_internal, u_Q_solar, use_cavity_radiation=True):
        ...
        def ode_func(T_val):
            return net.dTdt(T_val, 0.0, u_Q_solar, use_cavity_radiation=use_cavity_radiation)
    ...
    def update(self, z, t_gap, is_standard=False):
        ...
        if self.recovery_steps > 0:
            ...
            self.logs.append(f"LOS Recovery Covariance Contraction remaining: {self.recovery_steps}")
        ...
        for i in range(self.n_states):
            ...
            if is_nan or self.sensor_stuck_counter[i] > 10:
                if self.sensor_status[i] == "nominal":
                    ...
                    self.logs.append(f"Sensor {i} DEGRADED! NaN={is_nan}, Stuck count={self.sensor_stuck_counter[i]}")
            else:
        ...
        for i in active_sensors:
            ...
            if abs(nu_i) > 5.0 * sigma_innovation:
                self.logs.append(f"Outlier rejected on sensor {i}: nu={nu_i:.2f}, limit={5.0*sigma_innovation:.2f}")
                continue
            ...
            alpha = 0.05
            # R update
            self.R[i, i] = (1.0 - alpha) * self.R[i, i] + alpha * (nu_i**2 - self.P[i, i])
            self.R[i, i] = np.clip(self.R[i, i], 1e-3, 10.0) # Stability bounds
            
            # Q update
            self.Q[i, i] = (1.0 - alpha) * self.Q[i, i] + alpha * (K_i[i]**2 * nu_i**2)
        ...
        if p_trace > 50.0:
            ...
            self.logs.append(f"EKF DIVERGED! P trace={p_trace:.2f} (Divergence count={self.divergence_count})")
            if self.divergence_count >= 3:
                self.prediction_only_mode = True
                self.logs.append("Permanent PREDICTION ONLY Mode enabled due to repeated divergences.")
            else:
                ...
                self.Q = self.Q0.copy()
                self.R = self.R0.copy()
                self.logs.append("EKF state and covariances successfully reinitialized!")
...
def simulate_ekf_comparative():
    ...
    res_true = net_phys.simulate(duration=duration, dt=dt, method='LSODA', use_cavity_radiation=True)
    ...
    for i in range(6):
        ...
        print(f"Nodo: {name:10s} | EKF Std RMSE: {std_rmse:6.2f}°C (Peak: {std_peak:5.1f}°C) | EKF Rob RMSE: {rob_rmse:6.2f}°C (Peak: {rob_peak:5.1f}°C)")
    ...
    ax1.plot(t_min, true_temps_k[0] - 273.15, label="CPU Verdad Fundamental", color='#ff2a5f', linewidth=2.0)
    ax1.plot(t_min, z_measured[0] - 273.15, label="CPU Sensor Medido (Con ruidos y outliers)", color='#ffb821', alpha=0.3, linestyle='None', marker='.')
    ax1.plot(t_min, ekf_std_states[0] - 273.15, label="EKF Estándar", color='#a55eff', linestyle='--', linewidth=1.5)
    ax1.plot(t_min, ekf_rob_states[0] - 273.15, label="EKF Robusto (Rechaza outliers)", color='#00f0ff', linewidth=1.8)
    ...
    ax1.set_title("Estimación Térmica de CPU (Outliers e Intervalos LOS en sombra roja)", color='white', fontsize=12, pad=10)
    ...
    ax2.plot(t_min, true_temps_k[1] - 273.15, label="Battery Verdad Fundamental", color='#ffb821', linewidth=2.0)
    ax2.plot(t_min, z_measured[1] - 273.15, label="Battery Sensor Medido (Stuck NaN después de 133min)", color='#26ffad', alpha=0.3, linestyle='None', marker='.')
    ax2.plot(t_min, ekf_std_states[1] - 273.15, label="EKF Estándar", color='#a55eff', linestyle='--', linewidth=1.5)
    ax2.plot(t_min, ekf_rob_states[1] - 273.15, label="EKF Robusto (Identifica degradación)", color='#00f0ff', linewidth=1.8)
    
    ax2.set_title("Estimación Térmica de Batería (Degradación por pérdida de sensor)", color='white', fontsize=12, pad=10)
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Informe de EKF Robusto con Resiliencia ante Pérdidas de Señal (Fase T38)\n\n")
        f.write(f"**Generado:** {time.strftime('%Y-%m-%d %H:%M:%S')} | **Semilla:** 42\n\n")
        f.write("Este informe valida la robustez matemática del estimador de estado EKF adaptativo del Cubesat bajo fallos graves de telemetría (gaps de 40 minutos por eclipse, fallos persistentes del sensor de batería y picos extremos de ruido).\n\n")
        
        f.write("## 1. Tabla Comparativa de Desempeño Térmico (RMSE)\n\n")
        f.write("| Nodo | EKF Estándar RMSE (°C) | EKF Robusto RMSE (°C) | Reducción de Error | Estado de Estabilidad |\n")
        f.write("| :--- | :---: | :---: | :---: | :--- |\n")
        for _, r in df_comp.iterrows():
            improvement = (r['Std_RMSE_C'] - r['Robust_RMSE_C']) / r['Std_RMSE_C'] * 100.0
            rec_str = "Divergencia acotada" if r['Robust_RMSE_C'] < 1.5 else "Estable de alta precisión"
            f.write(f"| **{r['Node']}** | {r['Std_RMSE_C']:.3f}°C | {r['Robust_RMSE_C']:.3f}°C | **{improvement:.1f}%** | {rec_str} |\n")
            
        f.write("\n## 2. Análisis del Diseño de Resiliencia\n\n")
        f.write("> [!IMPORTANT]\n")
        f.write("> **Mitigaciones y Algoritmos Implementados:**\n")
        f.write("> 1. **Inflado de Covarianza en Eclipse (LOS)**: Durante los gaps de 40 minutos en eclipse ($t_{\\text{gap}} > 60$s), el filtro infla su incertidumbre $P = P + Q\\cdot(t_{\\text{gap}})^2$. Al restablecer la señal, contrae gradualmente la matriz para asimilar los nuevos datos sin perturbar el estimador.\n")
        f.write("> 2. **Descarte de Sensores Degradados (Dropouts)**: Cuando el sensor de batería se queda trabado en NaN después de 133 min, el filtro robusto deshabilita dinámicamente su actualización de medición ($H_{1,1} = 0$), basando su estado en la predicción analógica acoplada del modelo.\n")
        f.write("> 3. **Puerta de Innovación (Outlier Gating)**: Los picos esporádicos inducidos en CPU (+15K) y Payload (-12K) son filtrados con éxito al superar el límite crítico de $5\\cdot\\sigma_{\\text{innovation}}$, evitando la propagación del error al resto del chasis.\n\n")
