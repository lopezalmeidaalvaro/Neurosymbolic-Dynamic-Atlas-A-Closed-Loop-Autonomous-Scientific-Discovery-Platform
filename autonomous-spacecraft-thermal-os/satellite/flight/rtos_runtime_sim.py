...
class MockRTOS:
    ...
    def static_malloc(self, size_kb, name):
        ...
        """
        if self.heap_locked:
            raise MemoryError(f"RTOS Violation: Dynamic allocation (malloc) of {size_kb}KB for '{name}' after startup is forbidden!")
        self.malloc_calls += 1
    ...
    def simulate_task_execution(self, name, current_tick, cpu_load_modifier=1.0):
        ...
        if name == "INFERENCE" and random.random() < 0.02:
            # CPU Spike event!
            exec_time += random.uniform(30.0, 60.0) # Might exceed watchdog limit of 50ms!
            self.events_log.append((current_tick, "CPU_INFERENCE_SPIKE", f"Inference execution time surged to {exec_time:.2f}ms"))
            
        if name == "COMMS" and random.random() < 0.05:
            # Telemetry connection retry under EMI or signal loss
            exec_time += random.uniform(5.0, 15.0)
            self.events_log.append((current_tick, "COMMS_RETRY", f"COMMS retry delay: {exec_time:.2f}ms"))
        ...
        if name == "INFERENCE":
            ...
            if exec_time > self.watchdog_timeout_ms:
                ...
                self.events_log.append((current_tick, "WATCHDOG_RESET", f"Inference exceeded 50ms limit ({exec_time:.1f}ms)! Watchdog Reset triggered."))
            else:
...
def run_rtos_simulation():
    ...
    print(f"    - Heap bloqueado con éxito. Intentar malloc posterior fallará de forma segura.\n")
    ...
    total_ticks = 8640
    print(f"[*] Ejecutando simulación de vuelo acelerada de 24 horas ({total_ticks} ticks)...")
    ...
    for tick in range(total_ticks):
        ...
        if tick % rtos.tasks["COMMS"]["period"] == 0:
            exec_t = rtos.simulate_task_execution("COMMS", tick, cpu_load_modifier=load_mod)
        ...
        if tick % rtos.tasks["INFERENCE"]["period"] == 0:
            exec_t = rtos.simulate_task_execution("INFERENCE", tick, cpu_load_modifier=load_mod)
            tick_cpu_time += exec_t
            active_tasks_count += 1
        ...
        if tick % rtos.tasks["HOUSEKEEPING"]["period"] == 0:
            exec_t = rtos.simulate_task_execution("HOUSEKEEPING", tick, cpu_load_modifier=load_mod)
            tick_cpu_time += exec_t
            active_tasks_count += 1
    ...
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("# Informe de Restricciones de Vuelo RTOS y Determinismo (Fase T40)\n\n")
        f.write(f"**Generado:** {time.strftime('%Y-%m-%d %H:%M:%S')} | **Semilla:** 42\n\n")
        f.write("Este informe presenta la validación del software de vuelo (FSW) ejecutado en un entorno simulado de RTOS (FreeRTOS) con restricciones estrictas de hardware embarcado ARM Cortex-M.\n\n")
        
        f.write("## 1. Presupuesto de Memoria RAM Estática (Zero-Malloc)\n\n")
        f.write(f"| Componente | Memoria RAM Asignada (KB) | Presupuesto Disponible (KB) | Tipo de Asignación | Estado |\n")
        f.write(f"| :--- | :---: | :---: | :---: | :--- |\n")
        for k, v in rtos.static_allocations.items():
            f.write(f"| **{k}** | {v:.1f} KB | {rtos.RAM_LIMIT_KB:.1f} KB | Estática (Boot) | Bloqueada |\n")
        f.write(f"| **TOTAL RAM UTILIZADA** | **{rtos.total_ram_used_kb:.1f} KB** | **{rtos.RAM_LIMIT_KB:.1f} KB** | - | **Aprobada ({rtos.total_ram_used_kb/rtos.RAM_LIMIT_KB*100.0:.1f}%)** |\n\n")
        ...
        f.write(f"> - **Jitter Promedio**: `{avg_jitter:.3f} ms` (Límite crítico: `< 1.0 ms`)\n")
        f.write(f"> - **Watchdog Resets**: `{rtos.watchdog_resets}` eventos registrados. En el caso de sobrecarga grave ($> 50$ ms), el sistema de watchdog externo forzó el reinicio físico del FSW para restablecer el determinismo.\n\n")
        
        f.write("## 3. Registro de Eventos RTOS Críticos\n\n")
        f.write("A continuación se enumeran los eventos críticos detectados por el kernel del scheduler durante las 24 horas de operación:\n\n")
        ...
        f.write("El software de vuelo cumple plenamente con los requisitos de **Zero Dynamic Memory Allocation** (evitando la fragmentación de memoria en misiones largas) y mantiene un jitter inferior a 1ms bajo condiciones nominales. Los eventos de reinicio por Watchdog son absorbidos con éxito por la lógica de arranque en frío de la CPU.\n")
