...
def test_energy_conservation():
    ...
    for temp in T_final:
        assert -100.0 <= temp <= 200.0, f"Temperature {temp}C is physically unrealistic!"
...
def test_orbital_validation():
    ...
    sol_flux_shadow, eclipse_factor_shadow = solar_flux(shadow_timestamp, orbit_params, beta_angle=0)
    alb_flux_shadow = albedo_flux(shadow_timestamp, orbit_params, beta_angle=0)
    
    sol_flux_sun, eclipse_factor_sun = solar_flux(sunlit_timestamp, orbit_params, beta_angle=0)
    ...
    if eclipse_factor_shadow == 0.0:
        assert sol_flux_shadow == 0.0, "Solar flux must be zero in complete eclipse shadow!"
