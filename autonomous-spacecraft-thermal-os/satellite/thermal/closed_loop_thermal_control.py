...
class ClosedLoopController:
    ...
    def predict_horizon(self, T_current: np.ndarray, horizon_sec: float, Q_solar_avg: float, active_Q: np.ndarray, active_eps: np.ndarray) -> np.ndarray:
        """
        ...
        for _ in range(steps):
            ...
            for i in range(6):
                ...
                # Radiation
                Q_rad = active_eps[i] * SIGMA * self.net.A[i] * ((T[i] + 273.15)**4 - T_SPACE**4)
    ...
    def decide_action(self, T_current: np.ndarray, Q_solar_avg: float, current_Q: np.ndarray, current_eps: np.ndarray) -> Tuple[str, np.ndarray, np.ndarray]:
        """
        Look-Ahead Decisor:
        ...
        """
        # Predict horizons
        T_120 = self.predict_horizon(T_current, 120.0, Q_solar_avg, current_Q, current_eps)
        T_300 = self.predict_horizon(T_current, 300.0, Q_solar_avg, current_Q, current_eps)
        T_600 = self.predict_horizon(T_current, 600.0, Q_solar_avg, current_Q, current_eps)
...
def run_scenarios():
    ...
    for sc in scenarios:
        ...
        if sc == "High Load":
            Q_base = np.array([32.0, 1.0, 15.0, 0.0, 0.0, 0.0]) # Heavy load CPU=32W, Payload=15W
        else:
            Q_base = np.array([18.0, 1.0, 5.0, 0.0, 0.0, 0.0])  # Nominal CPU=18W, Payload=5W
            
        eps_base = np.array([0.1, 0.1, 0.1, 0.2, 0.15, 0.1])    # Louvers start semi-closed (eps=0.15)
        ...
        for step_idx, t in enumerate(np.arange(0.0, duration, dt)):
            ...
            action, active_Q, active_eps = controller.decide_action(T_current, Q_solar, Q_base, eps_base)
            ...
            # Node 0: CPU
            T_current = controller.predict_horizon(T_current, dt, Q_solar, active_Q, active_eps)
            
            # Advance uncontrolled baseline system (standard parameters, Louver closed)
            T_uncontrolled = controller.predict_horizon(T_uncontrolled, dt, Q_solar, Q_base, eps_base)
            ...
            q_rad_ctrl = active_eps[4] * SIGMA * net.A[4] * ((T_current[4]+273.15)**4 - T_SPACE**4)
            ...
            time_series.append({
                ...
                "Action": action,
                "Throttled": 1 if action in ["CPU THROTTLING", "EMERGENCY SAFE-MODE"] else 0,
    ...
    with open(csv_path, "w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Time_min", "T_CPU_Ctrl", "T_CPU_Unctrl", "Action", "Louver_Eps"])
        for p in results_summary["High Load"]["series"]:
            writer.writerow([p["Time_min"], p["T_CPU_Ctrl"], p["T_CPU_Unctrl"], p["Action"], p["Louver_Eps"]])
    ...
    for idx, sc in enumerate(scenarios):
        ...
        ax.plot(times, t_unctrl, label="Uncontrolled CPU (Louver Closed)", color='#64748b', linestyle='--', linewidth=1.5)
        ax.plot(times, t_ctrl, label="Active Look-Ahead EKF Controller", color=colors[idx], linewidth=2.5)
        ax.axhline(85.0, color='red', linestyle=':', label='Silicon Critical (85°C)')
        
        # Mark actions
        for i, act in enumerate(actions):
            if act != "NOMINAL" and i % 5 == 0:
                ax.annotate(act[:6], (times[i], t_ctrl[i]), textcoords="offset points", xytext=(0,10),
                            arrowprops=dict(arrowstyle="->", color='white', lw=0.5), color='white', fontsize=7)
                
        ax.set_title(f"Scenario: {sc} (3 Orbits Operational)", color='white', fontsize=11)
...
"""
    ...
    report_content = report_content.replace("{NOM_UNCTRL}", f"{results_summary['Nominal']['uncontrolled_peak']}")
    report_content = report_content.replace("{NOM_CTRL}", f"{results_summary['Nominal']['controlled_peak']}")
    report_content = report_content.replace("{NOM_AVOID}", f"{results_summary['Nominal']['burnouts_avoided']}")
    report_content = report_content.replace("{NOM_THROT}", f"{results_summary['Nominal']['throttling_fraction']}")
    report_content = report_content.replace("{NOM_ENG}", f"{results_summary['Nominal']['energy_wh']}")
    
    report_content = report_content.replace("{HIGH_UNCTRL}", f"{results_summary['High Load']['uncontrolled_peak']}")
    report_content = report_content.replace("{HIGH_CTRL}", f"{results_summary['High Load']['controlled_peak']}")
    report_content = report_content.replace("{HIGH_AVOID}", f"{results_summary['High Load']['burnouts_avoided']}")
    report_content = report_content.replace("{HIGH_THROT}", f"{results_summary['High Load']['throttling_fraction']}")
    report_content = report_content.replace("{HIGH_ENG}", f"{results_summary['High Load']['energy_wh']}")
    
    report_content = report_content.replace("{ECL_UNCTRL}", f"{results_summary['Eclipse']['uncontrolled_peak']}")
    report_content = report_content.replace("{ECL_CTRL}", f"{results_summary['Eclipse']['controlled_peak']}")
    report_content = report_content.replace("{ECL_AVOID}", f"{results_summary['Eclipse']['burnouts_avoided']}")
    report_content = report_content.replace("{ECL_THROT}", f"{results_summary['Eclipse']['throttling_fraction']}")
    report_content = report_content.replace("{ECL_ENG}", f"{results_summary['Eclipse']['energy_wh']}")
    
    report_content = report_content.replace("{HIGH_GRAD}", f"{results_summary['High Load']['uncontrolled_peak'] - 85.0}")
