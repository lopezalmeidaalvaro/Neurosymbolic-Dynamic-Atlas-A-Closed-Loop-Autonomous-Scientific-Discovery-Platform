...
def train_pinn():
    ...
    for epoch in range(epochs):
        ...
        # 2. Physics Loss
        y_phys = pinn(X_phys_tensor) * 300.0  # Denormalize output for physical equations
        
        # Compute dT/dt using autograd
        dT_dnorm = torch.autograd.grad(
            y_phys, X_phys_tensor, 
        ...
        Q_rad = e_phys_val * stefan_boltzmann * a_phys_val * (y_phys**4 - ambient_temp_K**4)
        ...
        if (epoch + 1) % 500 == 0:
            print(f" -> Epoch {epoch+1}/{epochs} | Loss: {loss.item():.6f} (Data: {loss_data.item():.6f}, Physics: {loss_physics.item():.6f}, Energy: {loss_energy.item():.6f})")
    ...
    def closure():
        ...
        dT_dnorm = torch.autograd.grad(y_phys, X_phys_tensor, grad_outputs=torch.ones_like(y_phys), create_graph=True)[0]
        dT_dt = dT_dnorm[:, 0:1] / 3600.0
    ...
    tracker = ExperimentTracker(storage_path=os.path.join(str(config.ROOT_DIR), "physics", "artifacts", "experiments.db"))
    hyperparams = {
        "epochs": epochs,
