...
def plot_orbital_results(df, output_path):
    ...
    ax1.plot(times, df["Solar_Flux_W_m2"], label="Solar Flux", color="#e0f2fe", linewidth=1.8)
    ax1.plot(times, df["Albedo_Flux_W_m2"], label="Earth Albedo", color="#38bdf8", linewidth=1.5)
    ax1.plot(times, df["Earth_IR_Flux_W_m2"], label="Earth IR", color="#fb7185", linewidth=1.5)
    ax1.plot(times, df["Total_Flux_W_m2"], label="Total Env Flux", color="#26ffad", linewidth=2.0, linestyle="--")
    ...
    ax1.set_title("LEO Cubesat Orbital Environment Heat Fluxes", color='white', fontsize=12, pad=10)
    ...
    keys = ["T_CPU_C", "T_Battery_C", "T_Payload_C", "T_Structure_C", "T_Radiator_C", "T_Panels_C"]
    
    for idx, key in enumerate(keys):
        ax2.plot(times, df[key], label=nodes[idx], color=cols[idx], linewidth=2.0)
        
    ax2.set_title("Coupled Multi-Node Telemetry Temperature Curves", color='white', fontsize=12, pad=10)
