...
def simulate_constellation_parallel(n_sats=50, n_workers=4):
    ...
    for i in range(n_sats):
        ...
        config = {
            "C": [200.0 + np.random.normal(0, 10.0), 500.0, 300.0, 1000.0, 200.0, 300.0],
...
def benchmark_gpu_acceleration():
    ...
    if HAS_TORCH:
        ...
        gpu_device_name = torch.cuda.get_device_name(0) if device == "cuda" else "CPU Vectorized (PyTorch)"
    else:
...
def generate_hpc_report(parallel_time, seq_time, n_sats, vectorized_time, gpu_device, surrogate_latency_ms, n_cores):
    """
...
def main():
    ...
    results_par, parallel_time = simulate_constellation_parallel(n_sats=n_sats, n_workers=min(4, n_cores))
    ...
    print(f" -> Completed 1000 batch sweeps in {vectorized_time:.4f} seconds on device: {gpu_device}")
    ...
    print(f" -> Avg Inference Latency: {surrogate_latency_ms:.4f} ms (sub-10ms limit met: {surrogate_latency_ms < 10.0})")
