...
class SafetyVisitor(ast.NodeVisitor):
    ...
    def __init__(self):
        ...
        self.blacklisted_modules = {
            ...
            "http", "ftplib", "smtplib", "telnetlib", "pydoc", 
            "webbrowser", "ctypes"
        }
        
        # Blacklisted function names and attribute names
        self.blacklisted_calls = {
            "eval", "exec", "system", "popen", "spawn", "fork", 
            "kill", "rmdir", "remove", "unlink", "rmtree", "chdir", 
            "listdir", "walk"
        }
    ...
    def visit_Import(self, node):
        for name in node.names:
            ...
            if base_module in self.blacklisted_modules:
                self.is_safe = False
                self.warnings.append(f"Blocked import of dangerous module: '{name.name}'")
    ...
    def visit_ImportFrom(self, node):
        if node.module:
            ...
            if base_module in self.blacklisted_modules:
                self.is_safe = False
                self.warnings.append(f"Blocked import from dangerous module: '{node.module}'")
        self.generic_visit(node)
    ...
    def visit_Call(self, node):
        ...
        if isinstance(node.func, ast.Name):
            if node.func.id in self.blacklisted_calls:
                self.is_safe = False
                self.warnings.append(f"Blocked dangerous function call: '{node.func.id}()'")
        
        # Check method/attribute calls (e.g. os.system("cmd"))
        elif isinstance(node.func, ast.Attribute):
            if node.func.attr in self.blacklisted_calls:
                self.is_safe = False
                self.warnings.append(f"Blocked dangerous attribute/method call: '.{node.func.attr}()'")
...
class SandboxExecutor:
    ...
    def __init__(self, use_docker=True, timeout=120, memory_limit_mb=512):
        ...
        if self.use_docker:
            try:
                ...
                print(f"[WARNING] SandboxExecutor: Docker not accessible ({e}). Falling back to local Subprocess mode.")
    ...
    def execute(self, code, input_data=None):
        ...
        try:
            if self.use_docker and self.docker_available:
                ...
                return self._execute_subprocess(sandbox_dir, "experiment.py", start_time)
        finally:
    ...
    def _execute_subprocess(self, sandbox_dir, script_name, start_time):
        ...
        try:
            ...
            error_msg = None
            if not success:
                error_msg = f"Subprocess exited with non-zero return code: {proc.returncode}"
    ...
    def _execute_docker(self, sandbox_dir, script_name, start_time):
        ...
        for item in os.listdir(current_pwd):
            ...
            if os.path.isfile(item_path) and item.endswith(".py") and item != "setup_phase5.py":
        ...
        try:
            ...
            execution_time = time.time() - start_time
            success = (exit_code == 0)
            error_msg = None if success else f"Docker container exited with status code: {exit_code}"
