...
def main():
    t_start = time.time()
    print("=" * 80)
    print("🔬 PRINCIPAL COMPUTATIONAL PHYSICS AUDITOR — DEEP REPRESENTATION AUDIT (ResNet-1D)")
    print("=" * 80)
    ...
    if not os.path.exists(MODEL_PATH):
        raise FileNotFoundError(f"Trained ResNet-1D model state dict not found at: {MODEL_PATH}")
    ...
    print(f"  Extracted Deep Feature Shapes:")
    ...
    print(f"  Deep Representation Shift (D_emb = 1 - CKA):")
    print(f"    - Pairs A-C (Synthetic -> Clinical) : {d_emb_deep_AC:.6f}  (CKA: {cka_deep_AC:.6f})")
    print(f"    - Pairs B-C (Composite -> Clinical) : {d_emb_deep_BC:.6f}  (CKA: {cka_deep_BC:.6f})")
    ...
    print(f"  EV3 Representation Shift (D_emb = 1 - CKA):")
    print(f"    - Pairs A-C (Synthetic -> Clinical) : {d_emb_ev3_AC:.6f}  (CKA: {cka_ev3_AC:.6f})")
    print(f"    - Pairs B-C (Composite -> Clinical) : {d_emb_ev3_BC:.6f}  (CKA: {cka_ev3_BC:.6f})")
    ...
    print("=" * 80)
    print(f"  A-C (Synthetic -> Clinical) | Deep D_emb: {d_emb_deep_AC:.6f}  vs  EV3 D_emb: {d_emb_ev3_AC:.6f}")
    print(f"  B-C (Composite -> Clinical) | Deep D_emb: {d_emb_deep_BC:.6f}  vs  EV3 D_emb: {d_emb_ev3_BC:.6f}")
    print("  " + "─" * 76)
    print(f"  Deep Space Deformation Degradation: {report['comparison_analysis']['degradation_in_deep_space']}")
    print(f"  Deformation Change Magnitude (A-C): {report['comparison_analysis']['difference_D_emb_A_C']:.6f}")
    print(f"  Deformation Change Magnitude (B-C): {report['comparison_analysis']['difference_D_emb_B_C']:.6f}")
    print("=" * 80)
    print(f"📂 Compiled deep representation report successfully exported to: {REPORT_FILE}")
