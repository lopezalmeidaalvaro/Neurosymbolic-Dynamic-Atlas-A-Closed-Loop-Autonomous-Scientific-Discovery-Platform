...
def load_sqlite_history(db_path="runs/math_search.db"):
    ...
    """
    candidate_paths = [db_path, "brain/state.db", "runs/math_search.db", "../runs/math_search.db"]
    actual_path = None
    ...
    if not actual_path:
        print(f"  [SQLite BRIDGE] No SQLite database found in candidate paths. Skipping migration.")
        return {}
...
def migrate_to_neo4j(sqlite_data, kg: ScientificKnowledgeGraph):
    ...
    ev3_observables = [
        ("lyapunov_max", "Exponente de Lyapunov máximo", "DYNAMICAL_ESTIMATOR", "Mide la tasa de divergencia de órbitas adyacentes."),
        ("correlation_dimension", "Dimensión de correlación", "TOPOLOGICAL_ESTIMATOR", "Mide la dimensión fractal de correlación Grassberger-Procaccia."),
        ("recurrence_rate", "RQA Recurrence Rate", "RECURRENCE_ESTIMATOR", "Porcentaje de estados recurrentes en el espacio."),
        ("determinism", "RQA Determinism", "RECURRENCE_ESTIMATOR", "Porcentaje de puntos recurrentes diagonales."),
        ("l_max", "RQA Diagonal Line Max Length", "RECURRENCE_ESTIMATOR", "Longitud máxima de línea diagonal en matriz de recurrencia."),
        ("recurrence_entropy", "RQA Shannon Entropy", "RECURRENCE_ESTIMATOR", "Entropía de la distribución diagonal."),
        ("multiscale_entropy", "Entropía multiescala", "ENTROPY_ESTIMATOR", "Espectro de entropía de muestra a múltiples escalas de tiempo."),
        ("sample_entropy", "Entropía de muestra (Sample Entropy)", "ENTROPY_ESTIMATOR", "Mide la regularidad y complejidad de la serie temporal."),
        ("spectral_entropy", "Entropía espectral", "SPECTRAL_ESTIMATOR", "Entropía de Shannon aplicada a la densidad espectral de potencia."),
        ("dominant_frequency", "Frecuencia dominante", "SPECTRAL_ESTIMATOR", "La frecuencia con mayor amplitud en el espectro."),
        ("variance", "Varianza de la señal", "STATISTICAL_ESTIMATOR", "Dispersión de los valores de la serie temporal."),
        ("autocorr_decay", "Decaimiento de autocorrelación", "STATISTICAL_ESTIMATOR", "Tiempo para que la autocorrelación decaiga a 1/e."),
        ("kurtosis", "Curtosis robusta", "STATISTICAL_ESTIMATOR", "Medida del apuntamiento y colas de la distribución."),
        ("skewness", "Asimetría robusta", "STATISTICAL_ESTIMATOR", "Medida de la asimetría de la distribución."),
        ("energy", "Energía total de la señal", "STATISTICAL_ESTIMATOR", "Suma de los cuadrados de las amplitudes espectrales.")
    ]
    
    obs_count = 0
    for obs_id, name, type_val, desc in ev3_observables:
        kg.create_observable(f"obs_{obs_id}", name=name, type=type_val, description=desc)
        obs_count += 1
    ...
    for node in nodes:
        ...
        if parent_id is not None:
            ...
            kg._execute_write(
                ...
                parent_id=parent_exp_id, child_id=experiment_id
            )
        ...
        if node.get("redundancy_flag") == 1 and node.get("redundant_to_id") is not None:
            ...
            kg._execute_write(
                ...
                child_id=experiment_id, red_id=red_exp_id
            )
    ...
    for emb in embeddings:
        ...
        for obs_key, _, _, _ in ev3_observables:
            ...
            if val is not None:
                ...
                RETURN r
                """
                kg._execute_write(query, ex_id=experiment_id, obs_id=f"obs_{obs_key}", val=float(val), sys_name=sys_name)
                emb_migrated += 1
    ...
    for ins in insights:
        ...
        kg.create_hypothesis(hypothesis_id, text=text, confidence=confidence, state='validated')
        ins_migrated += 1
        ...
        try:
            ...
            for node_id in supp_ids:
                kg.relate_experiment_to_hypothesis(f"exp_node_{node_id}", hypothesis_id, outcome='VALIDATED')
        except Exception:
            pass
    ...
    for conj in conjectures:
        ...
        state = 'validated' if status == 'validated' else ('rejected' if status == 'rejected' else 'pending')
        kg.create_hypothesis(hypothesis_id, text=text, confidence=confidence, state=state)
        conj_migrated += 1
        
    print(f"  [MIGRATE] Created {ins_migrated + conj_migrated} validated heuristics & conjectures.")
    print("=" * 60 + "\n")
    return True
...
def run_migration():
    ...
    with ScientificKnowledgeGraph() as kg:
        if not kg.connected:
            print("  [SQLite BRIDGE] Neo4j is offline. Skipping database migration bridge.")
            return
