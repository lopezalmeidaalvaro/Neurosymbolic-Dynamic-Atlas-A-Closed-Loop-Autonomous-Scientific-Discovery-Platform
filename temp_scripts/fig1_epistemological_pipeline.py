...
w, h = 1.8, 0.8
cx_A, cy_A, l_A, r_A, b_A, t_A = draw_box(
    ax, 1, 4, w, h, r"\textbf{Domain A}" + "\n" + "Synthetic Chaos", COLORS["Synthetic"]
)
cx_B, cy_B, l_B, r_B, b_B, t_B = draw_box(
    ax,
    4.1,
    4,
    w,
    h,
    r"\textbf{Domain B}" + "\n" + "Composite Biophysical",
    COLORS["Biophysical"],
)
cx_C, cy_C, l_C, r_C, b_C, t_C = draw_box(
    ax, 7.2, 4, w, h, r"\textbf{Domain C}" + "\n" + "Clinical ECG", COLORS["Clinical"]
)
cx_N, cy_N, l_N, r_N, b_N, t_N = draw_box(
    ax, 4.1, 1.5, w, h, r"\textbf{Domain N}" + "\n" + "Null Control", COLORS["Null"]
)
...
def draw_arrow(ax, x1, y1, x2, y2, text, color="black", rad=0.0):
    ax.annotate(
        ...
        xytext=(x1, y1),
        arrowprops=dict(
            arrowstyle="->", color=color, lw=2, connectionstyle=f"arc3,rad={rad}"
        ),
        zorder=1,
    )
...
draw_arrow(
    ...
    cx_C,
    t_C,
    r"$S_3 = 0.26 \mid D_{emb} = 0.98 \mid D_{attr} = 0.76$",
    rad=-0.3,
)
draw_arrow(
    ax, cx_A + 0.4, b_A, l_N, cy_N + 0.2, r"$S_1^{null} = 0.47$", color=COLORS["Null"]
)
draw_arrow(
    ax, r_N, cy_N + 0.2, cx_C - 0.4, b_C, r"$S_2^{null} = 0.16$", color=COLORS["Null"]
)
...
ax.text(
    5,
    0.5,
    r"\textbf{Transition Audit:} $\Delta K = -0.051 \ (p = 0.168)$",
    ha="center",
    va="center",
    fontsize=12,
