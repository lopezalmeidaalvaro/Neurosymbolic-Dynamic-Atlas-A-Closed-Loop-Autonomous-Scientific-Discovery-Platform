...
fields = ['lyapunov_max', 'spectral_entropy', 'dominant_frequency',
          'variance', 'autocorr_decay', 'kurtosis', 'skewness', 'energy']

print(f"{'Field':<22} {'noise=0.0 (seed='+str(seed0)+')':<28} {'noise=1.0 (seed='+str(seed1)+')':<28} {'ISOLATED?'}")
print("-" * 90)
all_isolated = True
for f in fields:
    v0 = emb0.get(f, None)
    ...
    print(f"  {f:<20} {str(round(v0,6) if v0 else 'N/A'):<28} {str(round(v1,6) if v1 else 'N/A'):<28} {mark}")
...
print(f"  |diff|: {diff:.6f}  {'PASS - embeddings are isolated' if diff > 0 else 'FAIL - embeddings identical'}")
print()
if all_isolated:
    print("  RESULT: ALL fields differ between noise=0 and noise=1 [ISOLATION CONFIRMED]")
else:
