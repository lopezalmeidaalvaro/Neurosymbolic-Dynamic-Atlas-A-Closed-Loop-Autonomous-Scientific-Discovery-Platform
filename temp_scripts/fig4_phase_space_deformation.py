...
ax.scatter(pca_synth[:, 0], pca_synth[:, 1], alpha=0.5, s=20, 
           color=COLORS["Synthetic"], edgecolors='white', linewidth=0.5, label=r'Synthetic Chaos (Compact Topology)')
ax.scatter(pca_clin[:, 0], pca_clin[:, 1], alpha=0.5, s=20, 
           color=COLORS["Clinical"], edgecolors='white', linewidth=0.5, label=r'Clinical ECG (Scattered \& Rotated)')
...
ax.text(0.05, 0.95, r"\textbf{$D_{emb} = 0.982$}\\" "\n" r"(Topological Fracture)", 
