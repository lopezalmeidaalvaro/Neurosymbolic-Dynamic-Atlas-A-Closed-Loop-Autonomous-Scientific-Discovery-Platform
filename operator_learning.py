...
class DeepONet(torch.nn.Module):
    ...
    """
    def __init__(self, branch_input_dim, trunk_input_dim=1, hidden_dim=64, output_dim=10):
        super().__init__()
        torch.manual_seed(42)
...
def train_deeponet(model, u_train, y_train, G_train, epochs=1000, lr=0.001):
    ...
    for epoch in range(epochs):
        ...
        if (epoch + 1) % 200 == 0 or epoch == 0:
            print(f"[DeepONet] Epoch {epoch + 1:4d}/{epochs:4d} | Rel L2 Loss: {loss.item():.6f}")
            
    return model


def learn_ode_solution_operator(ode_system, param_range, n_samples=100, m=50, epochs=500):
    """
    ...
    for i in range(n_samples):
        ...
        if ode_system.lower() == "lorenz":
            ...
            sys_data = synthetic_systems.generate_lorenz(n_timesteps=n_points, dt=0.05, rho=p_val)
            traj = sys_data["x"]
        elif ode_system.lower() == "rossler" or ode_system.lower() == "rössler":
            sys_data = synthetic_systems.generate_rossler(n_timesteps=n_points, dt=0.05, c=p_val)
            traj = sys_data["x"]
        else:
            # Fallback Duffing
    ...
    model = DeepONet(branch_input_dim=m, trunk_input_dim=1, hidden_dim=64, output_dim=10).to(dtype=torch.get_default_dtype())
