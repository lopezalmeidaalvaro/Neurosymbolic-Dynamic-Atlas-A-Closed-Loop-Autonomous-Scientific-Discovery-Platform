...
def test_confidence_weighted_retrieval_memory():
    ...
    # Store patterns
    patterns = [
        {"pattern_id": "pat_high", "sequence": ["H", "CNOT"], "frequency": 12, "avg_score": 0.9, "representation": "H->CNOT"},
        {"pattern_id": "pat_toxic", "sequence": ["H(q0)", "CNOT(q0,q1)"], "frequency": 8, "avg_score": 0.5, "representation": "H(q0)->CNOT(q0,q1)"},
        {"pattern_id": "pat_noise", "sequence": ["X(q0)", "X(q0)"], "frequency": 4, "avg_score": 0.2, "representation": "X(q0)->X(q0)"},
        {"pattern_id": "pat_neutral", "sequence": ["CNOT", "H"], "frequency": 3, "avg_score": 0.6, "representation": "CNOT->H"}
    ]
    ...
    causal_records = [
        ...
        {"pattern": "H(q0)->CNOT(q0,q1)", "delta_score": -0.2, "survival_status": False}
    ]
    ...
    knowledge_graph = {
        "nodes": {
            "pattern_pat_high": {"type": "Pattern", "attributes": {"sequence": ["H", "CNOT"], "type": "repeated_subsequence"}},
            "pattern_pat_toxic": {"type": "Pattern", "attributes": {"sequence": ["H(q0)", "CNOT(q0,q1)"], "type": "entanglement_motif"}},
            "pattern_pat_noise": {"type": "Pattern", "attributes": {"sequence": ["X(q0)", "X(q0)"], "type": "repeated_subsequence"}},
            "pattern_pat_neutral": {"type": "Pattern", "attributes": {"sequence": ["CNOT", "H"], "type": "repeated_subsequence"}},
            "circ_1": {"type": "Circuit", "attributes": {"raw": True, "score": 0.98, "fidelity": 1.0}}
        },
        "edges": [
            {"source": "circ_1", "target": "pattern_pat_high", "type": "contains_pattern"}
        ]
    }
    ...
    # weight = 1.07918
    pat_high_weight = next(p["weight"] for p in active if p["representation"] == "H->CNOT")
    assert pytest.approx(pat_high_weight, rel=1e-4) == (1.0 * 1.0 * (1.0 + math.log10(1.2)))
    ...
    pat_neutral_weight = next(p["weight"] for p in active if p["representation"] == "CNOT->H")
    assert pytest.approx(pat_neutral_weight, rel=1e-4) == 3e-5

def test_evolution_engine_kdi_calculation():
    population_manager = QuantumPopulationManager(qubits=2, population_size=5, max_gates=8, seed=10)
    memory = QuantumMemory()
    ...
    engine.pending_injections_this_gen = [
        {"pattern": "H->CNOT", "child_hash": "h1", "generation": 0, "discarded_in_loop": False},
        {"pattern": "H->CNOT", "child_hash": "h2", "generation": 0, "discarded_in_loop": False},
        {"pattern": "X->H", "child_hash": "h3", "generation": 0, "discarded_in_loop": False},
        {"pattern": "X->H", "child_hash": "h4", "generation": 0, "discarded_in_loop": True} # Discarded, shouldn't count in KDI
    ]
