...
class MockSandbox(BaseSandbox):
    def execute(self, code, input_data=None):
        return {"success": True, "result": {"output": "Mock success"}, "execution_time": 0.05}
...
class MockLLMReasoner:
    def generate_hypothesis(self, context):
        return {"hypothesis_text": "Mock LLM", "prediction": "Mock LLM Pred", "confidence_prior": 0.8}
    def design_experiment(self, hypothesis, data, methods):
        return {"experiment_description": "Mock LLM Exp", "dataset": "synthetic_lorenz", "method": "koopman", "falsification_criterion": "test", "python_code": "print('ok')"}
    def interpret_results(self, hypothesis, experiment, results):
        return {"verdict": "validated", "confidence_posterior": 0.9, "reasoning": "Mock LLM Interpretation"}
...
def test_orchestrator_agnostic_imports():
    ...
    assert "from physics.agents.hypothesis_generator import HypothesisGenerator" not in source
