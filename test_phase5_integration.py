...
def main():
    ...
    try:
        ...
        if not openai_key and not anthropic_key:
            print_result("TEST 1 - LLM Connection", "SKIP", "No API key configured in environment variables.")
            test_results["TEST 1"] = "SKIP"
        else:
            # We have keys configured, make a simple live call
            print("  Connecting to live LLM for Test 1...")
            response = reasoner.query("Eres un asistente. Responde exactamente con la palabra 'OK' y nada más.", "Di 'OK'")
            if "OK" in response:
                print_result("TEST 1 - LLM Connection", "PASS", "Live API connection established successfully.")
                test_results["TEST 1"] = "PASS"
            else:
                print_result("TEST 1 - LLM Connection", "FAIL", f"Unexpected LLM response: {response}")
    ...
    try:
        ...
        if res_safe["success"] and res_safe["result"] == {"result": 42} and not is_safe:
            print_result("TEST 2 - Sandbox Safety", "PASS", "Safe code executed; dangerous code blocked successfully.")
    ...
    try:
        ...
        if not missing_keys:
            print_result("TEST 3 - Hypothesis Generation", "PASS", f"Hypothesis generated: '{hypothesis['hypothesis_text'][:50]}...'")
            test_results["TEST 3"] = "PASS"
        else:
            print_result("TEST 3 - Hypothesis Generation", "FAIL", f"Missing keys: {missing_keys}")
    ...
    try:
        ...
        experiment = reasoner.design_experiment(mock_hypothesis, available_data, available_methods)
        ...
        if not missing_keys and has_code:
            print_result("TEST 4 - Experiment Design", "PASS", f"Experiment designed with Python code string ({len(experiment['python_code'])} chars).")
            test_results["TEST 4"] = "PASS"
        else:
            print_result("TEST 4 - Experiment Design", "FAIL", f"Missing keys: {missing_keys}, Code valid: {has_code}")
    ...
    try:
        ...
        if res["iterations"] == 1 and len(res["session_history"]) == 1:
            ...
            print_result("TEST 5 - End-to-end mini cycle", "PASS", f"Cycle completed successfully. Verdict: {verdict.upper()} (Epistemic Gain: {gain:.4f})")
            test_results["TEST 5"] = "PASS"
        else:
            print_result("TEST 5 - End-to-end mini cycle", "FAIL", f"Iterations count: {res['iterations']}")
    ...
    try:
        ...
        if hyp_count == 1 and exp_count == 1:
            details = f"Logged 1 Hypothesis and 1 Experiment to local SQLite DB."
            if neo4j_logged:
                details += " Also tracked in live Neo4j."
            print_result("TEST 6 - Knowledge Graph Logging", "PASS", details)
            test_results["TEST 6"] = "PASS"
        else:
            print_result("TEST 6 - Knowledge Graph Logging", "FAIL", f"Hypotheses stored: {hyp_count}, Experiments stored: {exp_count}")
