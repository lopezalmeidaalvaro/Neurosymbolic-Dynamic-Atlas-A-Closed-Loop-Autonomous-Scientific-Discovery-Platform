...
class AutonomousScientist:
    ...
    def __init__(self, llm_provider="openai", use_docker=True, knowledge_graph=None):
        ...
        if self.kg:
            ...
            try:
                ...
                self.kg = ScientificKnowledgeGraph(uri="bolt://localhost:7687", user="neo4j", password="password")
                # Try a quick test write or schemas to check online status
                self.kg.initialize_schema()
                self.kg_active = True
                print("[INFO] AutonomousScientist: Successfully connected to Neo4j Knowledge Graph.")
            except Exception as e:
                print(f"[WARNING] AutonomousScientist: Neo4j connection failed ({e}). Falling back to local SQLite database.")
                self.kg = None
                self._init_local_db()
    ...
    def compute_epistemic_gain(self, hypothesis_before, hypothesis_after, results):
        ...
        # Shannon Entropy
        entropy_prior = - (p_prior * math.log2(p_prior) + (1.0 - p_prior) * math.log2(1.0 - p_prior))
        entropy_post = - (p_post * math.log2(p_post) + (1.0 - p_post) * math.log2(1.0 - p_post))
        ...
        previous_texts = [item["hypothesis"]["hypothesis_text"] for item in self.session_history]
        if previous_texts:
            jaccard_distances = []
    ...
    def execute_experiment(self, experiment_design):
        ...
        print(f"[WARNING] Experiment execution failed. Initiating LLM code self-correction...")
        for attempt in range(2):
            ...
            try:
                corrected_design = self.llm._query_json(
                    system_prompt, 
                    user_prompt, 
                    ["experiment_description", "dataset", "method", "metrics", "falsification_criterion", "python_code"]
                )
                ...
                execution_res = self.sandbox.execute(python_code, input_data=input_data)
                if execution_res["success"]:
                    print(f"  [SUCCESS] LLM self-correction successfully resolved execution error on attempt {attempt + 1}!")
        ...
        if self.kg_active and self.kg:
            try:
                ...
                if len(self.session_history) > 0:
                    prev_hyp_id = f"hyp_{int(time.time()) - 100}"  # dummy lookup approximation
                    # We can link it if we tracked actual IDs
                    pass
                print(f"[INFO] Saved hypothesis and experiment to Neo4j database.")
                return
            except Exception as e:
                print(f"[WARNING] Failed to write to Neo4j ({e}). Falling back to local SQLite.")
        ...
        try:
            ...
            cursor.execute(
                ...
                (
                    ...
                    float(results.get("confidence_posterior", 0.5)) if isinstance(results, dict) else 0.5,
                    verdict
                )
            )
            ...
            cursor.execute(
                ...
                (
                    ...
                    1 if results else 0,
                    float(results.get("execution_time", 0.0)) if isinstance(results, dict) else 0.0
                )
            )
            ...
            conn.commit()
            conn.close()
            print(f"[INFO] Saved hypothesis and experiment to local SQLite database.")
        except Exception as e:
            print(f"[ERROR] Failed to save scientific data to local SQLite database: {e}")
    ...
    def run_discovery_cycle(self, domain, goal, max_iterations=3, patience=2):
        ...
        """
        print("\n" + "=" * 70)
        print(f"STARTING AUTONOMOUS SCIENTIFIC DISCOVERY LOOP")
        print(f"Goal: {goal}")
        print("=" * 70)
        ...
        for iteration in range(max_iterations):
            ...
            try:
                experiment = self.llm.design_experiment(hypothesis, context["available_data"], context["available_methods"])
            except Exception as e:
            ...
            if not execution_res["success"]:
                print(f"  [ERROR] Experiment execution failed permanently: {execution_res['error']}")
                continue
                
            print(f"  Execution completed in {execution_res['execution_time']:.2f} seconds.")
            print(f"  Raw Results: {json.dumps(execution_res['result'], indent=2)}")
            
            # 5. Interpret Results
            print("  Interpreting results...")
            try:
                interpretation = self.llm.interpret_results(hypothesis, experiment, execution_res["result"])
            except Exception as e:
                print(f"  [ERROR] Result interpretation failed: {e}")
                continue
                
            verdict = interpretation["verdict"]
            print(f"  Verdict: {verdict.upper()} (Posterior Confidence: {interpretation['confidence_posterior']})")
            print(f"  Reasoning: {interpretation['reasoning']}")
            
            # 6. Calculate Epistemic Gain
            gain = self.compute_epistemic_gain(hypothesis, interpretation, execution_res["result"])
            ...
            self.update_knowledge_graph(hypothesis, experiment, execution_res["result"], verdict)
    ...
    def interactive_mode(self, hypothesis):
        ...
        print("-" * 50)
        print("Options: [Enter] to Approve, 'auto' to enable autonomous mode, 'abort' to terminate.")
        ...
        if not sys.stdin.isatty():
            print("[INFO] Non-TTY environment detected. Automatically approving hypothesis.")
            return "approve"
        ...
        try:
            ...
            if choice == "auto":
                self.auto_mode = True
                print("Autonomous mode enabled. Scientist will now proceed without pausing.")
                return "approve"
            elif choice == "abort":
                return "abort"
            else:
    ...
    def generate_discovery_report(self, output_path="artifacts/discovery_report.md"):
        ...
        validated = sum(1 for item in self.session_history if item["interpretation"]["verdict"] == "validated")
        rejected = sum(1 for item in self.session_history if item["interpretation"]["verdict"] == "rejected")
        inconclusive = sum(1 for item in self.session_history if item["interpretation"]["verdict"] == "inconclusive")
