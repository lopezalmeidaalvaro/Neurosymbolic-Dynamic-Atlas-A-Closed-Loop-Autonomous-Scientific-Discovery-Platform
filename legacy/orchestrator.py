...
def run_branch(name, script, cert_file):
    ...
    try:
        ...
        elapsed = round(time.time() - t0, 4)
        return _fail_cert(name, "TIMEOUT", ["Timeout global (30s) en orquestador"], elapsed)
...
def infer_conclusions(merged):
    ...
    if b3:
        ...
        if b3.get("bisection_results"):
            conclusions["real_root_approx"] = b3["bisection_results"][0]["midpoint_float64"]

    b2 = merged.get("B2_bring_jerrard")
    if b2:
        conclusions["bring_jerrard"] = "SUCCESS"
    else:
        conclusions["bring_jerrard"] = "FAILED/TIMEOUT - Inferencia basada solo en B1 y B3"

    return conclusions

def main():
    print("\n" + "="*60)
    print("  ORQUESTADOR MATEMATICO (STRESS TEST) -- x^5 + 3x^4 - 2x^3 + 7x^2 - x + 1 = 0")
    print("="*60)
