...
def heat_kernel_trace_numeric(D2_sym, p0, p1, m, s_values, p0_val, p1_val, m_val):
    ...
    if negative_eigs:
        ...
        print(
            "=> Positividad espectral violada. Unitariedad de S-matrix: COMPROMETIDA\n"
        )
    else:
...
def derive_effective_hamiltonian(traces, s_values):
    ...
    for s, tr in traces:
        if tr.real > 0:
            ...
            print(
                f"  s={s:.2e}: Traza no-positiva ({tr:.4f}) => ln indefinido => ANOMALIA UNITARIA"
            )
...
def analyze_unitarity(eigenvalues, s_ref=1.0):
    ...
    if abs(U) < 1e-6:
        ...
        verdict = (
            "[VIOLACION]: Over-completeness => nuevos estados fantasma (ghost states)"
        )
    else:
