...
def compute_cka_degradation(X_clean, X_noisy_dict):
    ...
    X_clean_orig = np.nan_to_num(np.array([extract_ev3_features(x, extended=False) for x in X_clean]), nan=0.0)
    X_clean_ext = np.nan_to_num(np.array([extract_ev3_features(x, extended=True) for x in X_clean]), nan=0.0)
    ...
    for snr in snrs:
        ...
        X_noisy_orig = np.nan_to_num(np.array([extract_ev3_features(x, extended=False) for x in noisy_signals]), nan=0.0)
        X_noisy_ext = np.nan_to_num(np.array([extract_ev3_features(x, extended=True) for x in noisy_signals]), nan=0.0)
        ...
        records.append({
            "snr": snr,
            "cka_original": cka_orig,
            "cka_extended": cka_ext
        })
        
    return pd.DataFrame(records)

def compute_shap_stability(model, X_clean_features, X_noisy_features_dict, feature_names):
    """
    ...
    try:
        ...
        if isinstance(shap_clean, list):
            shap_clean_imp = np.mean([np.mean(np.abs(s), axis=0) for s in shap_clean], axis=0)
        else:
    ...
    for snr, X_noisy_feat in X_noisy_features_dict.items():
        try:
            ...
            if isinstance(shap_noisy, list):
                shap_noisy_imp = np.mean([np.mean(np.abs(s), axis=0) for s in shap_noisy], axis=0)
            else:
...
def run_full_robustness_study(signal_generator_func, n_signals=50):
    ...
    for snr in snr_levels:
        X_noisy_orig_dict[snr] = np.nan_to_num(np.array(X_noisy_orig_dict[snr]), nan=0.0)
    ...
    df_shap_orig = compute_shap_stability(model_orig, X_clean_orig, X_noisy_orig_dict, orig_names)
    df_shap_ext = compute_shap_stability(model_ext, X_clean_ext, X_noisy_ext_dict, ext_names)
...
def plot_degradation_curves(results_df, output_path="figures/robustness_degradation.pdf"):
    """
    ...
    # Panel A: CKA
    ax1.plot(snrs, results_df["cka_original"], 'o-', color='#3b82f6', label='EV3 Original (8D)', linewidth=2.5, markersize=7)
    ax1.plot(snrs, results_df["cka_extended"], 's-', color='#8b5cf6', label='EV3 Extended (15D)', linewidth=2.5, markersize=7)
    ax1.set_xlabel("SNR (dB)", fontsize=12)
    ax1.set_ylabel("Centered Kernel Alignment (CKA)", fontsize=12)
    ax1.set_title("(a) Latent Space Alignment Degradation", fontsize=13, fontweight='bold', pad=12)
    ax1.grid(True, linestyle='--', alpha=0.6)
    ax1.legend(fontsize=10, loc='lower right')
    
    # Panel B: Spearman
    ax2.plot(snrs, results_df["shap_stability_original"], 'o-', color='#10b981', label='EV3 Original (8D)', linewidth=2.5, markersize=7)
    ax2.plot(snrs, results_df["shap_stability_extended"], 's-', color='#ec4899', label='EV3 Extended (15D)', linewidth=2.5, markersize=7)
    ax2.set_xlabel("SNR (dB)", fontsize=12)
    ax2.set_ylabel("Spearman Rank Correlation", fontsize=12)
    ax2.set_title("(b) SHAP Feature Importance Stability", fontsize=13, fontweight='bold', pad=12)
...
def export_robustness_results(results_df, output_path="artifacts/robustness_results.json"):
    """
...
if __name__ == "__main__":
    ...
    plot_degradation_curves(df_results, output_path="figures/robustness_degradation.pdf")
    export_robustness_results(df_results, output_path="artifacts/robustness_results.json")
    print("=" * 60)
