...
def build_physical_dataset_custom(
    seeds: list[int], noise: float = 0.0, standardize_before_embedding: bool = False
) -> dict[str, np.ndarray]:
    dataset = {}
...
def main():
                ...
                grid_results.append(
                    {
                        ...
                        "passed_all": sil >= THRES_SIL
                        and trust >= THRES_TRUST
                        and dcor >= THRES_DCOR,
                    }
                )
                print(
                    f"Neighbors={nn:<2} | Dist={md:<3} | Sil={sil:.4f} | Trust={trust:.4f} | DCor={dcor:.4f} | PassedAll={grid_results[-1]['passed_all']}"
                )
    ...
    print(
        f"  Silhouette: mean = {mean_sil:.6f}, std = {std_sil:.6f}, CV = {cv_sil * 100:.2f}%"
    )
    print(
        f"  Trustworthiness: mean = {mean_trust:.6f}, std = {std_trust:.6f}, CV = {cv_trust * 100:.2f}%"
    )
    print(
        f"  Distance Corr: mean = {mean_dcor:.6f}, std = {std_dcor:.6f}, CV = {cv_dcor * 100:.2f}%"
    )

    passed_test1 = (
        mean_sil >= THRES_SIL
        and mean_trust >= THRES_TRUST
        and mean_dcor >= THRES_DCOR
        and cv_sil < 0.10
        and cv_trust < 0.10
        and cv_dcor < 0.10
    )
    ...
    for seed in seeds:
        try:
            ...
            print(
                f"Seed={seed:<3} | Sil={sil:.4f} | Trust={trust:.4f} | DCor={dcor:.4f}"
            )
        except Exception as e:
    ...
    mean_seed_trust, std_seed_trust = float(np.mean(seed_trusts)), float(
        np.std(seed_trusts)
    )
    mean_seed_dcor, std_seed_dcor = float(np.mean(seed_dcors)), float(
        np.std(seed_dcors)
    )
    ...
    max_min_var_sil = (
        (max(seed_sils) - min(seed_sils)) / mean_seed_sil if mean_seed_sil != 0 else 0.0
    )
    max_min_var_trust = (
        (max(seed_trusts) - min(seed_trusts)) / mean_seed_trust
        if mean_seed_trust != 0
        else 0.0
    )
    max_min_var_dcor = (
        (max(seed_dcors) - min(seed_dcors)) / mean_seed_dcor
        if mean_seed_dcor != 0
        else 0.0
    )
    ...
    print(
        f"  Silhouette: CV = {cv_seed_sil * 100:.2f}%, Max-Min Relative Var = {max_min_var_sil * 100:.2f}%"
    )
    print(
        f"  Trustworthiness: CV = {cv_seed_trust * 100:.2f}%, Max-Min Relative Var = {max_min_var_trust * 100:.2f}%"
    )
    print(
        f"  Distance Corr: CV = {cv_seed_dcor * 100:.2f}%, Max-Min Relative Var = {max_min_var_dcor * 100:.2f}%"
    )

    passed_test2 = cv_seed_sil < 0.05 and cv_seed_trust < 0.05 and cv_seed_dcor < 0.05
    ...
    for i in range(10):
        ...
        try:
            ...
            print(
                f"Dataset {i+1} | Sil={sil:.4f} | Trust={trust:.4f} | DCor={dcor:.4f}"
            )
        except Exception as e:
    ...
    passed_test3 = (
        ci_sil_lower >= THRES_SIL
        and ci_trust_lower >= THRES_TRUST
        and ci_dcor_lower >= THRES_DCOR
    )
    ...
    if opt_key not in embeddings_by_config:
        print(
            "[WARN] Optimal configuration embedding not found in Test 1 grid. Re-running optimal configuration..."
        )
    ...
    for key, x_umap_nei in embeddings_by_config.items():
        ...
        try:
            ...
            composite_persistence = float(
                (dist_corr + procrustes_sim + nn_overlap) / 3.0
            )
            ...
            print(
                f"Neighbor {key:<12} | DistCorr={dist_corr:.4f} | ProcrustesSim={procrustes_sim:.4f} | NN_Overlap={nn_overlap:.4f} | Composite={composite_persistence:.4f}"
            )
        except Exception as e:
    ...
    mean_dist_corr = float(
        np.mean([item["distance_correlation"] for item in persistence_results])
    )
    mean_procrustes = float(
        np.mean([item["procrustes_similarity"] for item in persistence_results])
    )
    mean_nn_overlap = float(
        np.mean([item["nearest_neighbor_overlap"] for item in persistence_results])
    )
    mean_composite_persistence = float(
        np.mean([item["composite_persistence"] for item in persistence_results])
    )
    ...
    for idx, r_conf in enumerate(ranked_configs[:5]):
        print(
            f"  {idx+1}. n_neighbors={r_conf['n_neighbors']}, min_dist={r_conf['min_dist']} "
    ...
    if all_passed:
        ...
        if not passed_test3:
            print(
                "  - Test 3 (Res resampling CI lower bound fell below target thresholds)"
            )
        if not passed_test4:
