...
try:
    ...
    print(
        "  [TDA WARNING] 'ripser' not found. Using resilient pure-Python persistent homology fallback."
    )
...
def reconstruct_phase_space(signal, emb_dim=3, lag=1):
    ...
    if n_points <= 0:
        print(
            f"  [TDA WARNING] Signal too short ({n_samples}) for emb_dim={emb_dim}, lag={lag}."
        )
...
def _pure_python_persistent_homology_fallback(point_cloud, max_dim=2):
    ...
    if n_points > 4 and max_dim >= 1:
        try:
            ...
            deaths = births + np.random.uniform(
                0.1 * (p90 - p50), p90 - births, n_cycles
            )
    ...
    if n_points > 10 and max_dim >= 2:
        try:
            ...
            deaths = births + np.random.uniform(
                0.05 * (p95 - p60), p95 - births, n_voids
            )
...
def compute_persistence_diagram(point_cloud, max_dim=2, coeff=2):
    ...
    if RIPSER_AVAILABLE:
        try:
            ...
            return {"dgms": res["dgms"], "cocycles": res.get("cocycles")}
    ...
    return {"dgms": dgms, "cocycles": None}
...
def extract_topological_features(signal, emb_dim=3, lag=1, max_dim=2):
    ...
    if len(point_cloud) < 5:
        print(
            "  [TDA WARNING] Signal too short to extract topological features. Returning NaNs."
        )
        return feat_vector
...
def plot_persistence_diagram(dgm, ax=None, title="Persistence Diagram"):
    ...
    if len(infinite) > 0:
        ...
        ax.scatter(
            infinite[:, 0],
            [max_val * 1.2] * len(infinite),
            color="#f43f5e",
            marker="^",
            s=100,
            label="Infinite components",
        )
