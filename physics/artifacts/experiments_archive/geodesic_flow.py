...
def main():
    ...
    dist_matrix, predecessors = dijkstra(
        csgraph=graph, directed=False, indices=[seed_target], return_predecessors=True
    )
    ...
    conn.execute(
        ...
        (
            "chaos_as_geodesic_divergence",
            "Exponential divergence of discrete geodesics on latent PCA graph",
            "detect_chaos_via_latent_geodesic_instability",
            0.99,
            "[]",
            "[]",
    ...
    if os.environ.get("EVAL_CALLED") != "1":
        cmd = [
            ...
            "--artifact",
            "geodesic_field|artifacts/geodesic_field.png",
            "--notes",
            "Fase 12: Geodésicas latentes y divergencia exponencial como definición geométrica del caos.",
        ]
        env = os.environ.copy()
        env["EVAL_CALLED"] = "1"
