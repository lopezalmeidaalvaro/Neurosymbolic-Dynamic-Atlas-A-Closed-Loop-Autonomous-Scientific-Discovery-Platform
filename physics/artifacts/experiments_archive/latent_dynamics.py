...
def main():
    ...
    print(
        f"  Puntos de inestabilidad kinemática (||a|| >> ||v||): {np.sum(instability_mask)} / {total_points}"
    )
    ...
    conn.execute(
        ...
        (
            "physics_of_embeddings",
            "Continuous predictability and definable Jacobian across latent space deformations",
            "model_dynamics_via_latent_vector_fields",
            0.99,
            "[]",
            "[]",
            "['latent_physics']",
    ...
    if os.environ.get("EVAL_CALLED") != "1":
        cmd = [
            ...
            "--artifact",
            "latent_physics|artifacts/latent_dynamics_data.json",
            "--notes",
            "Fase 14: Dinámica diferencial del espacio latente y modelado predictivo del flujo continuo de embeddings.",
        ]
        env = os.environ.copy()
        env["EVAL_CALLED"] = "1"
