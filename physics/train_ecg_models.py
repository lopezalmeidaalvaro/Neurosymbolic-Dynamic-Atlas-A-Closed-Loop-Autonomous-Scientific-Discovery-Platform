...
def main():
    ...
    for name, model in models.items():
        ...
        torch.save(model.state_dict(), f"models/{name}_base.pth")
        print(f"Saved models/{name}_base.pth")

        print(f"\n--- Fine-tuning {name.upper()} on Shifted Dataset ---")
        # Fine-tune
        train_model(model, ft_loader, epochs=5, lr=0.002)
        # Save fine-tuned checkpoint
        torch.save(model.state_dict(), f"models/{name}_ft.pth")
        print(f"Saved models/{name}_ft.pth")

    print("\n✅ Successfully trained all models and saved models.")
    print("=" * 60)
