...
def run_tests():
    ...
    if not online:
        ...
        try:
            ...
            kg.create_hypothesis(
                "test_hyp_01",
                "La velocidad caótica de Lorenz depende de la curvatura local.",
                0.85,
                "pending",
            )
            kg.create_equation(
            ...
            kg.create_experiment(
                "test_exp_01",
                "Verificación de curvatura de Lorenz",
                "UCR_Lorenz",
                "DBSCAN",
            )
            kg.create_observable(
                ...
                "curvatura_promedio",
                "GEOMETRIC",
                "Curvatura geodésica local promedio.",
            )

            # Relate nodes
            kg.relate_hypothesis_to_equation("test_hyp_01", "test_eq_01", "DERIVES")
            kg.relate_experiment_to_hypothesis(
                "test_exp_01", "test_hyp_01", "VALIDATED"
            )
            kg.relate_equation_to_observable("test_eq_01", "test_obs_01", "OUTPUT")

            # Verify nodes exist via Cypher queries
            check_hyp = kg._execute_read(
                "MATCH (h:Hypothesis {id: 'test_hyp_01'}) RETURN h"
            )
            check_rel = kg._execute_read(
                "MATCH (ex:Experiment {id: 'test_exp_01'})-[r:EVALUATES]->(h:Hypothesis {id: 'test_hyp_01'}) RETURN r.outcome AS outcome"
            )

            assert len(check_hyp) == 1, "Hypothesis node was not found."
            assert (
                len(check_rel) == 1 and check_rel[0]["outcome"] == "VALIDATED"
            ), "EVALUATES relationship was not matched."

            print(
                "  Test 1 PASSED: Successfully created and verified graph nodes/relationships."
            )
    ...
    if not online:
        ...
        try:
            ...
            evaluation_result = {"match": True, "jaccard_terms": 0.88}

            kg.log_discovery_result(
                "test_lorenz", "sindy", discovered_eqs, ground_truth, evaluation_result
            )

            # Verify creations
            check_h = kg._execute_read(
                "MATCH (h:Hypothesis {id: 'hyp_test_lorenz_sindy'}) RETURN h"
            )
            check_ex = kg._execute_read(
                "MATCH (ex:Experiment {id: 'exp_test_lorenz_sindy'}) RETURN ex"
            )
            check_eq = kg._execute_read(
                "MATCH (e:Equation) WHERE e.id STARTS WITH 'eq_test_lorenz_sindy_' RETURN e"
            )

            assert (
                len(check_h) == 1
            ), "Hypothesis f'hyp_test_lorenz_sindy' was not logged."
            assert (
                len(check_ex) == 1
            ), "Experiment f'exp_test_lorenz_sindy' was not logged."
            assert (
                len(check_eq) == 3
            ), f"Expected 3 Equation nodes, matched {len(check_eq)}."

            print(
                "  Test 2 PASSED: Discovery results logged into Graph DBMS correctly."
            )
    ...
    if not online:
        ...
        try:
            ...
            print(
                f"  Found {len(validated_hyps)} validated hypotheses: {[h['id'] for h in validated_hyps]}"
            )
            assert (
                len(validated_hyps) >= 1
            ), "Expected at least 1 validated hypothesis node."
    ...
    try:
        ...
        assert (
            "Report" in content or "Graph" in content
        ), "Unexpected Markdown report contents."
        if online:
            assert "Summary" in content, "Missing summary statistics in online report."
            assert (
                "Hypothesis" in content or "Hipótesis" in content
            ), "Missing Hypotheses header."
    ...
    try:
        ...
        if not sqlite_data:
            print(
                "  [SQLite BRIDGE] SQLite database was empty or not found. Normal (simulating load success)."
            )
                ...
                "nodes": [
                    {
                        ...
                        "semantic_notes": "Lorenz synthetic run",
                    }
                ],
        ...
        if online:
            ...
            kg._execute_write(
                "MATCH (n) WHERE n.id CONTAINS 'exp_node_999' OR n.id CONTAINS 'test_' DETACH DELETE n"
            )
            ...
            # Verify creations
            check_exp = kg._execute_read(
                "MATCH (ex:Experiment {id: 'exp_node_999'}) RETURN ex"
            )
            assert (
                len(check_exp) == 1
            ), "Migrated experiment node exp_node_999 was not found."
            print(
                "  Test 6 PASSED: SQLite historical records successfully bridged to Neo4j Graph DBMS."
            )
        else:
            print(
                "  Test 6 PASSED: SQLite loading verified successfully (Migration bypassed due to offline Neo4j)."
            )
    ...
    if online:
        ...
        kg._execute_write(
            "MATCH (n) WHERE n.id STARTS WITH 'test_' OR n.id CONTAINS 'test_lorenz' OR n.id CONTAINS '999' DETACH DELETE n"
        )
