...
def solve_ode_with_pinn(ode_system, t_domain, initial_conditions, params, epochs=3000):
    ...
    if isinstance(initial_conditions, dict):
        ic_vals = [initial_conditions.get('x', 0.0), initial_conditions.get('y', 0.0), initial_conditions.get('z', 0.0)][:num_components]
    else:
    ...
    if epochs >= 200:
        ...
        try:
            ...
            print(f"[WARNING] L-BFGS fine-tuning failed: {e}. Falling back to Adam weights.")
    else:
        print("[PINN] Bypassing L-BFGS fine-tuning due to short budget (epochs < 200)...")
...
def discover_parameters_with_pinn(ode_system, observed_data, t_observed, variable_params, knowledge_graph=None, epochs=2000):
    """
    ...
    for param in variable_params:
        ...
        val = 1.0
        if param == "sigma": val = 5.0
        elif param == "rho": val = 20.0
        elif param == "beta": val = 2.0
        elif param == "a": val = 0.1
        elif param == "c": val = 3.0
    ...
    t_observed_2d = t_observed.reshape(-1, 1) if len(t_observed.shape) == 1 else t_observed
    observe_constraints = []
    
    for idx in range(num_components):
        bc = dde.icbc.PointSetBC(t_observed_2d, observed_data[:, idx:idx+1], component=idx)
    ...
    print(f"[PINN Inverse] Estimating physical parameters {variable_params} using Adam...")
    model.compile("adam", lr=0.001, external_trainable_variables=ext_vars)
    model.train(iterations=epochs)
    
    if epochs >= 200:
        print(f"[PINN Inverse] Fine-tuning estimates using L-BFGS...")
        try:
            model.compile("L-BFGS", external_trainable_variables=ext_vars)
            # Limit L-BFGS to a few outer iterations to ensure lightning fast execution under test/short-budget runs
            model.train(iterations=min(epochs, 10))
        except Exception as e:
            print(f"[WARNING] L-BFGS fine-tuning in parameter discovery failed: {e}. Falling back to Adam weights.")
    else:
        print("[PINN Inverse] Bypassing L-BFGS fine-tuning due to short budget (epochs < 200)...")
    ...
    if knowledge_graph:
        try:
            ...
            print("[INFO] Discovered parameters logged to ScientificKnowledgeGraph successfully.")
        except Exception as e:
