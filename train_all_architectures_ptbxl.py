...
def main():
    ...
    os.makedirs("checkpoints/ptbxl", exist_ok=True)
    ...
    for name, model in models.items():
        ...
        torch.save(model.state_dict(), f"checkpoints/ptbxl/{name}_base.pth")
        print(f"Saved checkpoints/ptbxl/{name}_base.pth")

        print(f"\n--- Fine-tuning {name.upper()} (Domain Shifted) ---")
        train_model(model, ft_loader, epochs=5, lr=0.002)
        torch.save(model.state_dict(), f"checkpoints/ptbxl/{name}_ft.pth")
        print(f"Saved checkpoints/ptbxl/{name}_ft.pth")

    print("\n✅ Successfully trained all 6 architectures and saved checkpoints.")
    print("=" * 60)
