...
class NeuralODEModel:
    ...
    def __init__(self, input_dim, hidden_dim=64, num_layers=3):
        torch.manual_seed(42)
        self.ode_func = ODEFunc(input_dim, hidden_dim, num_layers).to(dtype=torch.get_default_dtype())
    ...
    def fit(self, t, X_obs, epochs=500, lr=0.001):
        ...
        for epoch in range(epochs):
            ...
            if (epoch + 1) % 100 == 0 or epoch == 0:
                print(f"[Neural ODE] Epoch {epoch + 1:3d}/{epochs:3d} | Loss (MSE): {loss.item():.6f}")
...
def train_neural_ode_on_system(system_name, n_timesteps=1000, epochs=300):
    ...
    if name == "lorenz":
        ...
        sys_data = synthetic_systems.generate_vanderpol(n_timesteps=n_timesteps, dt=0.01)
    ...
    relative_l2_error = np.linalg.norm(X_test - X_pred_test) / (np.linalg.norm(X_test) + 1e-8)
