...
def main():
    parser = argparse.ArgumentParser(
        description="Create a baseline snapshot from the current massive sweep report"
    )
    parser.add_argument(
        "--name",
        type=str,
    ...
    if index_path.exists():
        try:
            ...
            print(
                f"⚠️ Warning: Could not parse baseline_index.json ({e}). Re-initializing index."
            )
    ...
    new_entry = {"name": args.name, "timestamp": timestamp, "source": source_filename}
    ...
    try:
        ...
        print(
            f"✅ Updated baseline_index.json at {index_path} with baseline '{args.name}'"
        )
    except Exception as e:
