...
def main():
    ...
    try:
        ...
        assert orig_features.shape == (20, 8), f"Incorrect original shape: {orig_features.shape}"
        assert ext_features.shape == (20, 15), f"Incorrect extended shape: {ext_features.shape}"
        ...
        assert nan_fraction_orig <= 0.5, f"Excessive NaNs in original space: {nan_fraction_orig * 100:.2f}%"
        assert nan_fraction_ext <= 0.5, f"Excessive NaNs in extended space: {nan_fraction_ext * 100:.2f}%"
    ...
    try:
        ...
        assert len(df_cka) == len(snr_levels), f"DataFrame must have {len(snr_levels)} rows (one per SNR level)."
        assert "cka_original" in df_cka.columns, "DataFrame must contain 'cka_original'."
        assert "cka_extended" in df_cka.columns, "DataFrame must contain 'cka_extended'."
