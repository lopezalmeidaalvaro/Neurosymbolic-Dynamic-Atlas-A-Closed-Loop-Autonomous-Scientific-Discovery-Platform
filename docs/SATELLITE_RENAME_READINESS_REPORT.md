...
Do not rename `satellite/` to `satellite/` in this execution. Readiness is not high enough for a safe physical folder rename without additional compatibility wrappers and test coverage.

## Evidence From Import Inventory

| Signal | Value |
| --- | ---: |
| Import edges from `satellite/` | 34 |
...
| Risk | Level | Reason |
| --- | --- | --- |
| Package path confusion | High | Folder is `satellite/`, while many imports already target `satellite.*`. |
...
1. Add a compatibility package or import alias so `satellite` and `satellite` both resolve during transition.
