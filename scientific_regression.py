...
def main():
    parser = argparse.ArgumentParser(description="Evaluate scientific regressions against baseline snapshots")
    parser.add_argument(
        "--baseline",
        type=str,
        default=None,
    ...
    print(f"🔍 Comparing against baseline: '{selected_baseline['name']}' (timestamp: {selected_baseline['timestamp']})")
    ...
    baseline_systems = {r["system"]: r for r in baseline_sweep.get("certified_results", [])}
    current_systems = {r["system"]: r for r in current_sweep.get("certified_results", [])}
    
    systems_report = []
    global_status = "pass"
    
    all_systems = sorted(list(set(list(baseline_systems.keys()) + list(current_systems.keys()))))
    ...
    for system in all_systems:
        ...
        if not b_res:
            print(f"ℹ️ System '{system}' is new in current sweep (not present in baseline).")
        ...
        acc_std_pct = (abs(acc_std_delta) / b_acc_std * 100.0) if b_acc_std != 0 else 0.0
    ...
    try:
        ...
        print(f"✅ Scientific regression report successfully written to {report_output_path}")
    except Exception as e:
    ...
    for sys_data in systems_report:
        ...
        for m_name, m_data in metrics.items():
            stat_marker = "✓" if m_data["status"] == "pass" else ("⚠️" if m_data["status"] == "warning" else "❌")
            if m_name == "reproducibility_status":
                print(f"  {stat_marker} {m_name:<22}: {m_data['baseline']} -> {m_data['current']} (status: {m_data['status']})")
            else:
                pct_str = f"({m_data['pct_change']:.2f}%)" if "pct_change" in m_data else ""
                print(f"  {stat_marker} {m_name:<22}: baseline={m_data['baseline']:.6f} | current={m_data['current']:.6f} | delta={m_data['delta']:+.6f} {pct_str} (status: {m_data['status']})")
                
    print("\n" + "=" * 60)
    
    if global_status == "failure":
        print("❌ Scientific regression check failed. Code modifications caused unacceptable performance drop.")
        sys.exit(1)
    elif global_status == "warning":
        print("⚠️  Scientific regression check passed with warnings. Noticeable drift detected.")
        sys.exit(0)
    else:
