...
def main():
    ...
    rows = conn.execute("SELECT embedding_json FROM structural_embeddings WHERE system_name='logistic_sweep'").fetchall()
    ...
    scatter = plt.scatter(pca_result[:, 0], pca_result[:, 1], c=r_vals, cmap='viridis', s=20)
    ...
    for i in range(1, len(delta_d)-1):
        if delta_d[i] > delta_d[i-1] and delta_d[i] > delta_d[i+1] and delta_d[i] > mean_d + 0.5*std_d:
            peaks.append(i)
    ...
    plt.scatter(peak_r, peak_d, color='red', zorder=5, label='Bifurcaciones detectadas (Picos)')
    for r, d in zip(peak_r, peak_d):
        plt.axvline(x=r, color='red', linestyle='--', alpha=0.3)
        
    if onset_idx is not None:
        r_chaos = r_vals[onset_idx]
        plt.axvline(x=r_chaos, color='black', linestyle=':', label=f'Onset of Chaos ($r \\approx {r_chaos:.4f}$)')
