...
def main():
    ...
    if not os.path.exists(data_path):
        print("[ERROR] No se encontró latent_curvature_data.json. Ejecuta latent_curvature.py primero.")
        return
    ...
    scatter = plt.scatter(pca_coords[:, 0], pca_coords[:, 1], c=log_kappa, cmap='inferno', s=8, alpha=0.8)
    plt.colorbar(scatter, label='Log10(Curvatura $\kappa$)')
    plt.title('Geometría Diferencial: Curvatura Discreta en PCA')
    plt.xlabel('PC1')
    plt.ylabel('PC2')
    
    plt.savefig(os.path.join(ARTIFACTS_DIR, "latent_curvature.png"), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.imshow(log_area, aspect='auto', origin='lower', cmap='viridis',
    ...
    plt.savefig(os.path.join(ARTIFACTS_DIR, "latent_metric_density.png"), dpi=300, bbox_inches='tight')
    plt.close()
