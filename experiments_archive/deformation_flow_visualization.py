...
def main():
    ...
    if not os.path.exists(data_path):
        print("[ERROR] El archivo de proyección JSON no existe. Corre deformation_flow.py primero.")
        return
    ...
    scatter = plt.scatter(pca_coords[:, 0], pca_coords[:, 1], c=p_vals, cmap='plasma', s=5, alpha=0.7)
    ...
    for i in range(0, n_r, max(1, n_r // 15)):
        ...
        plt.plot(pca_coords[start_idx:end_idx, 0], pca_coords[start_idx:end_idx, 1], color='gray', alpha=0.3, linewidth=0.8)
        
    plt.savefig(os.path.join(ARTIFACTS_DIR, "deformation_flow_pca.png"), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.imshow(v_grid, aspect='auto', origin='lower', cmap='magma',
    ...
    plt.savefig(os.path.join(ARTIFACTS_DIR, "deformation_flow_velocity.png"), dpi=300, bbox_inches='tight')
    plt.close()
