...
def main():
    ...
    if not os.path.exists(data_path):
        print("[ERROR] No se encontró geodesic_flow_data.json. Ejecuta geodesic_flow.py primero.")
        return
    ...
    scatter = plt.scatter(pca_coords[:, 0], pca_coords[:, 1], c=distances, cmap='GnBu', s=5, alpha=0.8)
    ...
    # Seed
    plt.scatter(pca_coords[0, 0], pca_coords[0, 1], color='red', s=50, marker='*', label='Semilla Estable')
    plt.legend()
    plt.savefig(os.path.join(ARTIFACTS_DIR, "geodesic_field.png"), dpi=300, bbox_inches='tight')
    plt.close()
    
    # 2. Divergencia Exponencial Geodésica
    plt.figure(figsize=(10, 8))
    scatter2 = plt.scatter(pca_coords[:, 0], pca_coords[:, 1], c=lambda_g, cmap='coolwarm', s=5, alpha=0.8)
    plt.colorbar(scatter2, label='Exponente Geodésico ($\lambda_g$)')
    plt.title('Divergencia Exponencial: Caos Geométrico Local')
    plt.xlabel('PC1')
    plt.ylabel('PC2')
    plt.savefig(os.path.join(ARTIFACTS_DIR, "geodesic_divergence.png"), dpi=300, bbox_inches='tight')
    plt.close()
    
    # 3. Operadores de Renormalización (Ciclos de Auto-Similitud)
    plt.figure(figsize=(10, 5))
    x_axis = np.arange(len(renorm_scores))
    plt.plot(x_axis, renorm_scores, color='purple', label='Similitud Fractal (DTW / Norm)')
    ...
    plt.legend()
    plt.savefig(os.path.join(ARTIFACTS_DIR, "renormalization_cycles.png"), dpi=300, bbox_inches='tight')
    plt.close()
