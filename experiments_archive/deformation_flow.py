...
def main():
    ...
    print(f"1. Generando trayectorias vectorizadas (n_r={n_r}, n_p={n_p}, total={n_r*n_p})...")
    ...
    with open(os.path.join(ARTIFACTS_DIR, "deformation_flow_projection.json"), "w") as f:
    ...
    conn.execute(
        ...
        ("latent_deformation_geometry", "Continuous structural deformation applied to maps", "classify_dynamic_systems_by_geometric_flow_not_static_equation", 0.95, "[]", "[]", "['topology_flow']")
    )
    conn.commit()
    conn.close()
    ...
    if os.environ.get("EVAL_CALLED") != "1":
        cmd = [
            ...
            "eval", "none", "topology_flow", "scikit-learn",
            os.path.join("temp_scripts", "deformation_flow.py"),
            "--artifact", "deformation_flow|artifacts/deformation_flow_projection.json",
            "--notes", "Fase 11A: Construcción del flujo geométrico continuo entre familias dinámicas deformadas."
        ]
        env = os.environ.copy()
        env["EVAL_CALLED"] = "1"
