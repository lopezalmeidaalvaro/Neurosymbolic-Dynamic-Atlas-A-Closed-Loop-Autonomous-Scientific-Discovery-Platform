...
print(f"Puntos de equilibrio encontrados:")
...
for r_val in rho_vals:
    ...
    eqs_sub = [(eq[0].subs({beta: beta_val, rho: r_val}), 
                eq[1].subs({beta: beta_val, rho: r_val}), 
                eq[2].subs({beta: beta_val, rho: r_val})) for eq in equilibria]
    ...
    for i, eq in enumerate(eqs_sub):
        try:
            ...
            J_eq = J.subs({x: eq[0], y: eq[1], z: eq[2], sigma: sigma_val, beta: beta_val, rho: r_val})
