...
def main():
    ...
    if idx_lorenz and idx_logistic:
        ...
        centroid_logistic = (
            np.mean(chaotic_logistic, axis=0)
            if len(chaotic_logistic) > 0
            else vec_lorenz
        )
...
"""
    for lbl, sys_list in cluster_report.items():
        report_content += (
            f"- Familia {lbl if lbl != -1 else 'Singular/Aislada'}: {sys_list}\n"
        )

    os.makedirs(ARTIFACTS_DIR, exist_ok=True)
    with open(
        os.path.join(ARTIFACTS_DIR, "universal_atlas_report.md"), "w", encoding="utf-8"
    ) as f:
    ...
    if universality:
        conn.execute(
            ...
            (
                "continuous_discrete_universality",
                "High cosine similarity > 0.80 between continuous 3D attractors and discrete chaotic maps",
                "classify_chaotic_systems_via_intrinsic_geometry",
                0.96,
                "[]",
                "[]",
                "['universal_dynamics']",
    ...
    if os.environ.get("EVAL_CALLED") != "1":
        cmd = [
            ...
            "--artifact",
            "universal_atlas|artifacts/universal_atlas_report.md",
            "--notes",
            "Fase 13: Integración de atractores caóticos continuos y búsqueda de universalidad geométrica entre sistemas dinámicos.",
        ]
        env = os.environ.copy()
        env["EVAL_CALLED"] = "1"
