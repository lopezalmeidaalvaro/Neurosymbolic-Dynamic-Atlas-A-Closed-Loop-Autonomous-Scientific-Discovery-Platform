...
def main():
    ...
    print(f"Ejecutando barrido paramétrico con deformación estructural (N={len(r_vals)})...")
    ...
    if mean_similarity >= 0.85:
        verdict = "survived"
        print("  [VEREDICTO] La conjetura SOBREVIVE. El clúster absorbe la deformación estructural.")
        new_status = "validated_under_deformation"
        new_confidence = 0.99
    else:
        verdict = "falsified"
        print("  [VEREDICTO] La conjetura es FALSADA. El clúster se fractura ante asimetrías no lineales.")
    ...
    if os.environ.get("EVAL_CALLED") != "1":
        cmd = [
            ...
            "eval", "none", "epistemological_engine", "scipy",
            os.path.join("temp_scripts", "falsification_002.py"),
            "--artifact", "falsification_report|artifacts/falsification_002_results.json",
            "--notes", "Fase 10B: Falsación de universalidad topológica. Inyección de asimetría estructural en mapa logístico para testear la resistencia de clústeres en el espacio latente."
        ]
        env = os.environ.copy()
        env["EVAL_CALLED"] = "1"
