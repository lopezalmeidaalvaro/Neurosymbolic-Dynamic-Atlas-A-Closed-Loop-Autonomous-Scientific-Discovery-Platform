...
def main():
    ...
    print(f"  Nueva correlación (con ruido) : r = {r_pearson:.4f} (p-value={p_value:.4e})")
    
    if r_pearson >= 0.80:
        verdict = "survived"
        print("  [VEREDICTO] La conjetura SOBREVIVE. Es un invariante topológico robusto.")
        new_status = "validated_under_noise"
        new_confidence = 0.99
    else:
        verdict = "falsified"
        print("  [VEREDICTO] La conjetura es FALSADA. Era un artefacto estadístico frágil.")
    ...
    if os.environ.get("EVAL_CALLED") != "1":
        cmd = [
            ...
            "--artifact", "falsification_report|artifacts/falsification_001_results.json",
            "--notes", "Fase 10: Ejecución de falsación estocástica. Inyección de ruido Gaussiano (sigma=0.1) en Mapa Logístico para testear robustez de la correlación Varianza-Energía."
        ]
        env = os.environ.copy()
        env["EVAL_CALLED"] = "1"
