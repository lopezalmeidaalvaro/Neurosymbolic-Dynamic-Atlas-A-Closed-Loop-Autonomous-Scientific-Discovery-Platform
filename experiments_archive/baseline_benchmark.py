...
# Importaciones de sktime (Estado del Arte)
from sktime.transformations.panel.rocket import Rocket
from sktime.classification.distance_based import KNeighborsTimeSeriesClassifier

def generate_synthetic_data(n_samples_per_class=50, length=200):
    ...
    for _ in range(n_samples_per_class):
        ...
        x_per = np.sin(freq * t + phase) + np.random.normal(0, 0.1, length)
        X.append(x_per)
        y.append(1)
        
        # Clase 2: Ruido (Ruido Blanco Gaussiano)
        x_noise = np.random.normal(0, 1, length)
        X.append(x_noise)
        y.append(2)
...
def main():
    ...
    print("\n[1] Generando dataset sintético (Caos, Periódico, Ruido)...")
    X, y = generate_synthetic_data(n_samples_per_class=50, length=200)
