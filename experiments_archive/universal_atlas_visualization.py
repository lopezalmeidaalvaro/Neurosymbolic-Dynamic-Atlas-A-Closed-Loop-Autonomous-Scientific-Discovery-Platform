...
def main():
    ...
    scatter = plt.scatter(pca_coords[:, 0], pca_coords[:, 1], c=labels, cmap='tab20', s=40, alpha=0.8, edgecolors='w')
    ...
    for un in unique_names:
        ...
        plt.annotate(un, (centroid[0], centroid[1]), fontsize=9, fontweight='bold', ha='center',
    ...
    plt.savefig(os.path.join(ARTIFACTS_DIR, "universal_atlas_pca.png"), dpi=300, bbox_inches='tight')
    plt.close()
    
    # 2. Geodésicas Universales (Divergencia lambda_g)
    plt.figure(figsize=(10, 8))
    sc = plt.scatter(pca_coords[:, 0], pca_coords[:, 1], c=lambda_g, cmap='coolwarm', s=50, alpha=0.8, edgecolors='k')
    plt.colorbar(sc, label='Divergencia Geodésica Universal ($\lambda_g$)')
    plt.title('Geometría del Caos: Expansión Exponencial Compartida')
    plt.xlabel('PC1')
    plt.ylabel('PC2')
    plt.grid(True, linestyle='--', alpha=0.3)
    plt.savefig(os.path.join(ARTIFACTS_DIR, "universal_geodesics.png"), dpi=300, bbox_inches='tight')
    plt.close()
    ...
    plt.figure(figsize=(10, 8))
    sc2 = plt.scatter(pca_coords[:, 0], pca_coords[:, 1], c=log_curv, cmap='magma', s=50, alpha=0.8, edgecolors='k')
    plt.colorbar(sc2, label='Log10(Curvatura Local $\kappa$)')
    plt.title('Singularidades Topológicas y Regiones Hiperbólicas')
    plt.xlabel('PC1')
    plt.ylabel('PC2')
    plt.grid(True, linestyle='--', alpha=0.3)
    plt.savefig(os.path.join(ARTIFACTS_DIR, "curvature_clusters.png"), dpi=300, bbox_inches='tight')
    plt.close()
