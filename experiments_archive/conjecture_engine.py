...
def main():
    ...
    rows = conn.execute("SELECT system_name, lyapunov_max, spectral_entropy, dominant_frequency, variance, autocorr_decay, kurtosis, skewness, energy FROM structural_embeddings").fetchall()
        ...
        for j in range(i+1, len(features)):
            ...
            if abs(r_val) > 0.85:
                ...
                conjectures.append({
                    ...
                    "evidence_json": {"pearson_r": r_val, "p_value": p_val, "n_samples": len(matrix_scaled)}
                })
        ...
        for label, names in clusters.items():
            ...
            if len(unique_names) > 1:
                ...
                conjectures.append({
                    ...
                    "contradictory_systems": [],
                    "evidence_json": {"silhouette_score": best_sil, "cluster_size": len(names), "cluster_variance": float(cluster_var)}
                })
    ...
    for conj in conjectures:
        ...
        if conj["confidence_score"] > 0.6:
            experiment_count += 1
            exp_path = os.path.join(ARTIFACTS_DIR, f"generated_experiment_{experiment_count:03d}.md")
            with open(exp_path, "w", encoding="utf-8") as f:
                f.write(f"# Diseño de Experimento: Falsación de Conjetura {experiment_count}\n\n")
                ...
                f.write("> **Ejecuta lo siguiente para intentar romper esta conjetura:**\n")
                f.write(r"> 1. Introduce ruido estocástico (e.g., ruido Gaussiano $\sigma=0.1$) en las ecuaciones de estos sistemas y recalcula el embedding estructural." + "\n")
                f.write(r"> 2. Realiza un barrido paramétrico extremo fuera del régimen actual para forzar transiciones de fase desconocidas." + "\n")
                f.write(r"> 3. Verifica si la correlación o el clúster se disuelven (si la distancia coseno cae drásticamente o la correlación baja de 0.8)." + "\n\n")
