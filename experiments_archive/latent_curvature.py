...
def main():
    ...
    conn.execute(
        ...
        ("latent_metric_curvature", "High phase transitions corresponding to singular regions in latent space", "detect_phase_transitions_via_geometric_curvature", 0.98, "[]", "[]", "['differential_geometry']")
    )
    conn.commit()
    conn.close()
    ...
    if os.environ.get("EVAL_CALLED") != "1":
        cmd = [
            ...
            "--artifact", "latent_curvature|artifacts/latent_curvature.png",
            "--notes", "Fase 11B: Geometría diferencial y curvatura del espacio dinámico latente."
        ]
        env = os.environ.copy()
        env["EVAL_CALLED"] = "1"
